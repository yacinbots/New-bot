...
function ooredooCurl(string $url, string $method, $payload, array $headers, string $tag, int $timeout = 15): array
{
    ...
    file_put_contents('/tmp/ooredoo.log',
        date('Y-m-d H:i:s') . " [$tag] CODE:{$httpCode} ERR:{$err}"
        . " HDR_LINES:" . count($capturedHeaders)
...
function ooCheckpoint(string $msisdn, int $which): array
{
    ...
    file_put_contents('/tmp/ooredoo.log',
        date('Y-m-d H:i:s') . " [oo_chk{$which}] code={$r['code']} nonce={$nonce} chronos={$chronos}\n",
        FILE_APPEND
    );
...
{
    // phone = 05xxxxxxxx → msisdn = 213 + 5xxxxxxxx
    $msisdn = '213' . substr($phone, 1);

    sendMessage($psid, "🔄 جاري التحقق من الرقم {$phone}...");

    $result = ooSendOTP($msisdn);

    file_put_contents('/tmp/ooredoo.log',
        date('Y-m-d H:i:s') . " [handleOoredooPhone] msisdn={$msisdn} ok=" . ($result['ok'] ? '1' : '0') . " reason=" . ($result['reason'] ?? '') . "\n",
        FILE_APPEND
    );
