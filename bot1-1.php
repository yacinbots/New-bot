...
function ooredooCurl(string $url, string $method, $payload, array $headers, string $tag, int $timeout = 15): array
{
    // نجمع response headers بـ callback — أضمن طريقة بغض النظر عن gzip
    $capturedHeaders = [];
    $ch = curl_init($url);
    ...
    $opts = [
        ...
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_MAXREDIRS      => 3,
        // لا CURLOPT_ENCODING هنا — نتركه manual حتى لا يخرب HEADER_SIZE
        CURLOPT_HEADERFUNCTION => function ($ch, $line) use (&$capturedHeaders) {
            $capturedHeaders[] = rtrim($line);
            return strlen($line);
        },
    ];
    ...
    // فك gzip يدوياً إذا كان body مضغوطاً
    ...
    dbg("[$tag] CODE:{$httpCode} ERR:{$err} BODY:" . substr($body, 0, 200));
...
function handleOoredooPhone(string $psid, string $phone): void
{
    ...
    setSession($psid, [
        'state'    => 'oo_awaiting_otp',
        'msisdn'   => $msisdn,
        'nonce1'   => $result['nonce1'],
        'chronos1' => $result['chronos1'],
    ]);
