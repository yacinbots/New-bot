...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — Session-Pinned IP Pool + HTML=block detection + detailed logging
// ════════════════════════════════════════════════════════════════════════════

define('OOR_LOG',          '/tmp/oor.log');
define('OOR_PINNED_FILE',  '/tmp/oor_pinned_ip.json');

/**
 * oorLog — تسجيل مفصل مع كود الاستجابة والـ IP
 */
function oorLog(string $tag, string $proxyHost, int $httpCode, string $msg, string $body = ''): void
{
    $entry = date('Y-m-d H:i:s')
        ...
        . " | " . $msg;
    if ($body !== '') {
        $entry .= " | body=" . substr($body, 0, 300);
    }
    file_put_contents(OOR_LOG, $entry . "\n", FILE_APPEND);
}

/**
 * isHtmlResponse — هل الاستجابة HTML؟ = قيد IP مؤقت
 */
function isHtmlResponse(string $body): bool
{
...
/**
 * getPinnedProxyForSession — نفس الـ session key يستخدم نفس الـ proxy host
 * session_key = msisdn + url_path مجزأة
 * يضمن أن checkpoint + play يستخدمان نفس IP
 */
function getPinnedProxyForSession(string $sessionKey): ?string
{
    if (!file_exists(OOR_PINNED_FILE)) return null;
    $data = @json_decode(file_get_contents(OOR_PINNED_FILE), true);
    if (!is_array($data)) return null;
    $pin = $data[$sessionKey] ?? null;
    if (!$pin) return null;
    // صلاحية الـ pin: 10 دقائق
    if (time() - ($pin['ts'] ?? 0) > 600) return null;
    return $pin['proxy'] ?? null;
}

function setPinnedProxyForSession(string $sessionKey, string $proxy): void
{
    $data = [];
    if (file_exists(OOR_PINNED_FILE)) {
        $data = @json_decode(file_get_contents(OOR_PINNED_FILE), true) ?? [];
    }
    // تنظيف القديم
    foreach ($data as $k => $v) {
        if (time() - ($v['ts'] ?? 0) > 600) unset($data[$k]);
    }
    $data[$sessionKey] = ['proxy' => $proxy, 'ts' => time()];
    file_put_contents(OOR_PINNED_FILE, json_encode($data));
}

function clearPinnedProxy(string $sessionKey): void
{
    if (!file_exists(OOR_PINNED_FILE)) return;
    $data = @json_decode(file_get_contents(OOR_PINNED_FILE), true) ?? [];
...
/**
 * oorCurlSingle — طلب واحد عبر proxy محدد، مع استخراج headers
 */
function oorCurlSingle(
    string $url,
    string $method,
    string $body,
    array  $headers,
    ...
    $opts = [
        ...
        CURLOPT_HTTPHEADER     => $headers,
        CURLOPT_ENCODING       => 'gzip',
        CURLOPT_FOLLOWLOCATION => false, // لا نتبع redirect حتى نرى headers صح
        CURLOPT_HEADER         => true,
        CURLOPT_PROXY          => $pp['host'],
        CURLOPT_PROXYUSERPWD   => $pp['userpass'],
        CURLOPT_PROXYTYPE      => CURLPROXY_HTTP,
    ...
    $proxyIp    = curl_getinfo($ch, CURLINFO_PRIMARY_IP) ?: $pp['host'];
    $err        = curl_error($ch);
    $errno      = curl_errno($ch);
    curl_close($ch);

    if ($errno || $raw === false) {
        return ['ok' => false, 'http_code' => 0, 'body' => '', 'json' => null,
                'resp_headers' => [], 'err' => $err, 'proxy_ip' => $proxyIp];
    }
    ...
    $json = null;
    if (!empty($bodyStr) && !isHtmlResponse($bodyStr)) {
    ...
    return [
        ...
        'err'          => '',
        'proxy_ip'     => $proxyIp,
        'is_html'      => isHtmlResponse($bodyStr),
    ];
}

/**
 * oorCurl — المدخل الرئيسي:
 *  - يشتق session_key من الـ URL path لربط الطلبات المتسلسلة بنفس IP
 *  - إذا 200 + HTML → قيد IP مؤقت → يجرب proxy آخر (لا يضغط على نفس الـ IP)
 *  - إذا 500 → يعيد فوراً (كود خاص يعالجه المتصل)
 *  - يسجل كل شيء: proxy IP + http code + body مختصر
 */
function oorCurl(string $url, string $method, string $body, array $headers): array
{
    $proxies      = getOorProxies();
    $totalProxies = count($proxies);
    $empty        = ['http_code' => 0, 'body' => '', 'json' => null, 'resp_headers' => []];

    if ($totalProxies === 0) {
        oorLog('OOR', 'none', 0, 'لا توجد بروكسيات لأوريدو!');
        return $empty;
    }

    // session_key = path segment for IP pinning
    $urlPath    = parse_url($url, PHP_URL_PATH) ?? $url;
    $sessionKey = md5($urlPath);

    // ─── محاولة الـ proxy المربوط بهذه الجلسة أولاً ─────────────────────────
    $pinned = getPinnedProxyForSession($sessionKey);
    if ($pinned !== null) {
        $r = oorCurlSingle($url, $method, $body, $headers, $pinned);
        oorLog('OOR-PINNED', $r['proxy_ip'] ?? $pinned, $r['http_code'],
            "method={$method} url={$url}", $r['body'] ?? '');

        if ($r['ok'] && !($r['is_html'] ?? false) && $r['http_code'] !== 0) {
            // أعد الاستجابة مباشرة (بما فيها 500 ليعالجها المتصل)
            return [
        ...
        // HTML أو فشل → ألغ الـ pin وجرب proxies أخرى
        clearPinnedProxy($sessionKey);
        oorLog('OOR-PINNED-FAIL', $r['proxy_ip'] ?? $pinned, $r['http_code'],
            'HTML أو فشل اتصال — إلغاء الربط');
    }

    // ─── دوران على البروكسيات ──────────────────────────────────────────────
    $blockedByHtml = 0;

    // خلط البروكسيات للتوزيع العشوائي
    $shuffled = $proxies;
    shuffle($shuffled);

    foreach ($shuffled as $idx => $proxy) {
        $r = oorCurlSingle($url, $method, $body, $headers, $proxy);

        oorLog('OOR', $r['proxy_ip'] ?? $proxy, $r['http_code'],
            "[{$idx}/{$totalProxies}] method={$method} url={$url} err=" . ($r['err'] ?? ''),
            $r['body'] ?? '');

        if (!$r['ok'] || $r['http_code'] === 0) {
            // فشل اتصال — جرب التالي
            continue;
        }

        // HTML = قيد IP على هذا الـ proxy → تخطي فوراً
        if ($r['is_html'] ?? false) {
            $blockedByHtml++;
            oorLog('OOR-HTML-BLOCK', $r['proxy_ip'] ?? $proxy, $r['http_code'],
                'قيد IP مؤقت (HTML) — تخطي إلى proxy آخر', substr($r['body'], 0, 150));
            continue;
        }

        // 500 → أعده مباشرة (المتصل هو من يقرر: snapchat = مفعل مسبقاً، gamification = لعب)
        if ($r['http_code'] === 500) {
            setPinnedProxyForSession($sessionKey, $proxy);
            return [
                'http_code'    => 500,
                'body'         => $r['body'],
                'json'         => $r['json'],
        ...
        // استجابة صالحة — ربط هذا الـ proxy بالـ session
        setPinnedProxyForSession($sessionKey, $proxy);
        return [
            'http_code'    => $r['http_code'],
            'body'         => $r['body'],
    ...
    // ─── كل البروكسيات أعادت HTML أو فشلت ───────────────────────────────────
    oorLog('OOR', 'ALL', 0,
        "جميع البروكسيات فشلت! HTML_blocks={$blockedByHtml}/{$totalProxies} url={$url}");

    if ($blockedByHtml >= $totalProxies) {
        return ['http_code' => 503, 'body' => 'IP block على جميع البروكسيات', 'json' => null, 'resp_headers' => []];
    }
    return $empty;
}
...
function handleTelegramUpdate(array $update): void
{
    ...
    switch ($text) {
        ...
        case '/cancel':
            ...
            break;
        case '/oorlog':
            handleTgOorLog($chatId);
            break;
        case '/clearoorlog':
            @file_put_contents(OOR_LOG, '');
...
function sendTgMainMenu(string $chatId): void
{
    ...
    $keyboard = [
        ...
        [
            ['text' => '📋 سجل أوريدو',  'callback_data' => 'tg_oorlog'],
            ['text' => '🗑️ مسح السجل',  'callback_data' => 'tg_clear_oorlog'],
        ],
    ];
...
function handleTgProxiesInput(string $chatId, string $text): void
{
    ...
    foreach ($list as $item) {
        $normalized = normalizeProxy((string)$item);
        if ($normalized !== null) {
            $valid[] = $normalized;
        } else {
            $invalid[] = trim((string)$item);
        }
    }

    if (empty($valid)) {
        tgSendMessage($chatId,
            "❌ لا توجد بروكسيات بصيغة صحيحة.\n\n"
          . "الصيغ المقبولة:\n"
          . "<code>http://host:port:user:pass</code>\n"
          . "<code>host:port:user:pass</code>\n"
          . "<code>user:pass@host:port</code>\n\n"
          . "مثال:\n<code>http://gate.kookeey.info:1000:user123:pass123</code>"
        );
        return;
    }
    ...
    $msg = "✅ تم حفظ <b>" . count($valid) . "</b> بروكسي (جازي) بنجاح!";
    if (!empty($invalid)) {
        $msg .= "\n\n⚠️ تم تجاهل " . count($invalid) . " سطر غير صالح.";
    }
...
function normalizeProxy(string $item): ?string
{
    $item = trim($item);
    if ($item === '') return null;

    // صيغة كاملة: http(s)://host:port:user:pass
    if (preg_match('#^https?://.+:\d+:.+:.+$#', $item)) {
        // تأكد من http:// وليس https://
        return preg_replace('#^https://#', 'http://', $item);
    }

    // صيغة بدون بروتوكول: host:port:user:pass
    if (preg_match('#^[^/\s]+:\d+:[^:\s]+:[^:\s]+$#', $item)) {
        return 'http://' . $item;
    }

    // صيغة user:pass@host:port
    if (preg_match('#^([^@]+)@([^:]+:\d+)$#', $item, $m)) {
        [$user, $pass] = explode(':', $m[1], 2) + ['', ''];
        return 'http://' . $m[2] . ':' . $user . ':' . $pass;
    }
...
function handleTgOorProxiesInput(string $chatId, string $text): void
{
    ...
    if (empty($valid)) {
        tgSendMessage($chatId,
          ...
          . "<code>host:port:user:pass</code>\n"
          . "<code>user:pass@host:port</code>\n\n"
          . "مثال:\n<code>http://31.59.20.176:6754:nhtbnigs:3tdfei3ngchg</code>"
        );
        return;
    }
    ...
    $msg = "✅ تم حفظ <b>" . count($valid) . "</b> بروكسي (أوريدو) بنجاح!";
    if (!empty($invalid)) {
        $msg .= "\n\n⚠️ تم تجاهل " . count($invalid) . " سطر غير صالح:\n";
        foreach (array_slice($invalid, 0, 5) as $inv) {
            $msg .= "<code>" . htmlspecialchars(substr($inv, 0, 60)) . "</code>\n";
        }
    }
    tgSendMessage($chatId, $msg);
    sendTgMainMenu($chatId);
...
// ─── Ooredoo Log Viewer ──────────────────────────────────────────────────────
function handleTgOorLog(string $chatId): void
{
    if (!file_exists(OOR_LOG)) {
        tgSendMessage($chatId, '📋 سجل أوريدو فارغ.');
        return;
    }
    $content = file_get_contents(OOR_LOG);
    ...
    // إرسال على شكل ملف نصي إذا طويل جداً
    if (strlen($text) > 3500) {
        $tmpFile = '/tmp/oor_log_export_' . time() . '.txt';
        file_put_contents($tmpFile, $text);
        $ch = curl_init(TG_API . '/sendDocument');
        $postData = [
            'chat_id'  => $chatId,
            'document' => new CURLFile($tmpFile, 'text/plain', 'oor_log.txt'),
            'caption'  => '📋 سجل أوريدو — آخر ' . count($lastLines) . ' سطر',
        ];
        curl_setopt_array($ch, [
            CURLOPT_POST           => true,
    ...
    $keyboard = [[['text'=>'🗑️ مسح السجل','callback_data'=>'tg_clear_oorlog']]];
    tgSendMessage($chatId, "<pre>" . htmlspecialchars($text) . "</pre>", $keyboard);
}
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — Auth-Only Curl (OOR_AUTH_URL)
//
// السياق: WAF أوريدو يحجب /openid-connect/token على بعض الـ subnets
// الاستراتيجية:
//   1) جرب كل بروكسيات أوريدو — إذا ردّ واحد بـ JSON صالح → استخدمه
//   2) إذا كل البروكسيات أعادت HTML (محجوبة WAF) → Direct IP للسيرفر بدون proxy
//   3) سجّل كل محاولة بـ tag [AUTH-PROXY] أو [AUTH-DIRECT]
// ════════════════════════════════════════════════════════════════════════════
function oorAuthCurl(string $url, string $payload, array $headers, string $tag): array
{
    $empty   = ['http_code' => 0, 'body' => '', 'json' => null, 'resp_headers' => []];
    $proxies = getOorProxies();
    $htmlCount = 0;

    // ─── المرور 1: كل البروكسيات ───────────────────────────────────────────
    foreach ($proxies as $idx => $proxy) {
        ...
        if ($errno || $raw === false || $code === 0) {
            oorLog("AUTH-PROXY-{$tag}", $proxyIp, 0,
                "[{$idx}] فشل اتصال: {$err}");
            continue;
        }
        ...
        if (isHtmlResponse($bodyStr)) {
            $htmlCount++;
            oorLog("AUTH-HTML-{$tag}", $proxyIp, $code,
                "[{$idx}] WAF block — تخطي", substr($bodyStr, 0, 120));
            continue;
        }

        // استجابة JSON صالحة عبر proxy
        $json = @json_decode($bodyStr, true);
        oorLog("AUTH-PROXY-OK-{$tag}", $proxyIp, $code,
            "[{$idx}] نجح عبر proxy", substr($bodyStr, 0, 200));
        return [
            'http_code'    => $code,
            'body'         => $bodyStr,
    ...
    // ─── المرور 2: Direct IP (بدون proxy) ──────────────────────────────────
    // يُستخدم فقط إذا كل البروكسيات أعادت HTML (WAF block على الـ subnets)
    // IP السيرفر المباشر لم يُقيَّد بعد
    oorLog("AUTH-DIRECT-{$tag}", 'server-ip', 0,
        "كل البروكسيات ({$htmlCount}) محجوبة WAF — جرب Direct IP");
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_SSL_VERIFYPEER => false,
        CURLOPT_FOLLOWLOCATION => false,
        // لا CURLOPT_PROXY — طلب مباشر من IP السيرفر
    ]);
    $raw  = curl_exec($ch);
    $code = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
    $ip   = curl_getinfo($ch, CURLINFO_PRIMARY_IP) ?: 'direct';
    $err  = curl_error($ch);
    curl_close($ch);

    if ($raw === false || $code === 0) {
        oorLog("AUTH-DIRECT-FAIL-{$tag}", $ip, 0, "فشل Direct: {$err}");
        return $empty;
    }
    ...
    if (isHtmlResponse($bodyStr)) {
        oorLog("AUTH-DIRECT-HTML-{$tag}", $ip, $code,
            "Direct أيضاً محجوب WAF — فشل كامل", substr($bodyStr, 0, 120));
        return $empty;
    }

    $json = @json_decode($bodyStr, true);
    oorLog("AUTH-DIRECT-OK-{$tag}", $ip, $code,
        "نجح عبر Direct IP", substr($bodyStr, 0, 200));
...
function oorSendSMS(string $msisdn): bool
{
    ...
    $r = oorAuthCurl(OOR_AUTH_URL, $payload, $h, 'SMS');
...
function oorVerifyOTP(string $msisdn, string $otp): mixed
{
    ...
    $r = oorAuthCurl(OOR_AUTH_URL, $payload, $h, 'OTP');
...
function oorRefreshToken(string $msisdn, string $refreshToken): mixed
{
    ...
    $r = oorAuthCurl(OOR_AUTH_URL, $payload, $h, 'REFRESH');
    if ($r['http_code'] === 200 && isset($r['json']['access_token']))
...
/**
 * oorGamificationPlay:
 *  1) checkpoint/token أولاً (نفس IP مربوط)
 *  2) gamification/play
 *  3) إذا 500 → يعني "لُعبت مسبقاً" (مثل snapchat) → نرجع للـ status للتحقق الحقيقي
 *  4) نُعيد الاستجابة الحقيقية من status إذا كان 500
 */
function oorGamificationPlay(string $msisdn, string $at): array
{
    // ─── الخطوة 1: checkpoint/token ───────────────────────────────────────
    ...
    oorLog('GAM-CHECKPOINT', 'n/a', $chkR['http_code'],
        "msisdn={$msisdn} nonce=" . ($nonce ?: 'EMPTY') . " chronos=" . ($chronos ?: 'EMPTY'));

    // ─── الخطوة 2: gamification/play (نفس IP بفضل session pinning) ────────
    ...
    $playR = oorCurl($url, 'GET', '', $h);

    oorLog('GAM-PLAY', 'n/a', $playR['http_code'],
        "msisdn={$msisdn} body=" . substr($playR['body'] ?? '', 0, 200));

    // ─── الخطوة 3: 500 = مثل snapchat → التحقق من الحالة الفعلية ──────────
    if ($playR['http_code'] === 500) {
        oorLog('GAM-500', 'n/a', 500,
            'استجابة 500 — يعني لُعبت أو مشكلة مؤقتة، نتحقق من status الحقيقي');

        $statusR = oorGamificationStatus($msisdn, $at);

        if (is_array($statusR)) {
            $played       = $statusR['played'] ?? false;
            $giftName     = $statusR['giftName'] ?? ($statusR['allocations'][0] ?? '');
            $lastPlayedTime = $statusR['lastPlayedTime'] ?? '';

            oorLog('GAM-STATUS-VERIFY', 'n/a', 200,
                "played={$played} giftName={$giftName} lastPlayed={$lastPlayedTime}");

            if ($played) {
                // بناء استجابة مركّبة واضحة
                return [
                    'http_code'    => 200,
                    'body'         => json_encode($statusR),
                    'json'         => array_merge($statusR, [
                        '_source'    => 'status_after_500',
                        'giftName'   => $giftName ?: '0Mo',
                        'validityHour'=> $statusR['validityHour'] ?? 0,
                        'playedTime' => $lastPlayedTime,
                    ]),
                    'resp_headers' => [],
                ];
            }
            // status يقول لم تُلعب — خطأ مؤقت فعلي في السيرفر
            return [
                'http_code' => 500,
                'body'      => 'server_error_not_played',
                'json'      => null,
                'resp_headers' => [],
            ];
        }

        // لم نستطع التحقق من status → نعيد 500 كما هو
        return $playR;
    }
...
function handleOorAwaitingOtp(string $psid, string $text, array $session): void
{
    ...
    // رقم أوريدو → إعادة إرسال OTP لأوريدو
    ...
    // رقم جيزي في مرحلة OTP أوريدو → تحويل لتدفق جيزي
    if (preg_match('/^07\d{8}$/', $digits)) {
        clearSession($psid);
...
function oorSendMenu(string $psid, string $msisdn, string $at): void
{
    ...
    $planType  = $userInfo['planType'] ?? null;
    $isYooz    = $planType
        ? in_array(strtoupper((string)$planType), ['YOOZ', "N'YOOZ", 'NYOOZ'])
        : false;

    // ─── حالة الهدية بناءً على status الحقيقي فقط ─────────────────────────
    $played       = false;
    $lastPlayedStr= '';
    $giftAvail    = false;   // لا نقل "متاحة" حتى نتأكد
    $remainStr    = '';

    if (is_array($gamStatus)) {
        $played        = (bool)($gamStatus['played'] ?? false);
        ...
        if ($played && $lastPlayedStr) {
            ...
            if ($remaining > 0) {
                ...
                $giftAvail = true; // مرت 24 ساعة
            }
        } elseif (!$played) {
            $giftAvail = true;
        }
        // إذا فشل gamStatus → $giftAvail يبقى false، الزر موجود لكن بدون تأكيد
    }

    // ─── بناء label الهدية ────────────────────────────────────────────────
    if ($gamStatus === null) {
        $giftLabel = "1️⃣ الهدية اليومية ❓ (تحقق يدوي)";
    } elseif ($giftAvail) {
        $giftLabel = "1️⃣ الهدية اليومية ✅ (متاحة)";
    } else {
        $giftLabel = "1️⃣ الهدية اليومية ⏳" . ($remainStr ? " (بعد {$remainStr})" : '');
    }

    // ─── بناء الرسالة — بدون رقم أو نوع شريحة ────────────────────────────
    ...
    // ─── الأزرار ──────────────────────────────────────────────────────────
    $qr = [];
    if ($giftAvail || $gamStatus === null) {
        $qr[] = ['content_type'=>'text','title'=>'🎁 الهدية اليومية','payload'=>'OOR_DAILY_GIFT'];
    } else {
        $qr[] = ['content_type'=>'text','title'=>"⏳ " . ($remainStr ?: 'غير متاحة'),'payload'=>'OOR_DAILY_GIFT_WAIT'];
    }
...
function handleOorMenuText(string $psid, string $text, array $session): void
{
    ...
    // ─── إذا أرسل رقم هاتف أوريدو في أي مرحلة → إعادة التدفق من البداية ──
    if (preg_match('/^05\d{8}$/', $digits)) {
        handleOorNewPhone($psid, $digits);
        return;
    }

    // ─── إذا أرسل رقم جيزي → حوّله لتدفق جيزي ────────────────────────────
    if (preg_match('/^07\d{8}$/', $digits)) {
        handleNewPhone($psid, $digits);
        return;
    }
...
{
    // ─── فحص الحالة المخزنة في الجلسة أولاً ──────────────────────────────
    ...
    // ─── التحقق الحقيقي من السيرفر قبل الإعلان ──────────────────────────
    sendMessage($psid, "🔍 جاري التحقق من حالة الهدية...");
    $statusBefore = oorGamificationStatus($msisdn, $at);
    if (is_array($statusBefore) && ($statusBefore['played'] ?? false)) {
        // مُلعبت فعلاً — لا نعلن "جاهزة" كذباً
        $lastPlayedTime = $statusBefore['lastPlayedTime'] ?? '';
        $remaining = 0;
        if ($lastPlayedTime) {
            $remaining = max(0, 86400 - (time() - (int)@strtotime($lastPlayedTime)));
        }
        if ($remaining > 0) {
            $h = (int)($remaining / 3600); $m2 = (int)(($remaining % 3600) / 60);
            sendMessage($psid, "⏳ الهدية اليومية مُفعَّلة مسبقاً.\n\n⏰ تتاح بعد: {$h} ساعة و{$m2} دقيقة");
            // تحديث الجلسة بالوقت الحقيقي
            $sess2 = getSession($psid);
            setSession($psid, array_merge($sess2, [
                'oor_played'      => true,
                'oor_last_played' => $lastPlayedTime ?: date('c'),
            ]));
            return;
        }
    }
    ...
    if ($r['http_code'] === 200 && is_array($r['json'])) {
        $json     = $r['json'];
        $source   = $json['_source'] ?? 'play';
        $gift     = $json['giftName']     ?? '0Mo';
        $vh       = $json['validityHour'] ?? 0;
        $playedTs = $json['playedTime']   ?? $json['lastPlayedTime'] ?? date('c');

        // تسجيل الوقت الحقيقي
        $sess2 = getSession($psid);
        setSession($psid, array_merge($sess2, [
            'oor_played'      => true,
            'oor_last_played' => $playedTs,
        ]));

        if ($gift === '0Mo' || $gift === '0' || $gift === '') {
            if ($source === 'status_after_500') {
                sendMessage($psid, "ℹ️ الهدية مُفعَّلة مسبقاً — لم تربح شيئاً جديداً اليوم.");
            } else {
                sendMessage($psid, "☹️ حظ سيئ! لم تربح شيئاً اليوم.");
            }
        } else {
            if ($source === 'status_after_500') {
                sendMessage($psid, "🎉 هديتك اليوم: {$gift}" . ($vh ? " — صالحة {$vh} ساعة" : '') . "\n✅ تم التحقق من السيرفر مباشرة.");
            } else {
                sendMessage($psid, "🎉 تهانينا! لقد ربحت {$gift}" . ($vh ? " صالحة لمدة {$vh} ساعة" : ''));
            }
        }
        return;
    }

    // 500 = خطأ حقيقي في السيرفر (لم تُلعب)
    if ($r['http_code'] === 500) {
        sendMessage($psid, "⚠️ السيرفر يواجه مشكلة مؤقتة، حاول لاحقاً.");
        return;
    }

    // 503 = IP block على جميع البروكسيات
    if ($r['http_code'] === 503) {
        sendMessage($psid, "⚠️ هناك ضغط على شبكة أوريدو حالياً، جرب بعد دقائق.");
        return;
    }

    sendMessage($psid, "❌ خطأ غير متوقع (HTTP {$r['http_code']}). حاول لاحقاً.");
}
