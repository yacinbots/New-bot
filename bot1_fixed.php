...
{
    if (!file_exists(OOREDOO_PROXY_ENABLED_FILE)) return true; // افتراضي: مفعّل
...
// ════════════════════════════════════════════════════════════════════════════
/**
 * ooredooGetStickyProxy — يُعيد:
 *   null          → لا يوجد sticky أو انتهت صلاحيته
 *   ['_direct'=>true, 'ts'=>...] → كان الاتصال مباشراً (بدون بروكسي)
 *   ['host'=>..., 'userpass'=>..., 'ts'=>...] → بروكسي محدد
 */
...
/**
 * ooredooSetStickyProxy — $pp = null يعني اتصال مباشر (direct)
 */
function ooredooSetStickyProxy(string $psid, ?array $pp): void
{
    if ($pp === null || ($pp['_direct'] ?? false) || !isset($pp['host'])) {
        // اتصال مباشر — نخزّن علامة _direct
        $data = ['_direct' => true, 'ts' => time()];
    } else {
        $data = array_merge($pp, ['ts' => time()]);
    }
...
/**
 * stickyToProxy — يحوّل sticky array إلى proxy قابل للاستخدام في curl
 * يُعيد null إذا كان direct, أو ['host','userpass'] إذا كان بروكسي
 */
function stickyToProxy(?array $sticky): ?array
{
    if ($sticky === null) return null;
    if ($sticky['_direct'] ?? false) return null;  // direct connection
    if (!isset($sticky['host'])) return null;
    return ['host' => $sticky['host'], 'userpass' => $sticky['userpass'] ?? ''];
}
...
function ooredooLog(string $msg): void
{
    ...
    file_put_contents(OOREDOO_LOG, $line, FILE_APPEND);
    // Mirror critical errors to debug log
    if (str_contains($msg, 'BLOCKED') || str_contains($msg, 'ALL') || str_contains($msg, 'fail')) {
        file_put_contents('/tmp/ooredoo_errors.log', $line, FILE_APPEND);
    }
}

/**
 * ooredooProxyLabel — returns short label for log lines: "gate.kookeey.info:1000" or "direct"
 */
function ooredooProxyLabel(?array $proxy): string
{
    if ($proxy === null || ($proxy['_direct'] ?? false)) return 'direct';
...
function ooredooCurlRaw(
    ...
    $proxyLabel = ooredooProxyLabel($proxy);

    if ($errno || $raw === false) {
        ooredooLog("[{$logTag}] cURL error errno={$errno} msg={$errMsg} proxy={$proxyLabel}");
        return null;  // triggers proxy rotation in caller
    }
    ...
    // فحص IP block: HTTP 200/429 + HTML = محجوب أو rate-limited
    $isHtml = (stripos($body, '<!DOCTYPE') !== false || stripos($body, '<html') !== false);
    if ($isHtml) {
        ooredooLog("[{$logTag}] IP_BLOCKED http={$httpCode} proxy={$proxyLabel}");
        return ['ip_blocked' => true];
    }

    // 429 Too Many Requests → يتم معالجته في المتصل (rotate proxy)
    if ($httpCode === 429) {
        ooredooLog("[{$logTag}] RATE_LIMITED http=429 proxy={$proxyLabel}");
        return ['ip_blocked' => true]; // نعامله كـ block لإجبار التناوب
    }
    ...
    ooredooLog("[{$logTag}] http={$httpCode} proxy={$proxyLabel} body=" . substr($body, 0, 200));
...
function ooredooRequest(
    ...
    // إذا كان sticky proxy مُحدَّد → نستخدمه فقط (مع retry على 429)
    if ($stickyProxy !== null) {
        $maxStickyRetry = 2;
        for ($sri = 0; $sri < $maxStickyRetry; $sri++) {
            $r = ooredooCurlRaw($url, $method, $payload, $headers, $logTag, $timeout, $stickyProxy);
            if ($r === null || ($r['ip_blocked'] ?? false)) {
                ooredooLog("[{$logTag}] sticky proxy failed/blocked sri={$sri}");
                if ($sri < $maxStickyRetry - 1) { usleep(500000); continue; }
                return null;
            }
    ...
    if (!$useProxy) {
        ...
        if ($r === null || ($r['ip_blocked'] ?? false)) {
            ooredooLog("[{$logTag}] direct request blocked/failed");
            return null;
        }
    ...
    // الترتيب: direct أولاً ثم البروكسيات (عشوائي)
    $proxies = getAllProxies();
    if (empty($proxies)) {
    ...
    shuffle($proxies);
    $candidates = array_merge([null], array_map('parseProxy', $proxies));

    foreach ($candidates as $pp) {
        $r = ooredooCurlRaw($url, $method, $payload, $headers, $logTag, $timeout, $pp);
        if ($r === null) {
            // errno/SSL error → انتقل لبروكسي آخر فوراً
            usleep(150000);
            continue;
        }
        if ($r['ip_blocked'] ?? false) {
            // 200+HTML أو 429 → انتقل لبروكسي آخر
            usleep(150000);
            continue;
        }
...
function ooredooSendOTP(string $msisdn): array|false
{
    ...
    foreach ($candidates12 as $proxy) {
        ...
        $r1    = ooredooCurlRaw($url1, 'GET', '', $hdrs1, 'OO_STATUS', 8, $proxy);
        if (!$r1 || ($r1['ip_blocked'] ?? false) || ($r1['http_code'] ?? 0) !== 200) {
            ooredooLog('[OTP] Step1 fail proxy=' . ($proxy['host'] ?? 'direct'));
            continue;
        }
        ...
        $r2    = ooredooCurlRaw($url2, 'HEAD', '', $hdrs2, 'OO_USERINFO', 8, $proxy);
        if (!$r2 || ($r2['ip_blocked'] ?? false) || ($r2['http_code'] ?? 0) !== 204) {
            ooredooLog('[OTP] Step2 fail proxy=' . ($proxy['host'] ?? 'direct') . ' code=' . ($r2['http_code'] ?? 'null'));
            continue;
        }
        $workingProxy = $proxy; // قد يكون null (direct) أو array
        ooredooLog('[OTP] Step1+2 OK proxy=' . ($proxy['host'] ?? 'direct'));
        break;
    }
    if ($workingProxy === null && !isset($r2)) {
        // حتى direct فشل — لا يوجد اتصال
        ooredooLog('[OTP] Step1+2 ALL failed');
        return false;
    }

    // Step 3+4: checkpoint/token — يحتاج TLS نظيف
    // قاعدة أساسية: nonce + chronos من Step3 يجب أن يُستخدَما في Step4
    // على نفس الـ IP (sticky per iteration) — لا تغيير بينهما
    $candidates34 = array_merge([null], array_map('parseProxy', $allProxies));
    foreach ($candidates34 as $chkProxy) {
        $proxyLabel34 = ooredooProxyLabel($chkProxy);
        // نولّد ts و corrId مرة واحدة لكل محاولة — يستخدمان في كلا الطلبين
        $ts     = (string)(time() * 1000);
        $corrId = bin2hex(random_bytes(8)) . $ts;

        // ── Step3: POST /checkpoint/token → الحصول على nonce+chronos ──
        $hdrs3 = ooredooBaseHeaders($msisdn, 'd717f779e8ab59712211d85641a7e66fdc77fc21722d5c0b88395202420a0d69', $corrId, $ts);
        ...
        if (!$r3 || ($r3['ip_blocked'] ?? false)) {
            ooredooLog("[OTP] Step3 fail proxy={$proxyLabel34} errno noted");
            continue;
        }
        $nonce1   = $r3['resp_headers']['x-nonce-id']   ?? '';
        $chronos1 = $r3['resp_headers']['x-chronos-id'] ?? '';
        if (!$nonce1 || !$chronos1) {
            ooredooLog("[OTP] Step3 no nonce/chronos http=" . ($r3['http_code'] ?? '?') . " proxy={$proxyLabel34}");
            continue;
        }
        ooredooLog("[OTP] Step3 OK nonce=" . substr($nonce1, 0, 12) . " proxy={$proxyLabel34}");

        // ── Step4: POST token بدون OTP — نفس الـ IP كـ Step3 (nonce/chronos مقيّدان بـ IP) ──
        $hdrs4 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: deb1e6b940af70681feb13e8695e7d098c1c408b862e13615de463ad0eeb956f',
            "x-nonce-id: {$nonce1}",          // من Step3 على نفس IP
            'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
            'platform: ' . OOREDOO_PLATFORM,
            'x-version: ' . OOREDOO_X_VERSION,
            'x-signature: ' . OOREDOO_X_SIG,  // ثابت — لا يتغير
            "x-chronos-id: {$chronos1}",       // من Step3 على نفس IP
            "x-timestamp: {$ts}",              // نفس timestamp
        ...
        // $chkProxy هو نفس proxy Step3 — ضروري لصحة nonce/chronos
        $r4 = ooredooCurlRaw(OOREDOO_AUTH, 'POST', $body4, $hdrs4, 'OO_TOKEN1', 10, $chkProxy);
        if (!$r4 || ($r4['ip_blocked'] ?? false)) {
            ooredooLog("[OTP] Step4 cURL fail proxy={$proxyLabel34}");
            continue; // نجرب IP آخر في الدورة التالية
        }
        $code4 = $r4['http_code'] ?? 0;
        ooredooLog("[OTP] Step4 code={$code4} proxy={$proxyLabel34}");

        // 403 أو 401 = SMS أُرسل بنجاح ✅
        if ($code4 === 403 || $code4 === 401) {
            // نعيد الـ proxy (أو null إذا كان direct) لحفظه كـ sticky للـ verify
            return ['proxy' => $chkProxy];
        }
        ooredooLog("[OTP] Step4 unexpected code={$code4} proxy={$proxyLabel34} trying next");
    }
 ...
 */
function ooredooVerifyOTP(string $msisdn, string $otp, ?array $sticky): array|string|false
{
    // $sticky: ما أعادته ooredooGetStickyProxy → قد يكون ['_direct'=>true] أو ['host'=>...]
    // نستخدم stickyToProxy() لتحويله إلى curl proxy أو null

    $useProxy = ooredooProxyEnabled();
    $all      = $useProxy ? getAllProxies() : [];
    shuffle($all);

    // بناء قائمة المرشحين:
    // 1) الـ sticky proxy أولاً (نفس IP الذي أرسل OTP)
    // 2) direct
    // 3) باقي البروكسيات
    $stickyParsed = stickyToProxy($sticky); // null = direct أو proxy array
    $stickyHost   = $stickyParsed['host'] ?? '_direct_';

    $candidates = [];
    if ($sticky !== null) {
        // نضيف الـ sticky أولاً
        $candidates[] = $stickyParsed; // null أو array
    }
    // direct (إذا لم يكن sticky بالفعل direct)
    if ($stickyHost !== '_direct_') {
        $candidates[] = null;
    }
    // باقي البروكسيات
    foreach ($all as $p) {
        $pp = parseProxy($p);
        if (($pp['host'] ?? '') !== $stickyHost) {
            $candidates[] = $pp;
        }
    }
    ...
    foreach ($candidates as $proxy) {
        $proxyLabel = ooredooProxyLabel($proxy);
        // ts و corrId جديدان لكل محاولة، لكن Step6→Step7 يشتركان بنفس ts
        $ts     = (string)(time() * 1000);
        $corrId = bin2hex(random_bytes(8)) . $ts;

        // ── Step6: POST /checkpoint/token → nonce2+chronos2 ──
        ...
        if (!$r6 || ($r6['ip_blocked'] ?? false)) {
            ooredooLog("[VERIFY] Step6 fail proxy={$proxyLabel}");
            continue;
        }
        $nonce2   = $r6['resp_headers']['x-nonce-id']   ?? '';
        $chronos2 = $r6['resp_headers']['x-chronos-id'] ?? '';
        if (!$nonce2 || !$chronos2) {
            ooredooLog("[VERIFY] Step6 no nonce/chronos http=" . ($r6['http_code'] ?? '?') . " proxy={$proxyLabel}");
            continue;
        }
        ooredooLog("[VERIFY] Step6 OK nonce=" . substr($nonce2, 0, 12) . " proxy={$proxyLabel}");

        // ── Step7: POST token مع OTP — نفس الـ proxy (نفس IP) ← إجباري للـ nonce/chronos ──
        $hdrs7 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: 8fbdad0928c41754cc7cbd9379db399f1fe06fb7dbd6275a7616dfecc19d7246',
            "x-nonce-id: {$nonce2}",           // من Step6 على نفس IP
            'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
            'platform: ' . OOREDOO_PLATFORM,
            'x-version: ' . OOREDOO_X_VERSION,
            'x-signature: ' . OOREDOO_X_SIG,   // ثابت دائماً
            "x-chronos-id: {$chronos2}",        // من Step6 على نفس IP
            "x-timestamp: {$ts}",               // نفس ts
            'x-platform-info: ' . OOREDOO_PLAT_SIG,
            'x-force-update: true',
            'x-instance-id: ' . OOREDOO_INSTANCE_ID,
        ];
        ...
        // نُمرّر $proxy — نفسه الذي أعطى nonce2/chronos2 في Step6
        $r7 = ooredooCurlRaw(OOREDOO_AUTH, 'POST', $body7, $hdrs7, 'OO_TOKEN2', 12, $proxy);
        if (!$r7 || ($r7['ip_blocked'] ?? false)) {
            ooredooLog("[VERIFY] Step7 cURL fail proxy={$proxyLabel}");
            continue;
        }
        $code7 = $r7['http_code'] ?? 0;
        ooredooLog("[VERIFY] Step7 code={$code7} proxy={$proxyLabel}");

        // OTP خاطئ — نوقف فوراً، تغيير الـ proxy لن يغير شيئاً
        if ($code7 === 401) return 'wrong_otp';
        if ($code7 === 200) {
            $json = $r7['json'] ?? [];
        ...
        ooredooLog("[VERIFY] Step7 unexpected code={$code7} proxy={$proxyLabel} trying next");
        // كود 403 = نجرب proxy آخر (قد يكون IP block وليس OTP خاطئ)
    }
...
// ════════════════════════════════════════════════════════════════════════════
function ooredooGamificationStatus(string $msisdn, string $accessToken): ?array
{
    $ts     = (string)(time() * 1000);
    $corrId = bin2hex(random_bytes(8)) . $ts;
    $hdrs   = [
        ...
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
        'x-device-fingerprint: fe0645ef29bfa2fb33fc06793ee5c28efe984e49882e21d16d49411af3a3ef45',
        'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
    ...
    $res = ooredooRequest(OOREDOO_BFF . '/gamification/status', 'GET', '', $hdrs, 'OO_GSTATUS');
    if (!$res) return null;
    return $res['result']['json'] ?? null;
}

function ooredooValidateUser(string $msisdn, string $accessToken): ?array
{
    $ts     = (string)(time() * 1000);
    $corrId = bin2hex(random_bytes(8)) . $ts;
    $hdrs   = [
        ...
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
        'x-device-fingerprint: 73c75708dd18150cdf9f9376e6bd541da66c791a5148cc39d5def731c4812365',
        'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
        "x-correlation-id: {$corrId}",
        'platform: ' . OOREDOO_PLATFORM,
    ...
    $res = ooredooRequest($url, 'GET', '', $hdrs, 'OO_VALIDATE');
    if (!$res) return null;
    return $res['result']['json'] ?? null;
}

function ooredooGamificationPlay(string $msisdn, string $accessToken): ?array
{
    $ts     = (string)(time() * 1000);
    $corrId = bin2hex(random_bytes(8)) . $ts;

    // checkpoint أولاً لـ nonce/chronos الـ play
    $url3   = OOREDOO_BFF . '/checkpoint/token';
    ...
    // checkpoint يحتاج TLS نظيف → direct أولاً ثم البروكسيات
    $useProxy    = ooredooProxyEnabled();
    $candidates  = [null]; // direct أولاً
    if ($useProxy) {
        $all = getAllProxies(); shuffle($all);
        foreach ($all as $p) $candidates[] = parseProxy($p);
    }

    $nonce = ''; $chronos = ''; $usedProxy = null;
    foreach ($candidates as $chkProxy) {
        $proxyLabelPlay = ooredooProxyLabel($chkProxy);
        $r3 = ooredooCurlRaw(OOREDOO_BFF . '/checkpoint/token', 'POST', '{}', $hdrs3, 'OO_GPLAY_CHK', 10, $chkProxy);
        if (!$r3 || ($r3['ip_blocked'] ?? false)) {
            ooredooLog("[PLAY] checkpoint fail proxy={$proxyLabelPlay}");
            continue;
        }
        $nonce   = $r3['resp_headers']['x-nonce-id']   ?? '';
        $chronos = $r3['resp_headers']['x-chronos-id'] ?? '';
        if ($nonce && $chronos) {
            $usedProxy = $chkProxy; // نحفظ نفس proxy لـ OO_GPLAY
            ooredooLog("[PLAY] checkpoint OK nonce=" . substr($nonce, 0, 12) . " proxy={$proxyLabelPlay}");
            break;
        }
        ooredooLog("[PLAY] checkpoint no nonce proxy={$proxyLabelPlay}");
    }
    if (!$nonce || !$chronos) { ooredooLog("[PLAY] all candidates failed for checkpoint"); return null; }
    ...
    $hdrsPlay = [
        ...
        "x-msisdn: {$msisdn}",
        "x-nonce-id: {$nonce}",
        'x-device-fingerprint: 6641adee63ba0f9d8e381e7634b4d6a6561db1eece55b775b6d1c63428deea28',
        'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
        "x-correlation-id: {$corrId}",
        'platform: ' . OOREDOO_PLATFORM,
    ...
    $resPlay = ooredooRequest(OOREDOO_BFF . '/gamification/play', 'GET', '', $hdrsPlay, 'OO_GPLAY', 15, $usedProxy);
    if (!$resPlay) return null;
    $code = $resPlay['result']['http_code'] ?? 0;
    // 500 = لعبت اليوم (server-side check من Ooredoo)
    if ($code === 500) return ['already_played' => true];
    if ($code === 200) return array_merge(['already_played' => false], ($resPlay['result']['json'] ?? []));
...
function handleNewOoredooPhone(string $psid, string $phone): void
{
    ...
    if ($owner === $psid) {
        ...
        if ($user && !empty($user['access_token']) && !empty($user['refresh_token'])) {
            ...
            $refreshed = ooredooRefreshToken($user['refresh_token'], $msisdn);
            if (is_array($refreshed)) {
                $user = array_merge($user, ['msisdn' => $msisdn, 'access_token' => $refreshed['access_token'], 'refresh_token' => $refreshed['refresh_token']]);
                saveUser($psid, $user);
    ...
    // $otpRes['proxy'] قد يكون null (direct) أو ['host'=>..., 'userpass'=>...]
    ooredooSetStickyProxy($psid, $otpRes['proxy'] ?? null);
    // الـ verifyOTP سينشئ checkpoint2 جديد على نفس الـ IP المخزّن
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    $sticky = ooredooGetStickyProxy($psid);
    if ($sticky === null) {
    ...
    // $sticky مضمون الآن: إما ['_direct'=>true] أو ['host'=>..., 'userpass'=>...]
    $result = ooredooVerifyOTP($msisdn, $m[1], $sticky);
    if ($result === 'wrong_otp') {
        sendMessage($psid, "❌ الرمز المُدخل خاطئ!\n\n🔄 أعد إرسال الرمز الصحيح\n📱 أو أرسل رقمك مجدداً\n\n❌ للإلغاء أرسل: 0");
        return;
    }
...
/**
 * ooredooSendPostLoginInfo — يرسل نوع الشريحة + حالة الهدية بعد تسجيل الدخول
 *
 * قاعدة صارمة: لا يُرسل الرسالة حتى ينجح كلا الطلبين:
 *   ① validateUser  → planType + contractType
 *   ② gamificationStatus → حالة الهدية
 * يُعيد المحاولة حتى MAX_RETRIES مرات قبل أن يستسلم.
 */
function ooredooSendPostLoginInfo(string $psid, array $user): void
{
    ...
    ooredooLog("[POST_LOGIN] start msisdn={$msisdn}");

    // ── محاولة جلب البيانات حتى MAX_RETRIES ──────────────────────────
    $maxRetries   = 3;
    $validateData = null;
    $giftData     = null;

    for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
        ooredooLog("[POST_LOGIN] attempt={$attempt}/{$maxRetries}");

        if ($validateData === null) {
            $vd = ooredooValidateUser($msisdn, $token);
            if ($vd !== null && isset($vd['planType'])) {
                $validateData = $vd;
                ooredooLog("[POST_LOGIN] OO_VALIDATE OK planType=" . ($vd['planType'] ?? '?'));
            } else {
                ooredooLog("[POST_LOGIN] OO_VALIDATE failed attempt={$attempt}");
            }
        }

        if ($giftData === null) {
            $gd = ooredooGamificationStatus($msisdn, $token);
            if ($gd !== null && array_key_exists('played', $gd)) {
                $giftData = $gd;
                ooredooLog("[POST_LOGIN] OO_GSTATUS OK played=" . ($gd['played'] ? 'true' : 'false'));
            } else {
                ooredooLog("[POST_LOGIN] OO_GSTATUS failed attempt={$attempt}");
            }
        }

        // كلاهما نجح → نكمل
        if ($validateData !== null && $giftData !== null) break;

        // لم يكتمل → ننتظر قبل المحاولة التالية
        if ($attempt < $maxRetries) usleep(1500000); // 1.5 ثانية
    }

    // ── إذا فشل أحد الطلبين بعد كل المحاولات → لا نرسل الرسالة ──────
    if ($validateData === null || $giftData === null) {
        ooredooLog("[POST_LOGIN] ABORTED validate=" . ($validateData ? 'OK' : 'NULL') . " gift=" . ($giftData ? 'OK' : 'NULL'));
        sendMessage($psid,
            "⚠️ تعذر جلب معلومات حسابك من سيرفر أوريدو.\n\n" .
            "🔄 اضغط على '1' أو أعد إرسال رقمك للمحاولة مجدداً.\n\n" .
            "⚡ قناة التلقرام: https://t.me/tasjilbott"
        );
        return;
    }

    // ── بناء الرسالة — نصل هنا فقط إذا نجح كلا الطلبين ──────────────
    ...
    $giftReady = !$played;
    if (!$giftReady && $lastPlayed) {
        $giftReady = (time() - strtotime($lastPlayed)) >= 86400;
    }
    ...
    ooredooLog("[POST_LOGIN] sending info message planType={$planType} giftReady=" . ($giftReady ? 'true' : 'false'));
...
function activateOoredooDailyGift(string $psid, array $user): void
{
    ...
    // فحص الحالة أولاً
    $giftData = ooredooGamificationStatus($msisdn, $token);
    if ($giftData !== null) {
        $played     = $giftData['played'] ?? false;
        $lastPlayed = $giftData['lastPlayedTime'] ?? null;
        if ($played && $lastPlayed) {
    ...
    $result = ooredooGamificationPlay($msisdn, $token);
    ...
    if ($result['already_played'] ?? false) {
        ...
        $giftData2 = ooredooGamificationStatus($msisdn, $token);
 ...
 */
function ooredooRefreshToken(string $refreshToken, string $msisdn): array|false
{
    $ts    = (string)(time() * 1000);
    $hdrs  = [
        'User-Agent: Dart/3.11 (dart:io)',
    ...
    $res = ooredooRequest(OOREDOO_AUTH, 'POST', $body, $hdrs, 'OO_REFRESH');
    if (!$res) return false;
    $code = $res['result']['http_code'] ?? 0;
