...
function ooredooCurl(string $url, string $method, $payload, array $headers, string $tag, int $timeout = 15): array
{
    ...
    $opts = [
        ...
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_MAXREDIRS      => 3,
        CURLOPT_HEADER         => true,   // نحتاج headers للـ nonce/chronos
    ];
    ...
    file_put_contents('/tmp/ooredoo.log',
        date('Y-m-d H:i:s') . " [$tag] CODE:{$httpCode} ERR:{$err} BODY:" . substr((string)$raw, $hdrSize, 400) . "\n",
        FILE_APPEND
    );

    if ($err || $raw === false) return ['code' => 0, 'headers' => '', 'body' => '', 'json' => null];

    $rawStr  = (string)$raw;
    $hdrStr  = substr($rawStr, 0, $hdrSize);
    $body    = substr($rawStr, $hdrSize);
...
function ooCheckpoint(string $msisdn, int $which): array
{
    ...
    // استخراج X-Nonce-Id و X-Chronos-Id من الـ headers
    foreach (explode("\r\n", $r['headers']) as $line) {
        if (stripos($line, 'x-nonce-id:') === 0)   $nonce   = trim(substr($line, 11));
        if (stripos($line, 'x-chronos-id:') === 0) $chronos = trim(substr($line, 13));
    }
...
function ooSendOTP(string $msisdn): array
{
    ...
    if ($status === null) return ['ok' => false, 'reason' => 'status_fail'];

    $code = $status['code'] ?? '';
    if ($code === '002') return ['ok' => false, 'reason' => 'not_found'];   // رقم غير موجود
    // 004 = موجود → نكمل

    // Step 2
    $uinfoCode = ooCheckUserInfo($msisdn);
    if ($uinfoCode !== 204) return ['ok' => false, 'reason' => 'userinfo_fail'];

    // Step 3 — checkpoint أول
    $chk1 = ooCheckpoint($msisdn, 1);
    if (empty($chk1['nonce'])) return ['ok' => false, 'reason' => 'chk1_fail'];

    // Step 4 — token بدون OTP → يُطلق SMS (403 متوقع)
    $r = ooTokenRequest($msisdn, $chk1['nonce'], $chk1['chronos'], '');
    if ($r['code'] !== 403 && $r['code'] !== 200) {
        return ['ok' => false, 'reason' => 'sms_trigger_fail'];
    }
...
function handleOoredooPhone(string $psid, string $phone): void
{
    ...
    if (!$result['ok']) {
        if ($result['reason'] === 'not_found') {
            ...
            sendMessage($psid, "⚠️ تعذر الاتصال بسيرفر أوريدو، حاول لاحقاً.");
        }
        return;
    }
