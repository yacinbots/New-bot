...
function ooredooSendOTP(string $msisdn): array|false
{
    ...
    foreach ($candidates34 as $chkProxy) {
        ...
        $hdrs4 = [
            ...
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: deb1e6b940af70681feb13e8695e7d098c1c408b862e13615de463ad0eeb956f',
            "x-nonce-id: {$nonce1}",
            'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
            'platform: ' . OOREDOO_PLATFORM,
            'x-version: ' . OOREDOO_X_VERSION,
...
function ooredooVerifyOTP(string $msisdn, string $otp, ?array $stickyProxy): array|string|false
{
    ...
    foreach ($candidates as $proxy) {
        ...
        $hdrs7 = [
            ...
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: 8fbdad0928c41754cc7cbd9379db399f1fe06fb7dbd6275a7616dfecc19d7246',
            "x-nonce-id: {$nonce2}",
            'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
            'platform: ' . OOREDOO_PLATFORM,
            'x-version: ' . OOREDOO_X_VERSION,
        ...
        if ($code7 === 200) {
            ...
            if (isset($json['access_token'])) {
                ...
                return [
                    ...
                    // قيم headers ثابتة مرتبطة بالتوكن
                    'tok_fingerprint'   => '8fbdad0928c41754cc7cbd9379db399f1fe06fb7dbd6275a7616dfecc19d7246',
                    'tok_timestamp'     => $ts,
...
{
    // نستخدم نفس قيم الـ headers التي ولّدت بها التوكن — Ooredoo يقيّدها
    $fingerprint = $tokHdrs['fingerprint'] ?? 'fe0645ef29bfa2fb33fc06793ee5c28efe984e49882e21d16d49411af3a3ef45';
    ...
    $corrId      = $tokHdrs['correlation'] ?? (bin2hex(random_bytes(8)) . $ts);
...
{
    $fingerprint = $tokHdrs['fingerprint'] ?? '73c75708dd18150cdf9f9376e6bd541da66c791a5148cc39d5def731c4812365';
    $ts          = $tokHdrs['timestamp']   ?? (string)(time() * 1000);
    $instanceId  = $tokHdrs['instance_id'] ?? OOREDOO_INSTANCE_ID;
    $platSig     = $tokHdrs['plat_sig']    ?? OOREDOO_PLAT_SIG;
    $corrId      = $tokHdrs['correlation'] ?? (bin2hex(random_bytes(8)) . $ts);
...
function ooredooGamificationPlay(string $msisdn, string $accessToken, ?array $tokenProxy = null, array $tokHdrs = []): ?array
{
    ...
    $hdrs3  = ooredooBaseHeaders($msisdn, '193eb3908167a39506cc7ed89ce063e48733712cc562ae020933d56b694d86ce', $corrId, $ts);
    $hdrs3[] = 'content-type: application/json; charset=utf-8';
    $hdrs3[] = 'x-native-integrity-token: ' . OOREDOO_INTEGRITY_1;
    ...
    // قيم headers يجب أن تكون نفس التي ولّدت التوكن
    $playFp      = $tokHdrs['fingerprint'] ?? '6641adee63ba0f9d8e381e7634b4d6a6561db1eece55b775b6d1c63428deea28';
    $playTs      = $tokHdrs['timestamp']   ?? $ts;
    $playInst    = $tokHdrs['instance_id'] ?? OOREDOO_INSTANCE_ID;
    $playPlatSig = $tokHdrs['plat_sig']    ?? OOREDOO_PLAT_SIG;
    $playCorr    = $tokHdrs['correlation'] ?? $corrId;
    ...
    $hdrsPlay = [
        ...
        "x-msisdn: {$msisdn}",
        "x-nonce-id: {$nonce}",
        "x-device-fingerprint: {$playFp}",
        'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
        "x-correlation-id: {$playCorr}",
        'platform: ' . OOREDOO_PLATFORM,
        'x-version: ' . OOREDOO_X_VERSION,
        'x-signature: ' . OOREDOO_X_SIG,
        "x-chronos-id: {$chronos}",
        "x-platform-data-signature: {$playPlatSig}",
        "x-timestamp: {$playTs}",
        "x-instance-id: {$playInst}",
    ];
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    $user = [
        ...
        'tok_fingerprint'  => $result['tok_fingerprint']  ?? '8fbdad0928c41754cc7cbd9379db399f1fe06fb7dbd6275a7616dfecc19d7246',
...
{
    return [
        'fingerprint'  => $user['tok_fingerprint'] ?? '8fbdad0928c41754cc7cbd9379db399f1fe06fb7dbd6275a7616dfecc19d7246',
        ...
        'correlation'  => $user['tok_correlation'] ?? (bin2hex(random_bytes(8)) . (string)(time() * 1000)),
    ];
}
