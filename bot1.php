...
function loadProxies(): array
{
    ...
    return [
    "http://gate.kookeey.info:1000:5616770-0b673a8a97:176c9d4e77-DZ-55366665-1m",
    "http://gate.kookeey.info:1000:5616770-0b673a8a97:176c9d4e77-DZ-46923254-1m",
    "http://gate.kookeey.info:1000:5616770-0b673a8a97:176c9d4e77-DZ-12270773-1m",
    "http://gate.kookeey.info:1000:5616770-0b673a8a97:176c9d4e77-DZ-51691689-1m",
    "http://gate.kookeey.info:1000:5616770-0b673a8a97:176c9d4e77-DZ-84607726-1m",
    "http://gate.kookeey.info:1000:5616770-0b673a8a97:176c9d4e77-DZ-51216483-1m",

    "http://mobile.kookeey.info:1086:5616770-0b673a8a97:176c9d4e77-DZ-44223266-1m",
    "http://mobile.kookeey.info:1086:5616770-0b673a8a97:176c9d4e77-DZ-63033714-1m",
    "http://mobile.kookeey.info:1086:5616770-0b673a8a97:176c9d4e77-DZ-17296543-1m",
    "http://mobile.kookeey.info:1086:5616770-0b673a8a97:176c9d4e77-DZ-11195078-1m",

    "http://gate.kookeey.info:1000:9333766-e79e23761c:20b657f46f-DZ-77920671-1m",
    "http://gate.kookeey.info:1000:9333766-e79e23761c:20b657f46f-DZ-80568218-1m",
    "http://gate.kookeey.info:1000:9333766-e79e23761c:20b657f46f-DZ-09116589-1m",
    "http://gate.kookeey.info:1000:9333766-e79e23761c:20b657f46f-DZ-66240507-1m",
    "http://gate.kookeey.info:1000:9333766-e79e23761c:20b657f46f-DZ-27887057-1m",
    "http://gate.kookeey.info:1000:9333766-e79e23761c:20b657f46f-DZ-46667283-1m",

    "http://mobile.kookeey.info:1086:9333766-e79e23761c:20b657f46f-DZ-71773416-1m",
    "http://mobile.kookeey.info:1086:9333766-e79e23761c:20b657f46f-DZ-60720064-1m",
    "http://mobile.kookeey.info:1086:9333766-e79e23761c:20b657f46f-DZ-22997035-1m",
    "http://mobile.kookeey.info:1086:9333766-e79e23761c:20b657f46f-DZ-14338232-1m",

    "http://gate.kookeey.info:1000:1227697-6798c9908d:384190b67b-DZ-46097014-1m",
    "http://gate.kookeey.info:1000:1227697-6798c9908d:384190b67b-DZ-78179914-1m",
    "http://gate.kookeey.info:1000:1227697-6798c9908d:384190b67b-DZ-51094537-1m",
    "http://gate.kookeey.info:1000:1227697-6798c9908d:384190b67b-DZ-74802599-1m",
    "http://gate.kookeey.info:1000:1227697-6798c9908d:384190b67b-DZ-02179916-1m",
    "http://gate.kookeey.info:1000:1227697-6798c9908d:384190b67b-DZ-63466068-1m",

    "http://mobile.kookeey.info:1086:1227697-6798c9908d:384190b67b-DZ-33488534-1m",
    "http://mobile.kookeey.info:1086:1227697-6798c9908d:384190b67b-DZ-17769405-1m",
    "http://mobile.kookeey.info:1086:1227697-6798c9908d:384190b67b-DZ-48143724-1m",
    "http://mobile.kookeey.info:1086:1227697-6798c9908d:384190b67b-DZ-66395377-1m"
];
}
...
// ════════════════════════════════════════════════════════════════════════════
// Ooredoo Proxy System — مصفوفة بروكسيات مخصصة لأوريدو قابلة للتحديث من تلقرام
// صيغة: http://user:pass@host:port
// ════════════════════════════════════════════════════════════════════════════
function loadOorProxies(): array
{
    if (file_exists(OOR_PROXY_LIST_FILE)) {
        $d = json_decode(file_get_contents(OOR_PROXY_LIST_FILE), true);
        if (is_array($d) && count($d) > 0) return $d;
    }
    // الافتراضيات — قم بتحديثها من تلقرام بـ /setoorproxies
    return [
...
{
    file_put_contents(OOR_PROXY_LIST_FILE, json_encode($proxies, JSON_PRETTY_PRINT));
}

/**
 * parseOorProxy — يحلل صيغة http://user:pass@host:port
 */
function parseOorProxy(string $proxy): array
{
    $parsed = parse_url($proxy);
    $host   = ($parsed['host'] ?? '') . ':' . ($parsed['port'] ?? '80');
    $user   = $parsed['user'] ?? '';
    $pass   = $parsed['pass'] ?? '';
    return ['host' => $host, 'userpass' => "{$user}:{$pass}"];
}

/**
 * isHtmlRejection — يكشف ردود HTML تعني رفض البروكسي أو WAF
 */
function isHtmlRejection(string $body): bool
{
    return stripos($body, '<html') !== false
        || stripos($body, '<!DOCTYPE') !== false
        || stripos($body, 'Request Rejected') !== false
        || stripos($body, '<title>') !== false;
}

/**
 * oorCurlWithProxy — يجرب بروكسيات أوريدو واحدة تلو الأخرى
 * عند الـ HTML rejection يعيد المحاولة ببروكسي آخر صامتاً
 * إذا فشلت الكل يعيد null (المستدعي يرسل رسالة الضغط)
 */
function oorCurlWithProxy(
    string $url,
    string $method,
    string $body,
    array  $headers,
    ...
    $proxies = loadOorProxies();
    $total   = count($proxies);

    if ($total === 0) {
        dbg("[{$logTag}] OOR: no proxies configured");
        return null;
    }
    ...
    foreach ($shuffled as $idx => $p) {
        ...
        $rawStr     = (string)$raw;
        $headerStr  = substr($rawStr, 0, $headerSize);
        $bodyStr    = substr($rawStr, $headerSize);

        file_put_contents('/tmp/oor_proxy.log',
            date('Y-m-d H:i:s') . " [{$logTag}] proxy[{$idx}/{$total}]={$pp['host']} http={$code} errno={$errno} err={$errMsg} body=" . substr($bodyStr, 0, 200) . "\n",
            FILE_APPEND
        );

        // بروكسي فاشل — خطأ شبكة
        if ($errno || $raw === false || $code === 0) {
            continue;
        }

        // HTML rejection — WAF أو بروكسي محجوب، جرب التالي
        if (isHtmlRejection($bodyStr)) {
            dbg("[{$logTag}] proxy[{$idx}] HTML rejection, rotating...");
            continue;
        }

        // ناجح — بناء resp_headers
        $respH = [];
        foreach (explode("\r\n", $headerStr) as $line) {
    ...
    // كل البروكسيات فشلت أو أعطت HTML
    dbg("[{$logTag}] ALL {$total} OOR proxies failed/rejected");
    return null;
}
...
function handleTelegramUpdate(array $update): void
{
    ...
    switch ($text) {
        ...
        case '/oorproxies':
            ...
            break;
        case '/oorlog':
            handleTgOorLog($chatId, 60);
            break;
        case '/oorlog100':
            handleTgOorLog($chatId, 100);
            break;
        case '/clearoorlog':
            file_put_contents('/tmp/oor_proxy.log', '');
            tgSendMessage($chatId, '🗑️ تم مسح سجل أوريدو.');
            break;
        case '/matchgift':
...
function sendTgMainMenu(string $chatId): void
{
    ...
    $keyboard = [
        ...
        [
            ['text' => '📡 بروكسيات Ooredoo',        'callback_data' => 'tg_oor_proxies'],
            ['text' => '➕ تحديث بروكسيات Ooredoo',  'callback_data' => 'tg_oor_setproxies'],
        ],
    ];
...
// ─── Ooredoo Proxy Management ─────────────────────────────────────────────────
function handleTgShowOorProxies(string $chatId): void
{
    $proxies = loadOorProxies();
    $count   = count($proxies);
    $lines   = ["📡 <b>بروكسيات Ooredoo ({$count})</b>\n"];
    foreach ($proxies as $i => $p) {
        $parsed  = parse_url($p);
        $host    = $parsed['host'] ?? $p;
        $port    = $parsed['port'] ?? '';
        $lines[] = ($i + 1) . ". <code>{$host}:{$port}</code>";
    }
    $lines[] = "\n/setoorproxies لتحديث القائمة";
...
function handleTgSetOorProxies(string $chatId): void
{
    ...
    tgSendMessage($chatId,
        "📝 <b>تحديث بروكسيات Ooredoo</b>\n\n"
        . "أرسل قائمة البروكسيات، كل بروكسي في سطر بالصيغة:\n"
        . "<code>http://user:pass@host:port</code>\n\n"
        . "مثال:\n"
        . "<code>http://nhtbnigs:3tdfei3ngchg@31.59.20.176:6754</code>\n\n"
        . "⚠️ ستُستبدل القائمة الحالية بالقائمة الجديدة.\n"
        . "/cancel للإلغاء"
    );
}
...
function handleTgOorProxiesInput(string $chatId, string $text): void
{
    ...
    foreach ($list as $item) {
        $item = trim($item);
        // صيغة: http://user:pass@host:port
        if (preg_match('#^https?://[^:]+:[^@]+@[^:]+:\d+$#', $item)) {
            $valid[] = $item;
        }
    }

    if (empty($valid)) {
        tgSendMessage($chatId, "❌ لا توجد بروكسيات بصيغة صحيحة.\n\nالصيغة الصحيحة:\n<code>http://user:pass@host:port</code>");
        return;
    }

    saveOorProxies($valid);
    clearTgState($chatId);
    tgSendMessage($chatId, "✅ تم حفظ <b>" . count($valid) . "</b> بروكسي لـ Ooredoo بنجاح!\n\n" . implode("\n", array_map(fn($p) => "• <code>" . (parse_url($p, PHP_URL_HOST)) . ":" . (parse_url($p, PHP_URL_PORT)) . "</code>", $valid)));
    sendTgMainMenu($chatId);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // ── Ooredoo Debug Dashboard ───────────────────────────────────────────────
    if (isset($_GET['oor_debug']) && $_GET['oor_debug'] === 'tasjil2024') {
        header('Content-Type: text/html; charset=utf-8');

        $logFile  = '/tmp/oor_proxy.log';
        $lines    = file_exists($logFile) ? file($logFile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : [];
        $last     = array_slice($lines, -200);           // آخر 200 سطر
        $proxies  = loadOorProxies();

        // ── تحليل إحصائي ─────────────────────────────────────────────────────
        $stats       = [];   // proxy_host => [total, success, html, network, nonce_empty]
        $tagCounts   = [];   // tag => count
        $htmlHosts   = [];
        $netErrHosts = [];

        foreach ($last as $line) {
            // صيغة: 2025-01-01 12:00:00 [TAG] METHOD URL proxy=HOST http=CODE errno=N err=MSG body=...
            preg_match('/\[(\w+)\].*proxy=([\w.:]+).*http=(\d+).*errno=(\d+).*err=([^\s]*).*body=(.*)/', $line, $m);
            if (!$m) continue;
            [, $tag, $host, $code, $errno, $err, $body] = $m;

            $tagCounts[$tag] = ($tagCounts[$tag] ?? 0) + 1;
            if (!isset($stats[$host])) $stats[$host] = ['total'=>0,'success'=>0,'html'=>0,'network'=>0,'nonce_empty'=>0];
            $stats[$host]['total']++;

            $isHtml    = stripos($body, 'html') !== false || stripos($body, 'Rejected') !== false;
            $isNetErr  = ($errno > 0 || $code == 0);
            $isSuccess = (!$isHtml && !$isNetErr && $code >= 200 && $code < 500);

            if ($isHtml)    { $stats[$host]['html']++;    $htmlHosts[$host] = true; }
            elseif ($isNetErr) { $stats[$host]['network']++; $netErrHosts[$host] = true; }
            elseif ($isSuccess) $stats[$host]['success']++;
        }

        // كشف البروكسيات بنفس IP (subnet /24)
        $ipGroups = [];
        foreach ($proxies as $p) {
            $host = parse_url($p, PHP_URL_HOST);
            $parts = explode('.', $host);
            $subnet = implode('.', array_slice($parts, 0, 3));
            $ipGroups[$subnet][] = $host;
        }
        $duplicateSubnets = array_filter($ipGroups, fn($g) => count($g) > 1);
        ...
        .ok{color:#3fb950}.fail{color:#f85149}.warn{color:#d29922}.dim{color:#6e7681}
        .badge{display:inline-block;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:bold}
        .badge-ok{background:#1a4730;color:#3fb950}
        .badge-fail{background:#4b1113;color:#f85149}
        .badge-warn{background:#3d2c00;color:#d29922}
        pre{background:#161b22;padding:12px;border-radius:6px;overflow-x:auto;font-size:12px;line-height:1.5;border:1px solid #30363d;max-height:600px;overflow-y:auto}
        .section{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:16px;margin:16px 0}
        .alert{background:#4b1113;border:1px solid #f85149;border-radius:6px;padding:12px;margin:8px 0;color:#f85149}
        .info{background:#1a3a4a;border:1px solid #58a6ff;border-radius:6px;padding:12px;margin:8px 0;color:#79c0ff}
        ';

        echo "<!DOCTYPE html><html><head><meta charset='utf-8'><title>Ooredoo Debug</title><style>{$css}</style></head><body>";
        echo "<h1>🔍 Ooredoo Proxy Debug Dashboard</h1>";
        echo "<p class='dim'>السجل: <code>{$logFile}</code> | آخر تحديث: " . date('Y-m-d H:i:s') . " | إجمالي السطور: " . count($lines) . " | تحليل آخر " . count($last) . "</p>";

        // ── تنبيهات فورية ────────────────────────────────────────────────────
        echo "<div class='section'><h2>⚡ تنبيهات فورية</h2>";
        if (!empty($duplicateSubnets)) {
            foreach ($duplicateSubnets as $subnet => $hosts) {
                echo "<div class='alert'>⚠️ بروكسيات بنفس subnet /24 ({$subnet}.x): <strong>" . implode(', ', $hosts) . "</strong> — نفس IP الحقيقي محتمل</div>";
            }
        } else {
            echo "<div class='info'>✅ لا توجد بروكسيات بنفس subnet /24</div>";
        }
        if (!empty($htmlHosts)) {
            echo "<div class='alert'>🚫 HTML Rejection على: <strong>" . implode(', ', array_keys($htmlHosts)) . "</strong></div>";
        }
        if (!empty($netErrHosts)) {
            echo "<div class='alert'>📡 Network Error على: <strong>" . implode(', ', array_keys($netErrHosts)) . "</strong></div>";
        }

        // كشف nonce_empty من السجل
        $nonceEmpty = 0;
        foreach ($last as $ln) { if (str_contains($ln, 'nonce-id') || str_contains($ln, 'OOR_CHK')) { if (str_contains($ln, 'nonce_empty') || (str_contains($ln, 'http=200') && !str_contains($ln, 'x-nonce'))) $nonceEmpty++; } }
        echo "</div>";

        // ── إحصاء بالـ Tag ────────────────────────────────────────────────────
        echo "<div class='section'><h2>📊 الطلبات حسب النوع</h2><table>";
        echo "<tr><th>Tag</th><th>عدد الطلبات</th></tr>";
        arsort($tagCounts);
        foreach ($tagCounts as $tag => $cnt) {
            $color = str_contains($tag, 'CHK') ? 'warn' : (str_contains($tag, 'SMS') || str_contains($tag, 'VERIFY') ? 'ok' : 'dim');
            echo "<tr><td class='{$color}'>{$tag}</td><td>{$cnt}</td></tr>";
        }
        echo "</table></div>";

        // ── إحصاء البروكسيات ─────────────────────────────────────────────────
        echo "<div class='section'><h2>🌐 أداء كل بروكسي</h2><table>";
        echo "<tr><th>البروكسي</th><th>الكل</th><th>نجاح</th><th>HTML</th><th>Network</th><th>نسبة النجاح</th><th>الحالة</th></tr>";
        foreach ($stats as $host => $s) {
            $rate  = $s['total'] > 0 ? round($s['success'] / $s['total'] * 100) : 0;
            $color = $rate >= 70 ? 'ok' : ($rate >= 30 ? 'warn' : 'fail');
            $badge = $rate >= 70 ? 'badge-ok' : ($rate >= 30 ? 'badge-warn' : 'badge-fail');
            $status = $s['html'] > $s['success'] ? '🚫 HTML Rejection' : ($s['network'] > $s['success'] ? '📡 Network Error' : '✅ OK');
            echo "<tr><td><code>{$host}</code></td><td>{$s['total']}</td><td class='ok'>{$s['success']}</td><td class='fail'>{$s['html']}</td><td class='warn'>{$s['network']}</td><td class='{$color}'>{$rate}%</td><td>{$status}</td></tr>";
        }
        echo "</table></div>";

        // ── قائمة البروكسيات الحالية ─────────────────────────────────────────
        echo "<div class='section'><h2>📡 البروكسيات المحفوظة (" . count($proxies) . ")</h2><table>";
        echo "<tr><th>#</th><th>Host</th><th>Port</th><th>User</th><th>Subnet /24</th></tr>";
        foreach ($proxies as $i => $p) {
            $ph = parse_url($p, PHP_URL_HOST);
            $pp = parse_url($p, PHP_URL_PORT);
            $pu = parse_url($p, PHP_URL_USER);
            $subnet = implode('.', array_slice(explode('.', $ph), 0, 3));
            $dup = isset($duplicateSubnets[$subnet]) ? "<span class='badge badge-warn'>dup subnet</span>" : '';
            echo "<tr><td class='dim'>" . ($i+1) . "</td><td><code>{$ph}</code></td><td>{$pp}</td><td>{$pu}</td><td>{$subnet} {$dup}</td></tr>";
        }
        echo "</table></div>";

        // ── آخر 60 سطر من السجل ─────────────────────────────────────────────
        $recentLines = array_slice($lines, -60);
        echo "<div class='section'><h2>📋 آخر 60 سطر من السجل</h2>";
        echo "<pre>";
        foreach ($recentLines as $ln) {
            $colored = htmlspecialchars($ln);
            if (str_contains($ln, 'html') || str_contains($ln, 'Rejected'))  $colored = "<span class='fail'>{$colored}</span>";
            elseif (str_contains($ln, 'errno=0') && str_contains($ln, 'http=200')) $colored = "<span class='ok'>{$colored}</span>";
            elseif (str_contains($ln, 'errno=') && !str_contains($ln, 'errno=0'))  $colored = "<span class='warn'>{$colored}</span>";
            echo $colored . "\n";
        }
        echo "</pre>";
        echo "<form method='GET'><input type='hidden' name='oor_debug' value='tasjil2024'><input type='hidden' name='clear_log' value='1'><button type='submit' style='background:#4b1113;color:#f85149;border:1px solid #f85149;padding:8px 16px;border-radius:4px;cursor:pointer'>🗑️ مسح السجل</button></form>";
        echo "</div>";
        ...
        if (isset($_GET['clear_log'])) {
            file_put_contents($logFile, '');
            echo "<script>location.href=location.href.split('&clear_log')[0]</script>";
        }
        exit;
    }
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — HTTP Client (Proxy-aware)
// ════════════════════════════════════════════════════════════════════════════

/**
 * oorCurlOnProxy — executes one HTTP request through a specific proxy.
 * Returns structured response or null on network/HTML failure.
 * Used internally; callers that need sticky-proxy (nonce) call this directly.
 */
function oorCurlOnProxy(string $url, string $method, string $body, array $headers, string $proxyHost, string $proxyAuth, string $logTag = 'OOR', int $timeout = 15): ?array
{
    $ch   = curl_init($url);
    $opts = [
        CURLOPT_RETURNTRANSFER => true,
    ...
    file_put_contents('/tmp/oor_proxy.log',
        date('Y-m-d H:i:s') . " [{$logTag}] {$method} {$url} proxy={$proxyHost} http={$code} errno={$errno} err={$errMsg} body=" . substr($bodyStr, 0, 200) . "\n",
        FILE_APPEND
    );

    if ($errno || $raw === false || $code === 0) return null;
    if (isHtmlRejection($bodyStr))              return null; // HTML = WAF rejection
    ...
    $json = null;
    if (!empty($bodyStr)) { $d = @json_decode($bodyStr, true); if (is_array($d)) $json = $d; }
    return ['http_code' => $code, 'body' => $bodyStr, 'json' => $json, 'resp_headers' => $respH];
}

/**
 * oorCurl — kept for backward-compat with non-auth API calls (no nonce dependency).
 * Tries all proxies in random order; returns last-resort null on full failure.
 */
function oorCurl(string $url, string $method, string $body, array $headers, string $logTag = 'OOR', int $timeout = 15): array
{
    $r = oorCurlWithProxy($url, $method, $body, $headers, $logTag, $timeout);
    // oorCurl legacy callers expect an array always — return safe default on null
    return $r ?? ['http_code' => 0, 'body' => '', 'json' => null, 'resp_headers' => []];
}
...
function oorCheckStatus(string $msisdn): ?string
{
    ...
    $r       = oorCurlWithProxy($url, 'GET', '', $headers, 'OOR_STATUS');
    if ($r === null || $r['http_code'] !== 200 || !is_array($r['json'])) return null;
    return $r['json']['code'] ?? null;
}

/**
 * oorCheckpointOnProxy — يجيب nonce/chronos ويُعيد نفس البروكسي الذي نجح.
 * CRITICAL: الـ nonce مربوط بـ IP — كل الطلبات التالية يجب أن تمر بنفس البروكسي.
 *
 * Returns: ['nonce'=>'...', 'chronos'=>'...', 'proxy_host'=>'...', 'proxy_auth'=>'...']
 * أو null إن فشلت كل البروكسيات.
 */
function oorCheckpointOnProxy(string $msisdn, int $type): ?array
{
    ...
    foreach ($shuffled as $p) {
        $pp = parseOorProxy($p);
        $r  = oorCurlOnProxy($url, 'POST', '{}', $h, $pp['host'], $pp['userpass'], 'OOR_CHK' . $type);
        if ($r === null) continue;

        $nonce   = $r['resp_headers']['x-nonce-id']   ?? '';
        $chronos = $r['resp_headers']['x-chronos-id'] ?? '';

        if (empty($nonce)) continue; // بروكسي أجاب لكن بدون nonce — جرب التالي

        // نجح — نُعيد نفس البروكسي مع الـ nonce
        return [
            'nonce'       => $nonce,
            'chronos'     => $chronos,
            'proxy_host'  => $pp['host'],
            'proxy_auth'  => $pp['userpass'],
        ];
    }

    dbg("[OOR_CHK{$type}] All proxies failed to get nonce");
    return null;
}

/**
 * oorSendSMS — checkpoint + SMS على نفس البروكسي (sticky proxy).
 */
function oorSendSMS(string $msisdn): bool
{
    $chk = oorCheckpointOnProxy($msisdn, 1);
    ...
    $h = [
        ...
        "x-nonce-id: {$chk['nonce']}", 'x-platform-origin: '.OOR_X_ORIGIN,
        'platform: '.OOR_PLATFORM, 'x-version: '.OOR_X_VER, 'x-signature: '.OOR_X_SIG,
        "x-chronos-id: {$chk['chronos']}", 'x-timestamp: '.OOR_TOKEN1_TS,
    ...
    // SAME proxy as checkpoint — nonce is IP-bound
    $r = oorCurlOnProxy(OOR_AUTH_URL, 'POST', $payload, $h, $chk['proxy_host'], $chk['proxy_auth'], 'OOR_SMS');
    if ($r === null) return false;
    return $r['http_code'] === 403;
}

/**
 * oorVerifyOTP — checkpoint + verify sur le même proxy.
 */
function oorVerifyOTP(string $msisdn, string $otp): mixed
{
    $chk = oorCheckpointOnProxy($msisdn, 2);
    if ($chk === null) return false;
    ...
    $h = [
        ...
        "x-nonce-id: {$chk['nonce']}", 'x-platform-origin: '.OOR_X_ORIGIN,
        'platform: '.OOR_PLATFORM, 'x-version: '.OOR_X_VER, 'x-signature: '.OOR_X_SIG,
        "x-chronos-id: {$chk['chronos']}", 'x-timestamp: '.OOR_TOKEN2_TS,
        'x-platform-info: '.OOR_PLAT_SIG, 'x-force-update: true',
        'x-instance-id: '.OOR_INSTANCE_ID, 'Content-Type: application/x-www-form-urlencoded',
    ];
    // SAME proxy as checkpoint
    $r = oorCurlOnProxy(OOR_AUTH_URL, 'POST', $payload, $h, $chk['proxy_host'], $chk['proxy_auth'], 'OOR_VERIFY');
    if ($r === null) return false;
...
/**
 * oorRefreshToken — refresh لا يحتاج nonce، يجرب كل البروكسيات.
 */
function oorRefreshToken(string $msisdn, string $refreshToken): mixed
{
    ...
    $r = oorCurlWithProxy(OOR_AUTH_URL, 'POST', $payload, $h, 'OOR_REFRESH');
    if ($r !== null && $r['http_code'] === 200 && isset($r['json']['access_token']))
        return ['access_token'=>$r['json']['access_token'],'refresh_token'=>$r['json']['refresh_token']??$refreshToken];
    return 'expired';
}

/**
 * Refresh tokens, or re-send OTP if refresh fails. Returns new tokens array or 'expired'.
 */
...
{
    // Checkpoint + play MUST use the same proxy (nonce is IP-bound)
    ...
    foreach ($shuffled as $p) {
        $pp   = parseOorProxy($p);
        $chkR = oorCurlOnProxy($chkUrl, 'POST', '{}', $chkH, $pp['host'], $pp['userpass'], 'OOR_GCHK');
        if ($chkR === null) continue;

        $nonce   = $chkR['resp_headers']['x-nonce-id']   ?? '';
        $chronos = $chkR['resp_headers']['x-chronos-id'] ?? '';
        if (empty($nonce)) continue;

        // Same proxy for /gamification/play
        $url  = OOR_BFF . '/gamification/play';
        $h    = array_merge(oorAuthHeaders($msisdn, $at), ['accept: */*']);
        $h[]  = "x-nonce-id: {$nonce}";
        $h[]  = "x-chronos-id: {$chronos}";
        $r    = oorCurlOnProxy($url, 'GET', '', $h, $pp['host'], $pp['userpass'], 'OOR_GPLAY');
        if ($r !== null) return $r;
    }
...
{
    $msisdn = '213' . substr($phone, 1); // 0562679469 → 213562679469
    ...
    if ($statusCode !== '004') {
        sendMessage($psid, "⚠️ هناك ضغط على أوريدو حالياً، يرجى المحاولة لاحقاً.");
        return;
    }
...
function handleOorAwaitingOtp(string $psid, string $text, array $session): void
{
    ...
    // User sends a new 05 number → resend OTP to that number
    ...
    if ($result === false) {
        sendMessage($psid,
            "⚠️ هناك ضغط على أوريدو حالياً، يرجى المحاولة لاحقاً.\n\n❌ للإلغاء أرسل: 0"
        );
        return;
    }
...
function handleOorMenuText(string $psid, string $text, array $session): void
{
    ...
    // New 05 number in menu → change account
    $digits = preg_replace('/\D/', '', $text);
    if (preg_match('/^05\d{8}$/', $digits))     { handleOorNewPhone($psid, $digits); return; }
...
function handleOorInfo(string $psid, string $msisdn, string $at, string $planType, string $rt): void
{
    ...
    if ($data === null) {
        ...
        if ($data === null) { sendMessage($psid, "⚠️ هناك ضغط على أوريدو حالياً، يرجى المحاولة لاحقاً."); return; }
    }
