...
// OOREDOO_DEVICE_ID / INSTANCE_ID / PLAT_SIG → ديناميكية — see ooredooGetSessionIdentity()
...
// ════════ Ooredoo — FP/CORR/timestamps تُولَّد ديناميكياً ════════
// جميع القيم الثابتة السابقة (FP_AUTHED, FP_TOKEN1..2, FP_GPLAY_CHK, CORR_AUTHED,
// timestamps ثابتة) حُذفت — تُولَّد الآن بـ ooredooRandHex() / ooredooCorrId() / ooredooNowMs()
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — Dynamic Identity Generators
// ════════════════════════════════════════════════════════════════════════════

function ooredooNowMs(): string { return (string)(int)(microtime(true) * 1000); }

function ooredooRandHex(): string { return bin2hex(random_bytes(32)); }

function ooredooCorrId(): string
{
    return sprintf('%08x-%04x-1%03x-%04x-%012x',
...
function ooredooPlatformSig(string $instanceId): string
{
...
function ooredooGetSessionIdentity(string $psid): array
{
    $sess = ooredooGetSession($psid);
    if (!empty($sess['instance_id'])) {
        return [
            'instance_id'  => $sess['instance_id'],
            'platform_sig' => $sess['platform_sig'] ?? ooredooPlatformSig($sess['instance_id']),
        ];
    }
    $user = function_exists('getUser') ? getUser($psid) : null;
    if (!empty($user['tok_instance_id'])) {
        return [
            'instance_id'  => $user['tok_instance_id'],
            'platform_sig' => $user['tok_plat_sig'] ?? ooredooPlatformSig($user['tok_instance_id']),
        ];
    }
    $iid = ooredooNewInstanceId();
    return ['instance_id' => $iid, 'platform_sig' => ooredooPlatformSig($iid)];
}
...
// ════════════════════════════════════════════════════════════════════════════
function ooredooBaseHeaders(string $msisdn, string $psid, string $fp = '', string $corrId = '', string $ts = ''): array
{
    $id   = ooredooGetSessionIdentity($psid);
    $fp   = $fp     !== '' ? $fp     : ooredooRandHex();
    $corrId = $corrId !== '' ? $corrId : ooredooCorrId();
    $ts   = $ts     !== '' ? $ts     : ooredooNowMs();
    return [
        ...
        'x-platform-data-signature: '    . $id['platform_sig'],
        "x-timestamp: {$ts}",
        'x-instance-id: '                . $id['instance_id'],
    ];
}
 ...
 */
function ooredooSendOTP(string $msisdn, string $psid = ''): array|false
{
    $useProxy   = ooredooProxyEnabled();
    $allProxies = $useProxy ? getAllProxies() : [];
    if (!empty($allProxies)) shuffle($allProxies);

    // هوية الجلسة — تُولَّد مرة واحدة وتُحفظ مع البروكسي
    $identity = ooredooGetSessionIdentity($psid);
    $iid      = $identity['instance_id'];
    $platSig  = $identity['platform_sig'];

    // ── Step1+2: STATUS+USERINFO ──
    $candidates12 = array_merge([null], array_map('parseProxy', $allProxies));
    $step12done   = false;
    foreach ($candidates12 as $proxy12) {
        $hdrs1 = ooredooBaseHeaders($msisdn, $psid);
        $r1    = ooredooCurlRaw(OOREDOO_BFF . '/users/status?msisdn=' . urlencode($msisdn), 'GET', '', $hdrs1, 'OO_STATUS', 8, $proxy12);
        if (!$r1 || ($r1['ip_blocked'] ?? false) || ($r1['http_code'] ?? 0) !== 200) {
            ooredooLog('[OTP] Step1 fail proxy=' . ($proxy12['host'] ?? 'direct')); continue;
        }
        $hdrs2 = ooredooBaseHeaders($msisdn, $psid);
        $r2    = ooredooCurlRaw(OOREDOO_BFF . '/userInfo?msisdn=' . urlencode($msisdn), 'HEAD', '', $hdrs2, 'OO_USERINFO', 8, $proxy12);
        if (!$r2 || ($r2['ip_blocked'] ?? false) || ($r2['http_code'] ?? 0) !== 204) {
            ooredooLog('[OTP] Step2 fail proxy=' . ($proxy12['host'] ?? 'direct') . ' code=' . ($r2['http_code'] ?? 'null')); continue;
        }
        ooredooLog('[OTP] Step1+2 OK proxy=' . ($proxy12['host'] ?? 'direct'));
        $step12done = true; break;
    }
    if (!$step12done) { ooredooLog('[OTP] Step1+2 ALL failed'); return false; }

    // ── Step3+4: checkpoint+token ──
    // checkpoint و token يجب أن يكونا من نفس الـ IP (nonce/chronos مرتبطان به)
    // direct مضاف كـ fallback إذا كانت كل البروكسيات محجوبة (errno=56/402)
    $ooredooProxies = getOoredooProxies();
    shuffle($ooredooProxies);
    $candidates34 = array_map('parseOoredooProxy', $ooredooProxies);
    $candidates34[] = null; // direct — fallback أخير

    foreach ($candidates34 as $chkProxy) {
        $ts     = ooredooNowMs();
        $corrId = ooredooCorrId();

        // ── 3: POST /checkpoint/token ──
        $hdrs3 = ooredooBaseHeaders($msisdn, $psid, '', $corrId, $ts);
        ...
        if (!$r3 || ($r3['ip_blocked'] ?? false)) {
            ooredooLog('[OTP] Step3 fail proxy=' . ($chkProxy['host'] ?? 'direct')); continue;
        }
        $nonce1   = $r3['resp_headers']['x-nonce-id']   ?? '';
        $chronos1 = $r3['resp_headers']['x-chronos-id'] ?? '';
        if (!$nonce1 || !$chronos1) {
            ooredooLog('[OTP] Step3 no nonce/chronos http=' . ($r3['http_code'] ?? '?') . ' proxy=' . ($chkProxy['host'] ?? 'direct')); continue;
        }
        ...
        $hdrs4 = [
            ...
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: ' . ooredooRandHex(),
            "x-nonce-id: {$nonce1}",
            'x-platform-origin: mobile-android',
            'platform: android',
            'x-version: '   . OOREDOO_X_VERSION,
 ...
 */
function ooredooVerifyOTP(string $msisdn, string $otp, ?array $stickyProxy, string $psid = ''): array|string|false
{
    $identity = ooredooGetSessionIdentity($psid);
    $iid      = $identity['instance_id'];
    $platSig  = $identity['platform_sig'];
    ...
    foreach (array_map('parseOoredooProxy', $ooredooProxies) as $pp) {
        if ($stickyProxy === null || ($pp['host'] ?? '') !== ($stickyProxy['host'] ?? '')) $candidates[] = $pp;
    }
    $candidates[] = null; // direct fallback
    ...
    foreach ($candidates as $proxy) {
        ...
        $hdrs6 = ooredooBaseHeaders($msisdn, $psid, '', $corrId, $ts);
        ...
        if (!$r6 || ($r6['ip_blocked'] ?? false)) {
            ooredooLog("[VERIFY] Step6 fail proxy=" . ($proxy['host'] ?? 'direct')); continue;
        }
        $nonce2   = $r6['resp_headers']['x-nonce-id']   ?? '';
        $chronos2 = $r6['resp_headers']['x-chronos-id'] ?? '';
        if (!$nonce2 || !$chronos2) {
            ooredooLog("[VERIFY] Step6 no nonce/chronos http=" . ($r6['http_code'] ?? '?')); continue;
        }

        // ── 7: POST token مع OTP — نفس الـ proxy ──
        $hdrs7 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
        ...
        $body7 = http_build_query(['client_id' => OOREDOO_CLIENT_ID, 'grant_type' => 'password', 'username' => $msisdn, 'otp' => $otp]);
        $r7 = ooredooCurlRaw(OOREDOO_AUTH, 'POST', $body7, $hdrs7, 'OO_TOKEN2', 12, $proxy);
        if (!$r7 || ($r7['ip_blocked'] ?? false)) {
            ooredooLog("[VERIFY] Step7 fail proxy=" . ($proxy['host'] ?? 'direct')); continue;
        }
        ...
        if ($code7 === 200) {
            ...
            if (isset($json['access_token'])) {
                return [
                    ...
                    'tok_fingerprint' => ooredooRandHex(),
                    'tok_timestamp'   => $ts,
                    'tok_instance_id' => $iid,
                    'tok_plat_sig'    => $platSig,
...
// ════════════════════════════════════════════════════════════════════════════
function ooredooGamificationStatus(string $msisdn, string $accessToken, ?array $proxy = null, array $tokHdrs = [], string $psid = ''): ?array
{
    // الاكتشاف الحاسم: platform=web + origin=my.ooredoo.dz → يتجاوز Play Integrity check → 200
    // platform=android → 401 دائماً (BFF يتطلب Play Integrity session)
    $identity = ooredooGetSessionIdentity($psid);
    $iid      = $tokHdrs['tok_instance_id'] ?? $identity['instance_id'];
    $platSig  = $tokHdrs['tok_plat_sig']    ?? $identity['platform_sig'];
    $ts       = ooredooNowMs();
    ...
    $hdrs = [
        ...
        "authorization: Bearer {$accessToken}",
        "x-correlation-id: "              . ooredooCorrId(),
        'platform: web',
        'x-version: '                     . OOREDOO_X_VERSION,
        'x-signature: '                   . OOREDOO_X_SIG,
    ...
    if (!$r) return null;
    if ($r['ip_blocked'] ?? false) { ooredooLog("[GSTATUS] IP_BLOCKED proxy=" . ($proxy['host'] ?? 'direct')); return null; }
    $code = $r['http_code'] ?? 0;
    if ($code !== 200) { ooredooLog("[GSTATUS] http={$code} proxy=" . ($proxy['host'] ?? 'direct')); return null; }
    return $r['json'] ?? null;
}

function ooredooValidateUser(string $msisdn, string $accessToken, ?array $proxy = null, array $tokHdrs = [], string $psid = ''): ?array
{
    $identity = ooredooGetSessionIdentity($psid);
    $iid      = $tokHdrs['tok_instance_id'] ?? $identity['instance_id'];
    $platSig  = $tokHdrs['tok_plat_sig']    ?? $identity['platform_sig'];
    ...
    if (!$r) return null;
    if ($r['ip_blocked'] ?? false) { ooredooLog("[VALIDATE] IP_BLOCKED proxy=" . ($proxy['host'] ?? 'direct')); return null; }
    $code = $r['http_code'] ?? 0;
    if ($code === 401) { ooredooLog("[VALIDATE] 401 proxy=" . ($proxy['host'] ?? 'direct')); return null; }
    return $r['json'] ?? null;
}

function ooredooGamificationPlay(string $msisdn, string $accessToken, ?array $tokenProxy = null, array $tokHdrs = [], string $psid = ''): ?array
{
    $identity = ooredooGetSessionIdentity($psid);
    $iid      = $tokHdrs['tok_instance_id'] ?? $identity['instance_id'];
    $platSig  = $tokHdrs['tok_plat_sig']    ?? $identity['platform_sig'];
...
function handleNewOoredooPhone(string $psid, string $phone): void
{
    ...
    $otpRes = ooredooSendOTP($msisdn, $psid);
    if ($otpRes === false) {
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    if (preg_match('/^05\d{8}$/', $digitsNew)) {
        ...
        $otpRes = ooredooSendOTP($newMsisdn, $psid);
        if ($otpRes === false) {
    ...
    $result = ooredooVerifyOTP($msisdn, $m[1], $stickyProxy, $psid);
    if ($result === 'wrong_otp') {
        sendMessage($psid, "❌ الرمز المُدخل خاطئ!\n\n🔄 أعد إرسال الرمز الصحيح\n📱 أو أرسل رقمك مجدداً\n\n❌ للإلغاء أرسل: 0");
        return;
    }
    ...
    $user = [
        ...
        'tok_fingerprint'  => $result['tok_fingerprint']  ?? ooredooRandHex(),
        'tok_timestamp'    => $result['tok_timestamp']    ?? ooredooNowMs(),
        'tok_instance_id'  => $result['tok_instance_id']  ?? '',
...
{
    return [
        'fingerprint'    => $user['tok_fingerprint'] ?? ooredooRandHex(),
        'timestamp'      => $user['tok_timestamp']   ?? ooredooNowMs(),
        'tok_instance_id'=> $user['tok_instance_id'] ?? '',
        'tok_plat_sig'   => $user['tok_plat_sig']    ?? '',
        'correlation'    => $user['tok_correlation'] ?? '',
    ];
}
...
function ooredooSendPostLoginInfo(string $psid, array $user): void
{
    ...
    $validateData = ooredooValidateUser($msisdn, $token, $tokenProxy, $tokHdrs, $psid);
    $giftData     = ooredooGamificationStatus($msisdn, $token, $tokenProxy, $tokHdrs, $psid);
 ...
 */
function ooredooAuthedHeaders(string $msisdn, string $accessToken, string $contentType = '', string $psid = ''): array
{
    $identity = ooredooGetSessionIdentity($psid);
    $ts       = ooredooNowMs();
    $hdrs     = [
        ...
        'x-version: '                     . OOREDOO_X_VERSION,
        'x-signature: '                   . OOREDOO_X_SIG,
        'x-platform-data-signature: '     . $identity['platform_sig'],
        "x-timestamp: {$ts}",
...
function activateOoredooOffer(string $psid, array $user, int $offerNum): void
{
    ...
    if ($offer['type'] === 'dynamic') {
        ...
        $hdrs    = ooredooAuthedHeaders($msisdn, $token, 'application/json; charset=utf-8', $psid);
...
function activateOoredooSnapchat(string $psid, array $user): void
{
    ...
    $hdrs = ooredooAuthedHeaders($msisdn, $token, '', $psid);
...
function activateOoredooDailyGift(string $psid, array $user): void
{
    ...
    $result = ooredooGamificationPlay($msisdn, $token, $tokenProxy, $tokHdrs, $psid);
    ...
    if ($result['already_played'] ?? false) {
        ...
        $giftData2 = ooredooGamificationStatus($msisdn, $token, $tokenProxy, $tokHdrs, $psid);
 ...
 */
function ooredooRefreshToken(string $refreshToken, string $msisdn, ?array $tokenProxy = null, string $psid = ''): array|false
{
    $identity = ooredooGetSessionIdentity($psid);
    $iid      = $identity['instance_id'];
    $platSig  = $identity['platform_sig'];
