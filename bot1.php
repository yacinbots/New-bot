...
define('OOREDOO_PLAT_SIG',     'eyJwbGF0Zm9ybSI6ImFuZHJvaWQiLCJpcy1waHlzaWNhbC1kZXZpY2UiOnRydWUsImRldmljZS1pZCI6IjljNDE2OTMwLWE0YmUtMTFmMS1iMzVkLWVkM2Y5NzEzNjZjMzE3ODgxMjczOTk3NDcifQ==');
...
// fingerprint مشترك لكل طلبات ما بعد المصادقة (status, validateUser, play)
define('OOREDOO_FP_AUTHED',    'fe0645ef29bfa2fb33fc06793ee5c28efe984e49882e21d16d49411af3a3ef45');
// fingerprint خاص بطلب token1 (بدون OTP) — مُستخرج من النموذج
define('OOREDOO_FP_TOKEN1',    'deb1e6b940af70681feb13e8695e7d098c1c408b862e13615de463ad0eeb956f');
// fingerprint خاص بطلب token2 (مع OTP) — مُستخرج من النموذج
define('OOREDOO_FP_TOKEN2',    '8fbdad0928c41754cc7cbd9379db399f1fe06fb7dbd6275a7616dfecc19d7246');
// fingerprint خاص بـ gamification/play checkpoint — مُستخرج من النموذج
define('OOREDOO_FP_GPLAY_CHK', '193eb3908167a39506cc7ed89ce063e48733712cc562ae020933d56b694d86ce');
// correlation-id ثابت لطلبات ما بعد المصادقة (يُعاد استخدامه كقيمة افتراضية)
define('OOREDOO_CORR_AUTHED',  'c0f498f0-a578-11f1-b79f-c5cef379ec311788207347712');
...
// ════════════════════════════════════════════════════════════════════════════
function ooredooBaseHeaders(string $msisdn, string $fingerprint, string $correlationId, string $timestamp): array
{
    return [
        ...
        'x-platform-data-signature: ' . OOREDOO_PLAT_SIG,
        "x-timestamp: 1788205325462",
...
function ooredooSendOTP(string $msisdn): array|false
{
    ...
    $ts     = (string)(time() * 1000);
    $corrId = bin2hex(random_bytes(8)) . $ts;
    ...
    foreach ($candidates12 as $proxy12) {
        ...
        $hdrs1 = ooredooBaseHeaders($msisdn, '391d1f55cf33097ac656d9c1c8f4ef94fd427a7d6b01d5b17e490886a8e1c5ea', $corrId, $ts);
        ...
        $hdrs2 = ooredooBaseHeaders($msisdn, '03a0ff3c4002aaae3a2902683136d8cbca1104dd60e69b3392f3e2f67238e070', bin2hex(random_bytes(8)) . $ts, $ts);
    ...
    foreach ($candidates34 as $chkProxy) {
        ...
        $hdrs3 = ooredooBaseHeaders($msisdn, 'd717f779e8ab59712211d85641a7e66fdc77fc21722d5c0b88395202420a0d69', bin2hex(random_bytes(8)) . $ts, $ts);
        ...
        // OOREDOO_FP_TOKEN1: fingerprint ثابت مُستخرج من نموذج token بدون OTP
        $hdrs4 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: ' . OOREDOO_FP_TOKEN1,
            "x-nonce-id: {$nonce1}",
            'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
            'platform: ' . OOREDOO_PLATFORM,
            'x-version: ' . OOREDOO_X_VERSION,
            ...
            "x-timestamp: 1788205325462",
            'x-platform-info: ' . OOREDOO_PLAT_SIG,
            'x-force-update: true',
            'x-instance-id: ' . OOREDOO_INSTANCE_ID,
        ];
...
function ooredooVerifyOTP(string $msisdn, string $otp, ?array $stickyProxy): array|string|false
{
    ...
    foreach ($candidates as $proxy) {
        ...
        $hdrs6 = ooredooBaseHeaders($msisdn, '5e2db32cbb58d13fd03534c8cc334b6ded90061fe95fc5ff3aff43b4777ef086', $corrId, $ts);
        ...
        // OOREDOO_FP_TOKEN2: fingerprint ثابت مُستخرج من نموذج token مع OTP
        $hdrs7 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: ' . OOREDOO_FP_TOKEN2,
            "x-nonce-id: {$nonce2}",
            'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
            'platform: ' . OOREDOO_PLATFORM,
            'x-version: ' . OOREDOO_X_VERSION,
        ...
        if ($code7 === 200) {
            ...
            if (isset($json['access_token'])) {
                ...
                return [
                    ...
                    // قيم headers ثابتة مرتبطة بالتوكن — من constants مركزية
                    'tok_fingerprint'   => OOREDOO_FP_TOKEN2,
                    'tok_timestamp'     => $ts,
                    'tok_instance_id'   => OOREDOO_INSTANCE_ID,
                    'tok_plat_sig'      => OOREDOO_PLAT_SIG,
...
{
    // timestamp ثابت من tok_timestamp — Ooredoo يربط التوكن به
    $ts     = $tokHdrs["timestamp"] ?? (string)(time() * 1000);
    $corrId = OOREDOO_CORR_AUTHED;

    $hdrs = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
        'x-device-fingerprint: ' . OOREDOO_FP_AUTHED,
        'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
    ...
    $code = $r['http_code'] ?? 0;
    if ($code === 401) {
        ooredooLog("[GSTATUS] 401 proxy=" . ($proxy['host'] ?? 'direct') . " ts={$ts} fp=" . substr($fingerprint, 0, 12));
        return null;
    }
    return $r['json'] ?? null;
}
...
{
    // قيم ثابتة — مُستخرجة من نماذج الطلبات الحقيقية
    // validateUser يشارك نفس fingerprint + headers بقية طلبات ما بعد المصادقة
    $ts     = $tokHdrs["timestamp"] ?? (string)(time() * 1000);
    $corrId = OOREDOO_CORR_AUTHED;
...
function ooredooGamificationPlay(string $msisdn, string $accessToken, ?array $tokenProxy = null, array $tokHdrs = []): ?array
{
    ...
    // OOREDOO_FP_GPLAY_CHK — fingerprint ثابت لـ checkpoint الخاص بـ gamification/play
    $hdrs3  = ooredooBaseHeaders($msisdn, OOREDOO_FP_GPLAY_CHK, $corrId, $ts);
    $hdrs3[] = 'content-type: application/json; charset=utf-8';
    $hdrs3[] = 'x-native-integrity-token: ' . OOREDOO_INTEGRITY_1;
    ...
    // play يُرسل من نفس الـ proxy كـ checkpoint
    // timestamp ثابت من tok_timestamp — Ooredoo يربط التوكن به
    $playTs = $tokHdrs["timestamp"] ?? $ts;
    $hdrsPlay = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
        ...
        'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
        "x-correlation-id: " . OOREDOO_CORR_AUTHED,
        'platform: ' . OOREDOO_PLATFORM,
        'x-version: ' . OOREDOO_X_VERSION,
        'x-signature: ' . OOREDOO_X_SIG,
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    $user = [
        ...
        'tok_fingerprint'  => $result['tok_fingerprint']  ?? OOREDOO_FP_TOKEN2,
        'tok_timestamp'    => $result['tok_timestamp']    ?? (string)(time() * 1000),
        'tok_instance_id'  => $result['tok_instance_id']  ?? OOREDOO_INSTANCE_ID,
        'tok_plat_sig'     => $result['tok_plat_sig']     ?? OOREDOO_PLAT_SIG,
...
{
    return [
        'fingerprint'  => $user['tok_fingerprint'] ?? OOREDOO_FP_TOKEN2,
        'timestamp'    => $user['tok_timestamp']   ?? (string)(time() * 1000),
        'instance_id'  => $user['tok_instance_id'] ?? OOREDOO_INSTANCE_ID,
        'plat_sig'     => $user['tok_plat_sig']    ?? OOREDOO_PLAT_SIG,
        'correlation'  => $user['tok_correlation'] ?? OOREDOO_CORR_AUTHED,
    ];
}
 ...
 */
function ooredooAuthedHeaders(string $msisdn, string $accessToken, string $contentType = ''): array
{
    $ts     = "1788205325462"; // ثابت — مرتبط بالتوكن
    $corrId = OOREDOO_CORR_AUTHED;
    $hdrs   = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
    ...
    foreach ($candidates as $proxy) {
        ...
        if ($code === 200 && isset($json['access_token'])) {
            ...
            return [
                ...
                'tok_instance_id'   => OOREDOO_INSTANCE_ID,
                'tok_plat_sig'      => OOREDOO_PLAT_SIG,
                'tok_correlation'   => OOREDOO_CORR_AUTHED,
            ];
        }
    }
    return false;
