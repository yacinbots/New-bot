...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — توليد هوية جهاز عشوائية لكل جلسة توكن
// ════════════════════════════════════════════════════════════════════════════

/**
 * generateOoredooDeviceIdentity — يولّد device-id عشوائي بصيغة UUID v4
 * ويبني منه instance-id و platform-data-signature (base64 JSON)
 * يُستدعى مرة واحدة عند بداية دورة التوكن ويُحفظ مع التوكن
 */
function generateOoredooDeviceIdentity(): array
{
    // UUID v4 عشوائي
    $bytes = random_bytes(16);
    $bytes[6] = chr((ord($bytes[6]) & 0x0f) | 0x40); // version 4
    $bytes[8] = chr((ord($bytes[8]) & 0x3f) | 0x80); // variant RFC 4122
    $deviceId = vsprintf('%s%s-%s-%s-%s-%s%s%s', str_split(bin2hex($bytes), 4));

    // instance-id = device-id + timestamp عشوائي طويل (نموذج أوريدو)
    $instanceSuffix = (string)mt_rand(1000000000000, 9999999999999);
    $instanceId     = $deviceId . $instanceSuffix;

    // platform-data-signature = base64(JSON)
    $platData = json_encode([
        'platform'          => 'android',
        'is-physical-device'=> true,
        'device-id'         => $deviceId,
    ], JSON_UNESCAPED_UNICODE);
    $platSig = base64_encode($platData);
...
// ════════════════════════════════════════════════════════════════════════════
function ooredooBaseHeaders(string $msisdn, string $fingerprint, string $correlationId, string $timestamp, string $instanceId = '', string $platSig = ''): array
{
    $instId  = $instanceId ?: OOREDOO_INSTANCE_ID;
    $pSig    = $platSig    ?: OOREDOO_PLAT_SIG;
    return [
        ...
        "x-platform-data-signature: {$pSig}",
...
{
    // ──────────────────────────────────────────────────────────────────────
    // توليد هوية الجهاز العشوائية مرة واحدة لكل دورة توكن كاملة
    // device-id + instance-id + plat_sig تُولَّد هنا وتُمرَّر لكل الخطوات
    // وتُحفظ مع التوكن لاستخدامها في جميع الطلبات اللاحقة
    // ──────────────────────────────────────────────────────────────────────
    $devId = generateOoredooDeviceIdentity();
    ...
    foreach ($candidates12 as $proxy12) {
        ...
        $hdrs1 = ooredooBaseHeaders($msisdn, '391d1f55cf33097ac656d9c1c8f4ef94fd427a7d6b01d5b17e490886a8e1c5ea', $corrId, $ts, $devId['instance_id'], $devId['plat_sig']);
        ...
        $hdrs2 = ooredooBaseHeaders($msisdn, '03a0ff3c4002aaae3a2902683136d8cbca1104dd60e69b3392f3e2f67238e070', bin2hex(random_bytes(8)) . $ts, $ts, $devId['instance_id'], $devId['plat_sig']);
    ...
    foreach ($candidates34 as $chkProxy) {
        ...
        $hdrs3 = ooredooBaseHeaders($msisdn, 'd717f779e8ab59712211d85641a7e66fdc77fc21722d5c0b88395202420a0d69', bin2hex(random_bytes(8)) . $ts, $ts, $devId['instance_id'], $devId['plat_sig']);
        ...
        ooredooLog('[OTP] Step3 OK nonce=' . substr($nonce1, 0, 12) . ' proxy=' . ($chkProxy['host'] ?? 'direct') . ' device_id=' . substr($devId['device_id'], 0, 8));

        // ── 4: POST token بدون OTP — نفس الـ IP كـ checkpoint ──
        // نستخدم نفس instance_id و plat_sig العشوائيين المولّدين أعلاه
        $hdrs4 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            ...
            "x-timestamp: {$ts}",
            'x-platform-info: ' . $devId['plat_sig'],
            'x-force-update: true',
            "x-instance-id: " . $devId['instance_id'],
        ];
        ...
        if ($code4 === 403 || $code4 === 401) {
            // نعيد proxy + هوية الجهاز كاملة للـ verify
            return [
                'proxy'       => $chkProxy,
                'device_id'   => $devId['device_id'],
                'instance_id' => $devId['instance_id'],
                'plat_sig'    => $devId['plat_sig'],
                'tok_ts'      => $ts,
            ];
        }
 ...
 */
function ooredooVerifyOTP(string $msisdn, string $otp, ?array $stickyProxy, array $devId = []): array|string|false
{
    // استخدام هوية الجهاز المولّدة في ooredooSendOTP إذا مُرّرت، وإلا fallback للثوابت
    $instanceId = $devId['instance_id'] ?? OOREDOO_INSTANCE_ID;
    $platSig    = $devId['plat_sig']    ?? OOREDOO_PLAT_SIG;
    ...
    foreach ($candidates as $proxy) {
        ...
        // نفس instance_id و plat_sig التي وُلّدت في ooredooSendOTP
        $hdrs6 = ooredooBaseHeaders($msisdn, '5e2db32cbb58d13fd03534c8cc334b6ded90061fe95fc5ff3aff43b4777ef086', $corrId, $ts, $instanceId, $platSig);
        ...
        // نستخدم نفس instance_id و plat_sig العشوائيين المولّدين في ooredooSendOTP
        $hdrs7 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
        ...
        if ($code7 === 200) {
            ...
            if (isset($json['access_token'])) {
                ...
                // instance_id و plat_sig العشوائيان يُحفظان لاستخدامهما في كل طلب بعده
                return [
                    'access_token'      => $json['access_token'],
                    'refresh_token'     => $json['refresh_token'] ?? '',
                    'tok_fingerprint'   => OOREDOO_FP_TOKEN2,
                    'tok_timestamp'     => $ts,
                    'tok_instance_id'   => $instanceId,
...
{
    // نستخدم القيم العشوائية المحفوظة مع التوكن — instance_id و plat_sig مختلفان لكل مستخدم
    $ts         = $tokHdrs["timestamp"]   ?? (string)(time() * 1000);
    $instanceId = $tokHdrs["instance_id"] ?? OOREDOO_INSTANCE_ID;
    $platSig    = $tokHdrs["plat_sig"]    ?? OOREDOO_PLAT_SIG;
...
function ooredooGamificationPlay(string $msisdn, string $accessToken, ?array $tokenProxy = null, array $tokHdrs = []): ?array
{
    ...
    // نستخدم القيم العشوائية المحفوظة مع التوكن
...
function handleNewOoredooPhone(string $psid, string $phone): void
{
    ...
    // حفظ proxy + هوية الجهاز العشوائية للخطوة التالية
    ooredooSaveSession($psid, [
        'step'        => 'awaiting_otp',
        'msisdn'      => $msisdn,
        'ts'          => time(),
        'device_id'   => $otpRes['device_id']   ?? '',
        'instance_id' => $otpRes['instance_id'] ?? '',
        'plat_sig'    => $otpRes['plat_sig']    ?? '',
        'tok_ts'      => $otpRes['tok_ts']      ?? '',
    ]);
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    if (preg_match('/^05\d{8}$/', preg_replace('/\D/', '', $text))) {
        ...
        // تحديث هوية الجهاز العشوائية الجديدة في الجلسة
        ooredooSaveSession($psid, [
            'step'        => 'awaiting_otp',
            'msisdn'      => $newMsisdn,
            'ts'          => time(),
    ...
    // استرجاع هوية الجهاز المحفوظة في الجلسة
    $ooredooSess = ooredooGetSession($psid);
    $savedDevId  = [
        'instance_id' => $ooredooSess['instance_id'] ?? '',
        'plat_sig'    => $ooredooSess['plat_sig']    ?? '',
    ];

    if (!$stickyProxy) {
        // انتهت صلاحية الـ proxy session → نعيد الإرسال مع هوية جهاز جديدة
    ...
    $result = ooredooVerifyOTP($msisdn, $m[1], $stickyProxy, $savedDevId);
    if ($result === 'wrong_otp') {
        sendMessage($psid, "❌ الرمز المُدخل خاطئ!\n\n🔄 أعد إرسال الرمز الصحيح\n📱 أو أرسل رقمك مجدداً\n\n❌ للإلغاء أرسل: 0");
        return;
    }
    ...
    // tok_instance_id و tok_plat_sig هنا هي القيم العشوائية المولّدة في ooredooSendOTP
    // وليست الثوابت — هذا هو المحور الأساسي لعزل كل جلسة
    ...
    $user = [
        ...
        // tok_instance_id و tok_plat_sig عشوائيان ومختلفان بين كل مستخدم
...
{
    // instance_id و plat_sig تُقرأ من بيانات المستخدم (عشوائية لكل توكن)
    // إذا لم تُحفظ بعد (مستخدمون قدامى) → fallback للثوابت
    return [
