...
define('OOR_PROXY_FILE',  '/tmp/oor_proxies.json');
define('OOR_DEFAULT_PROXIES', [
...
function loadProxies(): array
{
    ...
    return [
    "https://change4.owlproxy.com:7778:MaDT4u2q7r70_custom_zone_DZ_st__city_sid_10530093_time_5:3191220",
    "https://change4.owlproxy.com:7778:QueJJSdTrt60_custom_zone_DZ_st__city_sid_00879162_time_5:3191258",
    "https://change4.owlproxy.com:7778:40rA5rbze770_custom_zone_DZ_st__city_sid_57791286_time_5:3191271",
    "https://change4.owlproxy.com:7778:ySQbhX1sf900_custom_zone_DZ_st__city_sid_77523822_time_5:3191286",
    "https://change4.owlproxy.com:7778:TXa9K7v0Vm30_custom_zone_DZ_st__city_sid_02587203_time_5:3191295",
    "https://change4.owlproxy.com:7778:mbygZqCmZr40_custom_zone_DZ_st__city_sid_83355997_time_5:3191307",
    "https://change4.owlproxy.com:7778:5LKp3u0fXF50_custom_zone_DZ_st__city_sid_61380639_time_5:3191326",
    "https://change4.owlproxy.com:7778:KjGSXpI41U50_custom_zone_DZ_st__city_sid_60814248_time_5:3191338",
    "https://change4.owlproxy.com:7778:29VszwUvdc20_custom_zone_DZ_st__city_sid_02682668_time_5:3191349",
    "https://change4.owlproxy.com:7778:KOcyEcv9bG30_custom_zone_DZ_st__city_sid_15824756_time_5:3191355",
    "https://change4.owlproxy.com:7778:KuDtkBS0Km50_custom_zone_DZ_st__city_sid_73143606_time_5:3191362",
    "https://change4.owlproxy.com:7778:eZBedVPmkW20_custom_zone_DZ_st__city_sid_34575418_time_5:3191380",
    "https://change4.owlproxy.com:7778:ctKn9PEsp640_custom_zone_DZ_st__city_sid_51754339_time_5:3191385",
    "https://change4.owlproxy.com:7778:PmoqnhHSat90_custom_zone_DZ_st__city_sid_93894029_time_5:3191390",
    "https://change4.owlproxy.com:7778:negwLvsfDe30_custom_zone_DZ_st__city_sid_06641526_time_5:3191405",
    "https://change4.owlproxy.com:7778:csB5WOVgZP40_custom_zone_DZ_st__city_sid_21452627_time_5:3191416",
    "https://change4.owlproxy.com:7778:cWTm3aglck20_custom_zone_DZ_st__city_sid_18874628_time_5:3191422",
    "https://change4.owlproxy.com:7778:zLYJzPAFSq80_custom_zone_DZ_st__city_sid_56378044_time_5:3191429",
    "https://change4.owlproxy.com:7778:TY9rtgxbOdA0_custom_zone_DZ_st__city_sid_52126568_time_5:3191449",
    "https://change4.owlproxy.com:7778:nxIxGPJZ2Y10_custom_zone_DZ_st__city_sid_15402981_time_5:3191458",
    "https://change4.owlproxy.com:7778:JsMRqDzz2o60_custom_zone_DZ_st__city_sid_83664722_time_5:3191465",
    "https://change4.owlproxy.com:7778:pzzIvSIb1h10_custom_zone_DZ_st__city_sid_94303764_time_5:3191470",
    "https://change4.owlproxy.com:7778:Q4X4MljOH410_custom_zone_DZ_st__city_sid_58694894_time_5:3191501"
];
}
...
function sendTgMainMenu(string $chatId): void
{
    ...
    $keyboard = [
        ...
        [
            ...
            ['text' => '➕ إضافة بروكسيات Ooredoo',   'callback_data' => 'tg_setoorproxies'],
        ],
        [
            ['text' => '🗑️ مسح بروكسيات Ooredoo',     'callback_data' => 'tg_clear_oor_proxies'],
        ],
    ];
...
function handleTgCallback(string $chatId, int $msgId, string $data): void
{
    switch ($data) {
        ...
        case 'tg_setoorproxies':
            handleTgSetOorProxies($chatId);
            break;
        case 'tg_clear_oor_proxies':
            handleTgClearOorProxies($chatId);
            break;
...
function handleTgStats(string $chatId): void
{
    ...
    $text = "📊 <b>إحصائيات Tasjil BOT</b>\n\n"
          ...
          . "🔗 بروكسيات Djezzy: <b>" . count($proxies) . "</b>\n"
          . "🟠 بروكسيات Ooredoo: <b>" . count($oorProxies) . "</b>\n"
...
function handleTgShowOorProxies(string $chatId): void
{
    ...
    $count   = count($proxies);
    $src     = file_exists(OOR_PROXY_FILE) ? '📁 محفوظة' : '⚙️ افتراضية';

    $lines = ["🟠 <b>بروكسيات Ooredoo ({$count}) — {$src}</b>\n"];
    foreach ($proxies as $i => $p) {
        $pp     = parseOorProxy($p);
        $host   = explode(':', $pp['host'])[0] ?? $pp['host'];
        $lines[] = ($i + 1) . ". <code>{$host}</code>";
    ...
    $keyboard = [
        [
            ['text' => '➕ إضافة بروكسيات', 'callback_data' => 'tg_setoorproxies'],
            ['text' => '🗑️ مسح الكل',       'callback_data' => 'tg_clear_oor_proxies'],
        ],
        [['text' => '🔙 رجوع', 'callback_data' => 'tg_stats']],
    ];
    tgSendMessage($chatId, implode("\n", $lines), $keyboard);
...
function handleTgSetOorProxies(string $chatId): void
{
    ...
    tgSendMessage($chatId,
        "🟠 <b>إضافة بروكسيات Ooredoo</b>\n\n"
        . "أرسل قائمة البروكسيات، كل بروكسي في سطر بأحد الصيغتين:\n\n"
        . "• <code>http://user:pass@host:port</code>\n"
        . "• <code>http://host:port:user:pass</code>\n\n"
        . "⚠️ ستُضاف إلى القائمة الحالية.\n"
        . "للاستبدال الكامل أرسل: /clearoor ثم أضف\n\n"
        . "/cancel للإلغاء"
    );
}
...
function handleTgOorProxiesInput(string $chatId, string $text): void
{
    ...
    foreach ($list as $item) {
        ...
        if (preg_match('#^https?://[^:]+:[^@]+@[^:]+:\d+#', $item)) { $valid[] = $item; continue; }
        // Colon format: http://host:port:user:pass
        if (preg_match('#^https?://.+:.+:.+:.+$#', $item)) { $valid[] = $item; continue; }
    }
    ...
    if (empty($valid)) {
        tgSendMessage($chatId,
            ...
            . "الصيغة المقبولة:\n"
            . "<code>http://user:pass@host:port</code>\n"
            . "أو: <code>http://host:port:user:pass</code>"
        );
        return;
    }

    // Merge with existing
    $existing = file_exists(OOR_PROXY_FILE)
        ? (@json_decode(@file_get_contents(OOR_PROXY_FILE), true) ?? [])
        : [];
    $merged   = array_values(array_unique(array_merge($existing, $valid)));
    saveOorProxies($merged);
    clearTgState($chatId);

    tgSendMessage($chatId,
        "✅ تم إضافة <b>" . count($valid) . "</b> بروكسي Ooredoo.\n"
        . "📦 الإجمالي الآن: <b>" . count($merged) . "</b>"
    );
    sendTgMainMenu($chatId);
}

function handleTgClearOorProxies(string $chatId): void
{
    if (file_exists(OOR_PROXY_FILE)) @unlink(OOR_PROXY_FILE);
    tgSendMessage($chatId,
        "🗑️ تم مسح بروكسيات Ooredoo المحفوظة.\n\n"
        . "⚙️ سيتم الرجوع للبروكسيات الافتراضية (" . count(OOR_DEFAULT_PROXIES) . " بروكسي)."
    );
    sendTgMainMenu($chatId);
}
...
{
    if (file_exists(OOR_PROXY_FILE)) {
        $d = @json_decode(@file_get_contents(OOR_PROXY_FILE), true);
        if (is_array($d) && count($d) > 0) return $d;
    }
    return OOR_DEFAULT_PROXIES;
}
function saveOorProxies(array $proxies): void
{
    file_put_contents(OOR_PROXY_FILE, json_encode($proxies, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
}
function parseOorProxy(string $proxy): array
{
    // Supports both:
    //   http://user:pass@host:port  (URL format)
    //   http://host:port:user:pass  (colon format)
    if (preg_match('#^https?://([^:]+):([^@]+)@([^:]+):(\d+)#', $proxy, $m)) {
        return ['host' => "{$m[3]}:{$m[4]}", 'userpass' => "{$m[1]}:{$m[2]}"];
    }
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — HTTP Client (Proxy-Rotating, HTML-rejection aware)
// ════════════════════════════════════════════════════════════════════════════

/** Single curl attempt — returns result with 'html_rejected' flag */
function oorCurlSingle(string $url, string $method, string $body, array $headers, string $proxyHost, string $proxyAuth): array
{
    $ch   = curl_init($url);
    $opts = [
        CURLOPT_RETURNTRANSFER => true,
    ...
    if ($err || $raw === false)
        return ['http_code'=>0,'body'=>'','json'=>null,'resp_headers'=>[],'html_rejected'=>false];
    ...
    $isHtml = (stripos($bodyStr, '<html') !== false
            || stripos($bodyStr, '<!DOCTYPE') !== false
            || stripos($bodyStr, 'Request Rejected') !== false);

    return ['http_code'=>$code,'body'=>$bodyStr,'json'=>$json,'resp_headers'=>$respH,'html_rejected'=>$isHtml];
}

/**
 * oorCurl — rotates through Ooredoo proxies.
 * HTML rejection → silent retry on next proxy.
 * All proxies rejected → http_code -2 (server busy).
 */
function oorCurl(string $url, string $method, string $body, array $headers): array
{
    $proxies = getOorProxies();
    foreach ($proxies as $proxy) {
        $pp = parseOorProxy($proxy);
        $r  = oorCurlSingle($url, $method, $body, $headers, $pp['host'], $pp['userpass']);
        file_put_contents('/tmp/oor.log',
            date('Y-m-d H:i:s')." [{$method}] HTTP:{$r['http_code']} HTML:".($r['html_rejected']?'1':'0')." PROXY:{$pp['host']}\n",
            FILE_APPEND);
        if (!$r['html_rejected']) return $r; // clean response — return immediately
        // HTML rejection → try next proxy silently
    }
    // All proxies blocked by WAF / load
    file_put_contents('/tmp/oor.log', date('Y-m-d H:i:s')." [oorCurl] ALL ".count($proxies)." proxies HTML-rejected — {$url}\n", FILE_APPEND);
    return ['http_code'=>-2,'body'=>'','json'=>null,'resp_headers'=>[],'html_rejected'=>true];
}
...
function oorVerifyOTP(string $msisdn, string $otp): mixed
{
    ...
    $r = oorCurl(OOR_AUTH_URL, 'POST', $payload, $h);
    if ($r['http_code'] === -2) return 'server_busy';
...
{
    // Call checkpoint for gamification endpoint first
...
function handleOorAwaitingOtp(string $psid, string $text, array $session): void
{
    ...
    if ($result === 'server_busy') {
        sendMessage($psid, "⚠️ هناك ضغط على Ooredoo حالياً، حاول لاحقاً.\n\n📱 أرسل رقمك مجدداً للمحاولة.\n❌ للإلغاء أرسل: 0");
        return;
    }
    if ($result === 'wrong_otp') {
        sendMessage($psid,
...
function handleOorDailyGift(string $psid, string $msisdn, string $at, string $rt, array $sess): void
{
    ...
    if ($r['http_code'] === -2) { sendMessage($psid, "⚠️ هناك ضغط على Ooredoo حالياً، حاول لاحقاً."); return; }
