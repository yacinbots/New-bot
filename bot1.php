...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — HTTP Client (Direct — No Proxy)
// ════════════════════════════════════════════════════════════════════════════
function oorCurl(string $url, string $method, string $body, array $headers): array
{
    ...
    $err        = curl_error($ch);
    curl_close($ch);
    file_put_contents('/tmp/oor.log', date('Y-m-d H:i:s')." [{$method} {$url}] HTTP:{$code} ERR:{$err}\n", FILE_APPEND);
    if ($err || $raw === false) return ['http_code'=>0,'body'=>'','json'=>null,'resp_headers'=>[]];
