...
function ooredooSendOTP(string $msisdn): array|false
{
    ...
    $res1   = ooredooRequest($url1 . '?msisdn=' . urlencode($msisdn), 'GET', '', $hdrs1, 'OO_STATUS');
    if (!$res1) { ooredooLog("[OTP] Step1 failed"); return false; }
    $proxy = $res1['proxy'];
    ...
    $res2  = ooredooRequest($url2, 'HEAD', '', $hdrs2, 'OO_USERINFO', 12, $proxy);
    if (!$res2) { ooredooLog("[OTP] Step2 failed"); return false; }
    $code2 = $res2['result']['http_code'] ?? 0;
    if ($code2 !== 204) { ooredooLog("[OTP] Step2 unexpected code={$code2}"); return false; }

    // ── 3: POST /checkpoint/token → nonce1+chronos1 ──
    ...
    $hdrs3[] = 'x-method: POST';
    $res3   = ooredooRequest($url3, 'POST', '{}', $hdrs3, 'OO_CHK1', 12, $proxy);
    if (!$res3) { ooredooLog("[OTP] Step3 checkpoint failed"); return false; }
    $nonce1   = $res3['result']['resp_headers']['x-nonce-id']   ?? '';
    $chronos1 = $res3['result']['resp_headers']['x-chronos-id'] ?? '';
    if (!$nonce1 || !$chronos1) { ooredooLog("[OTP] Step3 no nonce/chronos"); return false; }

    // ── 4: POST /openid-connect/token (بدون OTP) → 403 = SMS أُرسل ──
    $hdrs4 = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
    ...
    $res4 = ooredooRequest(OOREDOO_AUTH, 'POST', $body4, $hdrs4, 'OO_TOKEN1', 12, $proxy);
    if (!$res4) { ooredooLog("[OTP] Step4 token1 failed"); return false; }
    $code4 = $res4['result']['http_code'] ?? 0;
    if ($code4 === 403) {
        // ✅ SMS أُرسل — نحفظ proxy للاستخدام لاحقاً في خطوة OTP
        return ['nonce1' => $nonce1, 'chronos1' => $chronos1, 'proxy' => $proxy];
    }
    if ($code4 === 401) { ooredooLog("[OTP] Step4 401 — رقم خاطئ أو غير مسجل"); return false; }
    ooredooLog("[OTP] Step4 unexpected code={$code4}");
    return false;
}
...
function ooredooVerifyOTP(string $msisdn, string $otp, array $stickyProxy): array|string|false
{
    ...
    // ── 6: POST /checkpoint/token → nonce2+chronos2 (نفس IP) ──
    $url3   = OOREDOO_BFF . '/checkpoint/token';
    ...
    $hdrs6[] = 'x-method: POST';
    $res6 = ooredooRequest($url3, 'POST', '{}', $hdrs6, 'OO_CHK2', 12, $stickyProxy);
    if (!$res6) { ooredooLog("[VERIFY] Step6 checkpoint2 failed"); return false; }
    $nonce2   = $res6['result']['resp_headers']['x-nonce-id']   ?? '';
    $chronos2 = $res6['result']['resp_headers']['x-chronos-id'] ?? '';
    if (!$nonce2 || !$chronos2) { ooredooLog("[VERIFY] Step6 no nonce/chronos"); return false; }

    // ── 7: POST /openid-connect/token (مع OTP) ──
    $hdrs7 = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
    ...
    $res7 = ooredooRequest(OOREDOO_AUTH, 'POST', $body7, $hdrs7, 'OO_TOKEN2', 12, $stickyProxy);
    if (!$res7) { ooredooLog("[VERIFY] Step7 token2 failed"); return false; }
    $code7 = $res7['result']['http_code'] ?? 0;
    if ($code7 === 403) return 'wrong_otp';
    if ($code7 === 401) return 'wrong_otp';
    if ($code7 === 200) {
        $json = $res7['result']['json'];
    ...
    ooredooLog("[VERIFY] Step7 unexpected code={$code7} body=" . substr($res7['result']['body'] ?? '', 0, 200));
    return false;
}
