...
function processEvent(string $psid, array $event): void
{
    ...
    if ($state === 'oo_awaiting_otp')           { handleOoredooOtp($psid, $text, $session); return; }
    if ($state === 'oo_menu')                   { handleOoredooMenuInput($psid, $text, $session); return; }

    // أرقام الهاتف تتجاوز الـ pending دائماً
    if (preg_match('/^07\d{8}$/', $digits)) { clearPending($psid); handleNewPhone($psid, $digits); return; }
    if (preg_match('/^05\d{8}$/', $digits)) { clearPending($psid); handleOoredooPhone($psid, $digits); return; }
...
{
    if (str_starts_with($payload, 'OO_')) { handleOoredooPostback($psid, $payload); return; }
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO ALGERIA — Integration
// ════════════════════════════════════════════════════════════════════════════

// ─── Constants ───────────────────────────────────────────────────────────────
define('OO_BASE_URL',       'https://apis.ooredoo.dz');
define('OO_BFF_BASE',       OO_BASE_URL . '/api/ooredoo-bff');
define('OO_AUTH_TOKEN_URL', OO_BASE_URL . '/api/auth/realms/myooredoo/protocol/openid-connect/token');
define('OO_CLIENT_ID',      'myooredoo-app');
define('OO_X_VERSION',      '1.5.15');
define('OO_X_SIGNATURE',    'f320f896f3da2a5a0284f9af316efb4ab0432b26406413568db116fa9dc60feb');
define('OO_PLATFORM',       'android');
define('OO_X_PLATFORM_ORIGIN', 'mobile-android');
define('OO_DEVICE_ID',      '9c416930-a4be-11f1-b35d-ed3f971366c3');
define('OO_INSTANCE_ID',    '9c416930-a4be-11f1-b35d-ed3f971366c31788127399747');
define('OO_PLATFORM_DATA_SIG', 'eyJwbGF0Zm9ybSI6ImFuZHJvaWQiLCJpcy1waHlzaWNhbC1kZXZpY2UiOnRydWUsImRldmljZS1pZCI6IjljNDE2OTMwLWE0YmUtMTFmMS1iMzVkLWVkM2Y5NzEzNjZjMzE3ODgxMjczOTk3NDcifQ==');

// Static headers values extracted from real captures
define('OO_STATUS_FP',      '391d1f55cf33097ac656d9c1c8f4ef94fd427a7d6b01d5b17e490886a8e1c5ea');
define('OO_STATUS_CORR',    'eb6453a0-a555-11f1-b11e-b52f92bd0a701788192386522');
define('OO_STATUS_TS',      '1788192386564');

define('OO_UINFO_FP',       '03a0ff3c4002aaae3a2902683136d8cbca1104dd60e69b3392f3e2f67238e070');
define('OO_UINFO_CORR',     'ed48a400-a555-11f1-b11e-b52f92bd0a701788192389696');
define('OO_UINFO_TS',       '1788192389709');

define('OO_CHK1_FP',        'd717f779e8ab59712211d85641a7e66fdc77fc21722d5c0b88395202420a0d69');
define('OO_CHK1_CORR',      'edb2fe90-a555-11f1-b11e-b52f92bd0a701788192390393');
define('OO_CHK1_TS',        '1788192390400');
define('OO_CHK1_INTG',      '');

define('OO_TOK1_FP',        'deb1e6b940af70681feb13e8695e7d098c1c408b862e13615de463ad0eeb956f');
define('OO_TOK1_TS',        '1788192391037');

define('OO_CHK2_FP',        '5e2db32cbb58d13fd03534c8cc334b6ded90061fe95fc5ff3aff43b4777ef086');
define('OO_CHK2_CORR',      'f3179c10-a555-11f1-b11e-b52f92bd0a701788192399441');
define('OO_CHK2_TS',        '1788192399552');
define('OO_CHK2_INTG',      '');

define('OO_TOK2_FP',        '8fbdad0928c41754cc7cbd9379db399f1fe06fb7dbd6275a7616dfecc19d7246');
define('OO_TOK2_TS',        '1788192400922');

define('OO_VALIDATE_FP',    'fe0645ef29bfa2fb33fc06793ee5c28efe984e49882e21d16d49411af3a3ef45');
define('OO_GAMIF_FP',       'fe0645ef29bfa2fb33fc06793ee5c28efe984e49882e21d16d49411af3a3ef45');

// ─── Low-level curl (بدون بروكسي) ────────────────────────────────────────────
function ooredooCurl(string $url, string $method, $payload, array $headers, string $tag, int $timeout = 12): array
{
    // نجمع الـ response headers بـ variable منفصل
    $respHeaders = '';
    $ch = curl_init($url);

    // نحذف Accept-Encoding من الـ headers لأن curl سيضيفها تلقائياً
    $filteredHeaders = array_values(array_filter($headers, function($h) {
    ...
    $opts = [
        ...
        CURLOPT_HTTPHEADER     => $filteredHeaders,
        CURLOPT_FOLLOWLOCATION => true,
        CURLOPT_MAXREDIRS      => 2,
        // لا CURLOPT_HEADER ولا CURLOPT_ENCODING — نستخدم writeheader
        CURLOPT_WRITEHEADER    => fopen('php://temp', 'w+'),
    ];
    ...
    // اقرأ الـ headers من الـ temp stream
    $hdrStream = curl_getinfo($ch, CURLINFO_HEADER_OUT);
    rewind($opts[CURLOPT_WRITEHEADER]);
    $hdrStr = stream_get_contents($opts[CURLOPT_WRITEHEADER]);
    fclose($opts[CURLOPT_WRITEHEADER]);
    curl_close($ch);

    // فك gzip إذا لزم
    if (strlen($body) >= 2 && ord($body[0]) === 0x1f && ord($body[1]) === 0x8b) {
        $body = (string)(@gzdecode($body) ?: $body);
    }

    $json = @json_decode($body, true);

    dbg("[$tag] CODE:{$httpCode} ERR:{$err} BODY:" . substr($body, 0, 150));

    if ($err && $httpCode === 0) return ['code' => 0, 'headers' => '', 'body' => '', 'json' => null];

    return ['code' => $httpCode, 'headers' => $hdrStr, 'body' => $body, 'json' => $json];
}

// ─── Base headers builder ─────────────────────────────────────────────────────
function ooBaseHeaders(string $msisdn, string $fp, string $corr, string $ts): array
{
    return [
        ...
        'x-platform-origin: ' . OO_X_PLATFORM_ORIGIN,
        "x-correlation-id: {$corr}",
        'platform: ' . OO_PLATFORM,
        'x-version: ' . OO_X_VERSION,
        'x-signature: ' . OO_X_SIGNATURE,
        'x-platform-data-signature: ' . OO_PLATFORM_DATA_SIG,
        "x-timestamp: {$ts}",
        'x-instance-id: ' . OO_INSTANCE_ID,
    ];
}

// ─── Step 1: GET /users/status ────────────────────────────────────────────────
function ooCheckStatus(string $msisdn): ?array
{
    $url     = OO_BFF_BASE . '/users/status?msisdn=' . urlencode($msisdn);
    $headers = ooBaseHeaders($msisdn, OO_STATUS_FP, OO_STATUS_CORR, OO_STATUS_TS);
    $r       = ooredooCurl($url, 'GET', null, $headers, 'oo_status');
    if ($r['code'] !== 200 || !is_array($r['json'])) return null;
    return $r['json'];
}

// ─── Step 2: HEAD /userInfo ───────────────────────────────────────────────────
function ooCheckUserInfo(string $msisdn): int
{
    $url     = OO_BFF_BASE . '/userInfo?msisdn=' . urlencode($msisdn);
    $headers = ooBaseHeaders($msisdn, OO_UINFO_FP, OO_UINFO_CORR, OO_UINFO_TS);
    $r       = ooredooCurl($url, 'HEAD', null, $headers, 'oo_userinfo');
    return $r['code'];
}

// ─── Checkpoint helper ────────────────────────────────────────────────────────
function ooCheckpoint(string $msisdn, int $which): array
{
    $fp   = ($which === 1) ? OO_CHK1_FP   : OO_CHK2_FP;
    $corr = ($which === 1) ? OO_CHK1_CORR : OO_CHK2_CORR;
    $ts   = ($which === 1) ? OO_CHK1_TS   : OO_CHK2_TS;
    $intg = ($which === 1) ? OO_CHK1_INTG : OO_CHK2_INTG;

    $url     = OO_BFF_BASE . '/checkpoint/token';
    $headers = array_merge(ooBaseHeaders($msisdn, $fp, $corr, $ts), [
    ...
    $r       = ooredooCurl($url, 'POST', '{}', $headers, "oo_chk{$which}");
    $nonce   = '';
    $chronos = '';

    // استخراج X-Nonce-Id و X-Chronos-Id من الـ headers (case-insensitive)
    foreach (explode("\r\n", $r['headers']) as $line) {
        $lline = strtolower(trim($line));
        if (str_starts_with($lline, 'x-nonce-id:')) {
            $nonce   = trim(substr($line, strpos($line, ':') + 1));
        }
        if (str_starts_with($lline, 'x-chronos-id:')) {
            $chronos = trim(substr($line, strpos($line, ':') + 1));
        }
    }

    dbg("[oo_chk{$which}] code={$r['code']} nonce={$nonce} chronos={$chronos}");

    return ['nonce' => $nonce, 'chronos' => $chronos, 'code' => $r['code']];
}

// ─── Token request (step 4 & 7) ──────────────────────────────────────────────
function ooTokenRequest(string $msisdn, string $nonce, string $chronos, string $otp = ''): array
{
    $fp = $otp ? OO_TOK2_FP : OO_TOK1_FP;
    $ts = $otp ? OO_TOK2_TS : OO_TOK1_TS;

    $payload = ['client_id' => OO_CLIENT_ID, 'grant_type' => 'password', 'username' => $msisdn];
    ...
    $headers = [
        ...
        "x-timestamp: {$ts}",
        'x-platform-info: ' . OO_PLATFORM_DATA_SIG,
        'x-force-update: true',
        'x-instance-id: ' . OO_INSTANCE_ID,
        'content-type: application/x-www-form-urlencoded',
    ];

    $r = ooredooCurl(OO_AUTH_TOKEN_URL, 'POST', http_build_query($payload), $headers, 'oo_token' . ($otp ? '2' : '1'));
    return $r;
}

// ─── sendOTP trigger (steps 1→4, يُطلق SMS) ──────────────────────────────────
function ooSendOTP(string $msisdn): array
{
    // Step 1 — GET /users/status
    $status = ooCheckStatus($msisdn);
    if ($status === null) {
        // إذا فشل الـ status نكمل بدونه — بعض الأرقام قد لا تعيد 200
        file_put_contents('/tmp/ooredoo.log', date('Y-m-d H:i:s') . " [ooSendOTP] status returned null, continuing\n", FILE_APPEND);
    } else {
        $statusCode = $status['code'] ?? '';
        // '002' = رقم غير موجود
        if ($statusCode === '002') return ['ok' => false, 'reason' => 'not_found'];
        // '004' أو أي كود آخر → نكمل
    }

    // Step 2 — HEAD /userInfo (نتقبل أي كود — الهدف فقط إرسال الطلب)
    $uinfoCode = ooCheckUserInfo($msisdn);
    file_put_contents('/tmp/ooredoo.log', date('Y-m-d H:i:s') . " [ooSendOTP] userinfo_code={$uinfoCode}\n", FILE_APPEND);
    // لا نوقف التنفيذ هنا — نكمل للـ checkpoint

    // Step 3 — checkpoint أول → نحصل على nonce + chronos
    $chk1 = ooCheckpoint($msisdn, 1);
    file_put_contents('/tmp/ooredoo.log', date('Y-m-d H:i:s') . " [ooSendOTP] chk1_code={$chk1['code']} nonce=" . substr($chk1['nonce'], 0, 20) . "\n", FILE_APPEND);

    if (empty($chk1['nonce'])) {
        return ['ok' => false, 'reason' => 'chk1_fail'];
    }

    // Step 4 — token بدون OTP → يُطلق SMS
    // الكود المتوقع 403، لكن نتقبل 401 أيضاً — أي 4xx = SMS أُطلق
    $r = ooTokenRequest($msisdn, $chk1['nonce'], $chk1['chronos'], '');
    file_put_contents('/tmp/ooredoo.log', date('Y-m-d H:i:s') . " [ooSendOTP] token1_code={$r['code']}\n", FILE_APPEND);

    if ($r['code'] >= 400 && $r['code'] < 500) {
        // 4xx = SMS أُرسل بنجاح (403 هو الطبيعي)
        return ['ok' => true, 'nonce1' => $chk1['nonce'], 'chronos1' => $chk1['chronos']];
    }
    if ($r['code'] === 200) {
        // نادراً يعيد 200 مباشرة — نتقبله أيضاً
        return ['ok' => true, 'nonce1' => $chk1['nonce'], 'chronos1' => $chk1['chronos']];
    }

    return ['ok' => false, 'reason' => 'sms_trigger_fail_code_' . $r['code']];
}

// ─── verifyOTP (steps 6→7) ────────────────────────────────────────────────────
function ooVerifyOTP(string $msisdn, string $otp, string $nonce1, string $chronos1): array
{
    // Step 6 — checkpoint ثانٍ
    $chk2 = ooCheckpoint($msisdn, 2);
    if (empty($chk2['nonce'])) return ['ok' => false, 'reason' => 'chk2_fail'];

    // Step 7 — token مع OTP
    $r = ooTokenRequest($msisdn, $chk2['nonce'], $chk2['chronos'], $otp);

    if ($r['code'] === 200 && isset($r['json']['access_token'])) {
        return ['ok' => true, 'token' => $r['json']];
    }
    if ($r['code'] === 403) return ['ok' => false, 'reason' => 'wrong_otp'];
    if ($r['code'] === 401) return ['ok' => false, 'reason' => 'wrong_otp'];

    return ['ok' => false, 'reason' => 'verify_fail'];
}

// ─── validateUser POST-login ──────────────────────────────────────────────────
function ooValidateUser(string $msisdn, string $accessToken): ?array
{
    $corr = 'oo-val-' . time() . '-' . rand(1000,9999);
    $ts   = (string)(time() * 1000);
    $url  = OO_BFF_BASE . '/users/validateUser?msisdn=' . urlencode($msisdn);

    $headers = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
        'x-device-fingerprint: ' . OO_VALIDATE_FP,
        'x-platform-origin: ' . OO_X_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
        "x-correlation-id: {$corr}",
        'platform: ' . OO_PLATFORM,
    ...
    $r = ooredooCurl($url, 'GET', null, $headers, 'oo_validate');
    return is_array($r['json']) ? $r['json'] : null;
}

// ─── gamificationStatus ──────────────────────────────────────────────────────
function ooGamifStatus(string $msisdn, string $accessToken): ?array
{
    $corr = 'oo-gam-' . time() . '-' . rand(1000,9999);
    $ts   = (string)(time() * 1000);
    $url  = OO_BFF_BASE . '/gamification/status';

    $headers = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
        'x-device-fingerprint: ' . OO_GAMIF_FP,
        'x-platform-origin: ' . OO_X_PLATFORM_ORIGIN,
        "authorization: Bearer {$accessToken}",
        "x-correlation-id: {$corr}",
        'platform: ' . OO_PLATFORM,
    ...
    $r = ooredooCurl($url, 'GET', null, $headers, 'oo_gamif');
    return is_array($r['json']) ? $r['json'] : null;
}

// ─── Ooredoo Phone Entry Point ────────────────────────────────────────────────
function handleOoredooPhone(string $psid, string $phone): void
{
    $msisdn = '213' . substr($phone, 1);

    dbg("[OO] handleOoredooPhone called psid={$psid} phone={$phone} msisdn={$msisdn}");
    sendMessage($psid, "🔄 جاري التحقق من الرقم {$phone}...");

    // Step 1 — status
    dbg("[OO] calling ooCheckStatus msisdn={$msisdn}");
    $statusData = ooCheckStatus($msisdn);
    dbg("[OO] ooCheckStatus result=" . json_encode($statusData));

    if ($statusData !== null) {
        $statusCode = $statusData['code'] ?? '';
        if ($statusCode === '002') {
            sendMessage($psid, "❌ هذا الرقم غير موجود في شبكة أوريدو.\n\n📱 أرسل رقمك مجدداً.");
            dbg("[OO] not_found code=002");
            return;
        }
    }

    // Step 2 — userInfo
    dbg("[OO] calling ooCheckUserInfo");
    $uCode = ooCheckUserInfo($msisdn);
    dbg("[OO] ooCheckUserInfo code={$uCode}");

    // Step 3 — checkpoint1
    dbg("[OO] calling ooCheckpoint(1)");
    $chk1 = ooCheckpoint($msisdn, 1);
    dbg("[OO] checkpoint1 code={$chk1['code']} nonce=" . substr($chk1['nonce'], 0, 20) . " chronos=" . substr($chk1['chronos'], 0, 20));

    if (empty($chk1['nonce'])) {
        sendMessage($psid, "⚠️ فشل الاتصال بسيرفر أوريدو (checkpoint). حاول مجدداً بعد لحظات.");
        dbg("[OO] chk1_fail nonce empty");
        return;
    }

    // Step 4 — token sans OTP → trigger SMS
    dbg("[OO] calling ooTokenRequest (no otp)");
    $r = ooTokenRequest($msisdn, $chk1['nonce'], $chk1['chronos'], '');
    dbg("[OO] token1 code={$r['code']} body=" . substr($r['body'], 0, 100));
    ...
    if ($r['code'] >= 400 && $r['code'] < 500) {
        ...
        sendMessage($psid, "⚠️ تعذر إرسال رمز التحقق (code={$r['code']}).\n\nحاول لاحقاً.\n⚡ https://t.me/tasjilbott");
        dbg("[OO] sms_trigger_fail code={$r['code']}");
        return;
    }

    setSession($psid, [
        'state'    => 'oo_awaiting_otp',
        'msisdn'   => $msisdn,
        'nonce1'   => $chk1['nonce'],
        'chronos1' => $chk1['chronos'],
    ]);
    ...
    dbg("[OO] OTP sent successfully to {$msisdn}");
}

// ─── OTP Handler (step 5 UX) ─────────────────────────────────────────────────
function handleOoredooOtp(string $psid, string $text, array $session): void
{
    $msisdn      = $session['msisdn'] ?? '';
    $nonce1      = $session['nonce1'] ?? '';
    $chronos1    = $session['chronos1'] ?? '';
    $phoneDisplay = '0' . substr($msisdn, 3);
    ...
    // إذا أرسل رقم أوريدو جديد → إعادة إرسال الرمز
    $digits = preg_replace('/\D/', '', $text);
    if (preg_match('/^05\d{8}$/', $digits)) {
        $newMsisdn = '213' . substr($digits, 1);
        sendMessage($psid, "📲 جاري إعادة إرسال رمز التحقق إلى الرقم {$digits}...");
        $r = ooSendOTP($newMsisdn);
        if (!$r['ok']) {
            if ($r['reason'] === 'not_found') {
                sendMessage($psid, "❌ هذا الرقم غير موجود في شبكة أوريدو.\n\n📱 أرسل رقمك مجدداً.");
            } else {
                sendMessage($psid, "⚠️ تعذر إرسال الرمز، حاول مجدداً.");
            }
            return;
        }
        setSession($psid, [
        ...
        sendMessage($psid,
            "✅ تم إرسال رمز التحقق إلى الرقم {$digits}.\n\n" .
            "🔢 الرجاء إدخال الرمز المكوّن من 6 أرقام:\n\n" .
            "📱 أو أرسل رقمك مجدداً لاستقبال رمز جديد\n\n" .
            "❌ لإلغاء العملية أرسل: 0"
        );
    ...
    if (empty($msisdn) || empty($nonce1)) {
        clearSession($psid);
        sendMessage($psid, "❌ حدث خطأ في الجلسة، أرسل رقمك مجدداً.");
        return;
    }

    $verify = ooVerifyOTP($msisdn, $m[1], $nonce1, $chronos1);

    if (!$verify['ok']) {
        if ($verify['reason'] === 'wrong_otp') {
            sendMessage($psid,
                "❌ الرمز المُدخل خاطئ!\n\n" .
                "🔄 أعد إرسال الرمز الصحيح\n" .
                "📱 أو أرسل رقم هاتفك مجدداً لاستقبال رمز جديد\n\n" .
    ...
    // نجح التحقق — نجلب بيانات الحساب
    $tokenData   = $verify['token'];
    $accessToken = $tokenData['access_token'];

    sendMessage($psid, "✅ تم التحقق بنجاح! جاري تحميل بيانات حسابك...");

    $validateData = ooValidateUser($msisdn, $accessToken);
    $gamifData    = ooGamifStatus($msisdn, $accessToken);
    ...
    file_put_contents(USERS_DIR . "/oo_{$psid}.json", json_encode([
        'psid'          => $psid,
        'msisdn'        => $msisdn,
        'access_token'  => $accessToken,
        'refresh_token' => $tokenData['refresh_token'] ?? '',
        'plan_type'     => $validateData['planType'] ?? '',
        'saved_at'      => time(),
    ], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));

    sendOoredooMenu($psid, $msisdn, $validateData, $gamifData);
}

// ─── Ooredoo Menu ────────────────────────────────────────────────────────────
function sendOoredooMenu(string $psid, string $msisdn, ?array $validateData, ?array $gamifData): void
{
    $phone    = '0' . substr($msisdn, 3);
    $planType = $validateData['planType'] ?? 'PREPAID';
    $isYooz   = in_array(strtoupper($planType), ['YOOZ', "N'YOOZ", 'NYOOZ']);

    // حالة الهدية اليومية
    $played        = $gamifData['played'] ?? true;
    $lastPlayedTime = $gamifData['lastPlayedTime'] ?? '';
    $giftReady     = !$played;

    // حساب الوقت المتبقي لـ 24 ساعة إذا لم تكن جاهزة
    $giftTimeLabel = '';
    if (!$giftReady && $lastPlayedTime) {
        try {
            $lastTs  = strtotime($lastPlayedTime);
            $nextTs  = $lastTs + 86400;
            $diff    = $nextTs - time();
            if ($diff > 0) {
                $h = (int)floor($diff / 3600);
                $mn = (int)floor(($diff % 3600) / 60);
                $giftTimeLabel = " (متاحة بعد {$h}س {$mn}د)";
            }
        } catch (Throwable $e) {}
    }
    ...
    $text .= "إذا لم تظهر لك الأزرار أرسل الرقم المناسب 👇\n\n";
    $text .= "━━━━━━━━━━━━━━\n\n";

    if ($giftReady) {
        $text .= "1️⃣ تفعيل الهدية اليومية ✅ متاحة الآن\n📩 أرسل: 1\n\n";
    } else {
        $text .= "1️⃣ تفعيل الهدية اليومية{$giftTimeLabel}\n📩 أرسل: 1\n\n";
    }

    $text .= "2️⃣ تفعيل العروض\n📩 أرسل: 2\n\n";
    ...
    if ($giftReady) {
        $qr[] = ['content_type' => 'text', 'title' => '🎁 تفعيل الهدية اليومية', 'payload' => 'OO_DAILY_GIFT'];
    } else {
        $qr[] = ['content_type' => 'text', 'title' => "⏳ الهدية{$giftTimeLabel}", 'payload' => 'OO_DAILY_GIFT'];
    }

    $qr[] = ['content_type' => 'text', 'title' => '📦 تفعيل العروض',     'payload' => 'OO_OFFERS'];

    if ($isYooz) {
        $qr[] = ['content_type' => 'text', 'title' => '🎯 سناب شات يوز', 'payload' => 'OO_SNAP_YOOZ'];
    }

    $qr[] = ['content_type' => 'text', 'title' => '📊 معلوماتي', 'payload' => 'OO_INFO'];
...
// ─── Ooredoo Menu Text Input Handler ─────────────────────────────────────────
function handleOoredooMenuInput(string $psid, string $text, array $session): void
{
    $msisdn      = $session['msisdn'] ?? '';
    $accessToken = $session['access_token'] ?? '';
    $phone       = '0' . substr($msisdn, 3);

    // إذا أرسل رقم أوريدو → تسجيل دخول جديد
    $digits = preg_replace('/\D/', '', $text);
    if (preg_match('/^05\d{8}$/', $digits)) {
        handleOoredooPhone($psid, $digits);
        return;
    }

    switch (trim($text)) {
        case '1':
            handleOoredooDailyGift($psid, $msisdn, $accessToken);
            break;
        case '2':
            sendMessage($psid, "📦 خاصية العروض قيد التطوير، ترقبها قريباً! ⚡");
            break;
        case '3':
            handleOoredooSnapYooz($psid, $msisdn, $accessToken);
            break;
        case '4':
            handleOoredooInfo($psid, $msisdn, $accessToken);
            break;
        default:
            // Postbacks come through handleOoredooPostback — text inputs land here
            sendMessage($psid,
                "❌ اختيار خاطئ\n\n📌 استخدم الأزرار أو أرسل الرقم المناسب:\n\n" .
                "1️⃣ الهدية اليومية\n2️⃣ العروض\n3️⃣ سناب شات يوز (يوز/نيوز)\n4️⃣ معلوماتي"
            );
    }
}

// ─── Postback: Ooredoo ───────────────────────────────────────────────────────
// (يُستدعى من handlePostback)
function handleOoredooPostback(string $psid, string $payload): void
{
    $sess        = getSession($psid);
    $msisdn      = $sess['msisdn'] ?? '';
    $accessToken = $sess['access_token'] ?? '';

    if (empty($msisdn) || empty($accessToken)) {
        sendMessage($psid, "⚠️ الجلسة منتهية، أرسل رقم أوريدو للتسجيل من جديد.");
        return;
    }
...
// ─── Daily Gift ──────────────────────────────────────────────────────────────
function handleOoredooDailyGift(string $psid, string $msisdn, string $accessToken): void
{
    // فحص حالة الهدية أولاً
    $gamif = ooGamifStatus($msisdn, $accessToken);
    if ($gamif === null) {
        sendMessage($psid, "❌ تعذر التحقق من حالة الهدية، حاول مجدداً.");
        return;
    }

    $played         = $gamif['played'] ?? true;
    $lastPlayedTime = $gamif['lastPlayedTime'] ?? '';
    ...
    if ($played) {
        ...
        sendMessage($psid, "⏳ لقد استفدت من الهدية اليومية مسبقاً.{$label}\n\n🔁 عد لاحقاً بعد اكتمال 24 ساعة.");
        return;
    }

    // الهدية جاهزة → نطلق التفعيل
    sendMessage($psid, "🎮 جاري تفعيل الهدية اليومية...");

    $corr = 'oo-play-' . time() . '-' . rand(1000,9999);
    $ts   = (string)(time() * 1000);
    $url  = OO_BFF_BASE . '/gamification/play';
    ...
    $r = ooredooCurl($url, 'POST', '{}', $headers, 'oo_gift_play');

    if ($r['code'] === 200 || $r['code'] === 201) {
        $phone = '0' . substr($msisdn, 3);
        sendMessage($psid,
            "🎉 تم تفعيل الهدية اليومية بنجاح!\n\n" .
            "✅ الرقم: {$phone}\n\n" .
            "⚡ قناة التلغرام: https://t.me/tasjilbott"
        );
    } elseif ($r['code'] === 400 || $r['code'] === 409) {
        sendMessage($psid, "⚠️ لقد استفدت من الهدية اليومية مسبقاً، عد بعد 24 ساعة.");
    } else {
        sendMessage($psid, "❌ حدث خطأ أثناء تفعيل الهدية (HTTP {$r['code']}). حاول لاحقاً.");
    }
}

// ─── Snap Yooz ───────────────────────────────────────────────────────────────
function handleOoredooSnapYooz(string $psid, string $msisdn, string $accessToken): void
{
    sendMessage($psid, "🎯 جاري تفعيل سناب شات يوز...");

    $corr = 'oo-snap-' . time() . '-' . rand(1000,9999);
    $ts   = (string)(time() * 1000);
    $url  = OO_BFF_BASE . '/offers/activate/snapchat-yooz';
    ...
    $r = ooredooCurl($url, 'POST', '{}', $headers, 'oo_snap_yooz');

    $phone = '0' . substr($msisdn, 3);
    if ($r['code'] === 200 || $r['code'] === 201) {
        sendMessage($psid,
            "🎉 تم تفعيل سناب شات يوز بنجاح!\n\n" .
            "✅ الرقم: {$phone}\n\n" .
            "⚡ قناة التلغرام: https://t.me/tasjilbott"
        );
    } elseif ($r['code'] === 400 || $r['code'] === 409) {
        sendMessage($psid, "⚠️ لقد استفدت من هذا العرض مسبقاً أو أن رقمك غير مؤهل.");
    } else {
        sendMessage($psid, "❌ حدث خطأ أثناء التفعيل (HTTP {$r['code']}). حاول لاحقاً.");
    }
}

// ─── Info ─────────────────────────────────────────────────────────────────────
function handleOoredooInfo(string $psid, string $msisdn, string $accessToken): void
{
    sendMessage($psid, "🔍 جاري جلب معلومات حسابك...");

    $validate = ooValidateUser($msisdn, $accessToken);
    $gamif    = ooGamifStatus($msisdn, $accessToken);

    $phone     = '0' . substr($msisdn, 3);
    $planType  = $validate['planType']               ?? 'غير معروف';
    $contractType = $validate['contractType']        ?? 'غير معروف';
    $status    = $validate['msisdnStatusDescription'] ?? 'غير معروف';
    $played    = $gamif['played'] ?? null;
    $gameUI    = ($gamif['gameUIEnable'] ?? false) ? '✅' : '❌';
    $giftStatus = ($played === false) ? '✅ جاهزة' : '⏳ تم الاستخدام';

    $text  = "📊 معلومات حساب أوريدو\n\n";
    $text .= "📱 الرقم: {$phone}\n";
    $text .= "📦 نوع الشريحة: {$planType}\n";
    $text .= "📋 نوع العقد: {$contractType}\n";
    $text .= "🔆 حالة الخط: {$status}\n\n";
    $text .= "🎁 الهدية اليومية: {$giftStatus}\n";
    $text .= "🎮 واجهة الألعاب: {$gameUI}\n\n";
    $text .= "⚡ قناة التلغرام: https://t.me/tasjilbott";
