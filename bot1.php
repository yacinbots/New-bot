...
{
    // بناء command line curl — يعمل بشكل موثوق في PHP-FPM
    $cmd = 'curl -s -S --max-time ' . $timeout . ' --connect-timeout 6';
    $cmd .= ' -k';  // SSL skip verify
    $cmd .= ' -D /tmp/oo_hdr_' . getmypid() . '.txt';  // headers to file

    // Method
    if ($method === 'POST') {
        $postData = is_array($payload) ? http_build_query($payload) : (string)($payload ?? '');
        $cmd .= ' -X POST --data ' . escapeshellarg($postData);
    } elseif ($method === 'HEAD') {
        $cmd .= ' -I';
    }
    ...
    foreach ($headers as $h) {
        // skip Accept-Encoding — curl يضيفها تلقائياً
        if (stripos($h, 'Accept-Encoding:') === 0) continue;
        $cmd .= ' -H ' . escapeshellarg($h);
    }
    $cmd .= ' --compressed';
    $cmd .= ' ' . escapeshellarg($url);
    $cmd .= ' 2>/tmp/oo_err_' . getmypid() . '.txt';

    $body     = '';
    $httpCode = 0;
    $hdrStr   = '';
    $hdrFile  = '/tmp/oo_hdr_' . getmypid() . '.txt';
    $errFile  = '/tmp/oo_err_' . getmypid() . '.txt';

    $body = (string)shell_exec($cmd);
    $body = $body ?: '';

    // اقرأ الـ headers من الملف
    if (file_exists($hdrFile)) {
        $hdrStr = file_get_contents($hdrFile);
        // استخرج HTTP code من أول سطر
        if (preg_match('#HTTP/[\d.]+ (\d+)#', $hdrStr, $m)) {
            $httpCode = (int)$m[1];
        }
        @unlink($hdrFile);
    }
    ...
    if (file_exists($errFile)) {
        $err = trim(file_get_contents($errFile));
        @unlink($errFile);
    }
    ...
    if ($httpCode === 0) return ['code' => 0, 'headers' => '', 'body' => '', 'json' => null];
