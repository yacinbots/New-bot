...
function ooredooSendOTP(string $msisdn): array|false
{
    ...
    // نجرب: direct أولاً، ثم كل البروكسيات
    // checkpoint و token يُرسَلان من نفس الـ IP (sticky per iteration)
...
{
    // checkpoint و token يحتاجان TLS نظيف → direct أولاً ثم sticky ثم البروكسيات
    $useProxy   = ooredooProxyEnabled();
    $candidates = [null]; // direct دائماً في المقدمة
...
function ooredooGamificationPlay(string $msisdn, string $accessToken): ?array
{
    ...
    foreach ($candidates as $chkProxy) {
        ...
        if ($nonce && $chronos) { $usedProxy = $chkProxy; break; }
        ooredooLog('[PLAY] checkpoint no nonce proxy=' . ($chkProxy['host'] ?? 'direct'));
    }
...
function ooredooSendPostLoginInfo(string $psid, array $user): void
{
    ...
    // طلب validateUser و gamificationStatus بشكل متوازٍ (تسلسلي هنا لبساطة PHP)
    $validateData = ooredooValidateUser($msisdn, $token);
