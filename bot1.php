...
// ════════════════════════════════════════════════════════════════════════════
function ooredooGamificationStatus(string $msisdn, string $accessToken, ?array $proxy = null): ?array
{
    $ts     = (string)(time() * 1000);
    $corrId = bin2hex(random_bytes(8)) . $ts;
    $hdrs   = [
    ...
    // نستخدم نفس الـ proxy الذي أصدر التوكن — Ooredoo يربط الجلسة بالـ IP
    ...
    if (($r['http_code'] ?? 0) === 401) {
        ooredooLog("[GSTATUS] 401 Unauthorized — token/IP mismatch proxy=" . ($proxy['host'] ?? 'direct'));
        return null;
    }
    return $r['json'] ?? null;
}

function ooredooValidateUser(string $msisdn, string $accessToken, ?array $proxy = null): ?array
{
    $ts     = (string)(time() * 1000);
    $corrId = bin2hex(random_bytes(8)) . $ts;
    $hdrs   = [
    ...
    // نفس الـ proxy الذي أصدر التوكن
    ...
    if (($r['http_code'] ?? 0) === 401) {
        ooredooLog("[VALIDATE] 401 — token/IP mismatch proxy=" . ($proxy['host'] ?? 'direct'));
        return null;
    }
    return $r['json'] ?? null;
}

function ooredooGamificationPlay(string $msisdn, string $accessToken, ?array $tokenProxy = null): ?array
{
    // ─────────────────────────────────────────────────────────────────────
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    // ✅ نجح — نحفظ التوكن + البروكسي الذي أصدره
...
function ooredooSendPostLoginInfo(string $psid, array $user): void
{
    ...
    // نسترجع الـ proxy الذي أصدر التوكن — Ooredoo يربط الجلسة بالـ IP
    $tokenProxy   = ooredooGetTokenProxy($user);
    ooredooLog("[POST_LOGIN] msisdn={$msisdn} token_proxy=" . ($tokenProxy['host'] ?? 'direct'));
    $validateData = ooredooValidateUser($msisdn, $token, $tokenProxy);
    $giftData     = ooredooGamificationStatus($msisdn, $token, $tokenProxy);
...
function activateOoredooDailyGift(string $psid, array $user): void
{
    ...
    // فحص الحالة أولاً — نفس الـ proxy الذي أصدر التوكن
    $tokenProxy = ooredooGetTokenProxy($user);
    ooredooLog("[ACTIVATE] msisdn={$msisdn} token_proxy=" . ($tokenProxy['host'] ?? 'direct'));
    $giftData = ooredooGamificationStatus($msisdn, $token, $tokenProxy);
    if ($giftData !== null) {
        $played     = $giftData['played'] ?? false;
        $lastPlayed = $giftData['lastPlayedTime'] ?? null;
    ...
    $result = ooredooGamificationPlay($msisdn, $token, $tokenProxy);
    ...
    if ($result['already_played'] ?? false) {
        ...
        $giftData2 = ooredooGamificationStatus($msisdn, $token, $tokenProxy);
