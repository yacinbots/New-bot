...
{
    // نحاول عبر كل البروكسيات — كل proxy يمر بالـ 4 خطوات كاملة
    // إذا فشل أي step نحاول proxy آخر (لا نثبّت proxy قبل Step3)
    $useProxy = ooredooProxyEnabled();
    $proxies  = $useProxy ? getAllProxies() : [null];
    if ($useProxy) shuffle($proxies);

    foreach ($proxies as $rawProxy) {
        $proxy = ($rawProxy !== null) ? parseProxy($rawProxy) : null;
        $ts     = (string)(time() * 1000);
        ...
        $r1    = ooredooCurlRaw($url1, 'GET', '', $hdrs1, 'OO_STATUS', 10, $proxy);
        if (!$r1 || ($r1['ip_blocked'] ?? false) || ($r1['http_code'] ?? 0) !== 200) {
            ooredooLog("[OTP] Step1 failed proxy=" . ($proxy['host'] ?? 'direct'));
            continue;
        }

        // ── 2: HEAD /userInfo — نفس الـ proxy ──
        ...
        $r2    = ooredooCurlRaw($url2, 'HEAD', '', $hdrs2, 'OO_USERINFO', 10, $proxy);
        if (!$r2 || ($r2['ip_blocked'] ?? false) || ($r2['http_code'] ?? 0) !== 204) {
            ooredooLog("[OTP] Step2 failed proxy=" . ($proxy['host'] ?? 'direct') . " code=" . ($r2['http_code'] ?? 'null'));
            continue;
        }

        // ── 3: POST /checkpoint/token → nonce1+chronos1 — نفس الـ proxy ──
        ...
        $hdrs3[] = 'x-method: POST';
        $r3 = ooredooCurlRaw(OOREDOO_BFF . '/checkpoint/token', 'POST', '{}', $hdrs3, 'OO_CHK1', 12, $proxy);
        if (!$r3 || ($r3['ip_blocked'] ?? false)) {
            ooredooLog("[OTP] Step3 failed proxy=" . ($proxy['host'] ?? 'direct'));
            continue;
        }
        $nonce1   = $r3['resp_headers']['x-nonce-id']   ?? '';
        $chronos1 = $r3['resp_headers']['x-chronos-id'] ?? '';
        if (!$nonce1 || !$chronos1) {
            ooredooLog("[OTP] Step3 no nonce/chronos http=" . ($r3['http_code'] ?? '?'));
            continue;
        }

        // ── 4: POST token بدون OTP — نفس الـ proxy ──
        // 403 = SMS أُرسل ✅ | 401 = SMS أُرسل أيضاً (Ooredoo يرجع 401 في بعض الحالات) ✅
        $hdrs4 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
        ...
        $r4   = ooredooCurlRaw(OOREDOO_AUTH, 'POST', $body4, $hdrs4, 'OO_TOKEN1', 12, $proxy);
        if (!$r4 || ($r4['ip_blocked'] ?? false)) {
            ooredooLog("[OTP] Step4 cURL fail proxy=" . ($proxy['host'] ?? 'direct'));
            continue;
        }
        $code4 = $r4['http_code'] ?? 0;
        ooredooLog("[OTP] Step4 code={$code4} proxy=" . ($proxy['host'] ?? 'direct'));

        // 403 أو 401 = طلب SMS أُرسل بنجاح — كلاهما صحيح
        if ($code4 === 403 || $code4 === 401) {
            return ['proxy' => $proxy];
        }
        // أي كود آخر → نجرب proxy تالي
        ooredooLog("[OTP] Step4 unexpected code={$code4}, trying next proxy");
    }
...
{
    // نحاول أولاً بالـ sticky proxy، ثم نجرب باقي البروكسيات إذا فشل
    $useProxy   = ooredooProxyEnabled();
    $candidates = [];
...
function ooredooGamificationPlay(string $msisdn, string $accessToken): ?array
{
    ...
    // نختار بروكسي ثم نثبّته للـ play request
    $useProxy = ooredooProxyEnabled();
    $proxy    = null;
    if ($useProxy) {
        $proxies = getAllProxies();
        if (!empty($proxies)) {
            shuffle($proxies);
            $proxy = parseProxy($proxies[0]);
        }
    }
    $res3 = ooredooRequest($url3, 'POST', '{}', $hdrs3, 'OO_GPLAY_CHK', 12, $proxy);
    if (!$res3) return null;
    $nonce   = $res3['result']['resp_headers']['x-nonce-id']   ?? '';
    $chronos = $res3['result']['resp_headers']['x-chronos-id'] ?? '';
    if (!$nonce || !$chronos) { ooredooLog("[PLAY] no nonce/chronos"); return null; }

    $usedProxy = $res3['proxy'];
