...
define('OOR_LOG_FILE',     '/tmp/oor_requests.log');
...
function oorLogRequest(string $url, string $method, int $httpCode, string $proxy, string $responsePreview): void
{
    oorLog("REQUEST: {$method} {$url} | HTTP: {$httpCode} | PROXY: {$proxy} | RESP: " . substr($responsePreview, 0, 300));
}

// ════════════════════════════════════════════════════════════════════════════
// بروكسيات مخصصة لـ Ooredoo
// ════════════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — Curl مع بروكسيات مخصصة + معالجة ذكية
// ════════════════════════════════════════════════════════════════════════════
function oorCurl(string $url, string $method, string $body, array $headers): array
{
    ...
    if ($totalProxies === 0) {
        oorLog("ERROR: لا توجد بروكسيات لأوريدو!");
    ...
    foreach ($proxies as $idx => $proxy) {
        ...
        // تسجيل الطلب
        oorLogRequest($url, $method, $code, $proxy, $bodyStr);

        // فشل اتصال
        if ($err || $raw === false || $code === 0) {
            oorLog("PROXY_FAIL: {$proxy} | errno: {$errno} | err: {$err}");
            continue;
        }

        // التحقق من استجابة HTML (قيد على IP)
        $bodyLower = strtolower($bodyStr);
        if (stripos($bodyStr, '<html') !== false || stripos($bodyStr, '<!DOCTYPE') !== false) {
            oorLog("HTML_RESPONSE: {$proxy} | HTTP: {$code} | سيتم تجربة بروكسي آخر");
            $htmlResults[] = [
                'proxy' => $proxy,
                'code' => $code,
        ...
        // ✅ استجابة 200 مع JSON صالح → نجاح
        if ($code === 200 && is_array($json) && !empty($json)) {
            return ['http_code' => $code, 'body' => $bodyStr, 'json' => $json, 'resp_headers' => $respH];
        }

        // استجابة 201/403/409 (استجابات متوقعة) نعيدها فوراً
        if ($code === 201 || $code === 403 || $code === 409) {
            return ['http_code' => $code, 'body' => $bodyStr, 'json' => $json, 'resp_headers' => $respH];
        }

        // استجابات 4xx/5xx غير المتوقعة → نعيد المحاولة ببروكسي آخر
        oorLog("UNEXPECTED_HTTP: {$proxy} | HTTP: {$code} | إعادة محاولة...");
    }

    // ❌ كل البروكسيات فشلت أو أعادت HTML
    oorLog("ALL_PROXIES_FAILED: عدد محاولات HTML: " . count($htmlResults));

    if (count($htmlResults) === $totalProxies) {
        return ['http_code' => 503, 'body' => 'جميع البروكسيات أعادت HTML (قيد على IP)', 'json' => null, 'resp_headers' => []];
    }
...
function sendTgMainMenu(string $chatId): void
{
    ...
    $keyboard = [
        ...
        [
            ...
            ['text' => '📋 سجل أوريدو',                  'callback_data' => 'tg_oor_log'],
        ],
    ];
...
{
    if (!file_exists(OOR_LOG_FILE)) {
        tgSendMessage($chatId, "📋 لا يوجد سجل لأوريدو بعد.");
        return;
    }
    $log = file_get_contents(OOR_LOG_FILE);
    $lines = explode("\n", $log);
    $lastLines = array_slice($lines, -50);
    $text = "📋 <b>آخر 50 سطر من سجل أوريدو</b>\n\n<code>" . htmlspecialchars(implode("\n", $lastLines)) . "</code>";
...
function handleTgProxiesInput(string $chatId, string $text): void
{
    ...
    if (empty($valid)) {
        tgSendMessage($chatId, '❌ لا توجد بروكسيات بصيغة صحيحة. الصيغة: host:port:user:pass');
        return;
    }
...
function processEvent(string $psid, array $event): void
{
    ...
    // معالجة الأرقام - سواء في مرحلة menu أو في أي مرحلة أخرى
...
function handleAwaitingOtp(string $psid, string $text, array $session): void
{
    ...
    if (preg_match('/^05\d{8}$/', $digits)) {
        $newMsisdn = '213' . substr($digits, 1);
        sendMessage($psid, "📲 جاري إعادة إرسال رمز التحقق إلى الرقم {$digits}...");
        sendOorOtpAndWait($psid, $newMsisdn, $digits);
        return;
    }
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — Phone Handler (05)
// ════════════════════════════════════════════════════════════════════════════
function sendOorOtpAndWait(string $psid, string $msisdn, string $phone): void
{
    $statusCode = oorCheckStatus($msisdn);
    if ($statusCode !== '004') {
        sendMessage($psid, "❌ هذا الرقم غير موجود في شبكة Ooredoo أو غير نشط.");
        return;
    }
...
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — Main Menu (محسّن: لا يعرض نوع الشريحة إن لم يكن متاحاً)
// ════════════════════════════════════════════════════════════════════════════
...
function handleOorDailyGift(string $psid, string $msisdn, string $at, string $rt, array $sess): void
{
    ...
    // معالجة استجابة 500 (مثل سناب شات)
    if ($r['http_code'] === 500) {
        sendMessage($psid, "⏳ تم لعب الهدية مسبقاً أو لم تمر 24 ساعة بعد.");
        // تحديث الحالة محلياً
