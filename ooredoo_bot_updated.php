...
define('OOREDOO_INTEGRITY_1',  '');
define('OOREDOO_INTEGRITY_2',  '');
// ════════ Ooredoo — Dynamic Session Values (مُطابق لـ Python) ════════
// في Python: _iid() = str(uuid1()) + str(int(time()*1000))
// في Python: _pld(d) = base64(json({"platform":"android","is-physical-device":true,"device-id":d}))
// نُولّد هذه القيم ديناميكياً في كل جلسة تماماً كما يفعل Python

function ooredooGenerateIid(): string
{
    // uuid v1 (time-based) + timestamp_ms — مُطابق لـ Python: str(uuid.uuid1()) + str(_now_ms())
    $uuid1 = sprintf(
...
function ooredooGeneratePlatSig(string $iid): string
{
    // مُطابق لـ Python: base64(json({"platform":"android","is-physical-device":true,"device-id":iid}))
    $payload = json_encode([
        'platform'           => 'android',
...
function ooredooNewDeviceContext(): array
{
    $iid     = ooredooGenerateIid();
    $platSig = ooredooGeneratePlatSig($iid);
    return ['iid' => $iid, 'plat_sig' => $platSig];
}
...
// ════════════════════════════════════════════════════════════════════════════
// Back to School Gift Config (يتحكم بها من تلقرام)
// ════════════════════════════════════════════════════════════════════════════
define('BACK_TO_SCHOOL_FILE', '/tmp/back_to_school_config.json');

function getBackToSchoolConfig(): array
{
    $defaults = ['enabled' => false];
    if (!file_exists(BACK_TO_SCHOOL_FILE)) return $defaults;
    $d = json_decode(file_get_contents(BACK_TO_SCHOOL_FILE), true);
    return is_array($d) ? array_merge($defaults, $d) : $defaults;
}

function saveBackToSchoolConfig(array $cfg): void
{
    file_put_contents(BACK_TO_SCHOOL_FILE, json_encode($cfg, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
}
...
function sendTgMainMenu(string $chatId): void
{
    ...
    $backToSchoolCfg = getBackToSchoolConfig();
    $backToSchoolStatus = $backToSchoolCfg['enabled'] ? '✅ مفعّلة' : '❌ معطلة';
    ...
    $text = "🤖 <b>لوحة تحكم Tasjil BOT</b>\n\n"
          ...
          . "🔗 QR: <code>{$qr}</code>\n"
          . "━━━━━━━━━━━━━━━━━━━━\n"
          . "🎒 هدية العودة للمدرسة: <b>{$backToSchoolStatus}</b>\n"
          . "━━━━━━━━━━━━━━━━━━━━";
    ...
    $keyboard = [
        ...
        [
            ['text' => $backToSchoolCfg['enabled'] ? '🔴 إيقاف هدية العودة للمدرسة' : '🟢 تفعيل هدية العودة للمدرسة', 'callback_data' => 'tg_toggle_back_to_school'],
        ],
    ];
...
function handleTgCallback(string $chatId, int $msgId, string $data): void
{
    switch ($data) {
        ...
        case 'tg_edit_ooredoo_proxies':
            ...
            break;
        case 'tg_toggle_back_to_school':
            $cfg = getBackToSchoolConfig();
            $cfg['enabled'] = !$cfg['enabled'];
            saveBackToSchoolConfig($cfg);
            $status = $cfg['enabled'] ? '✅ مفعّلة' : '❌ معطلة';
            tgSendMessage($chatId, "🎒 هدية العودة للمدرسة الآن: <b>{$status}</b>");
            sendTgMainMenu($chatId);
            break;
...
function handleTgStats(string $chatId): void
{
    ...
    $text = "📊 <b>إحصائيات Tasjil BOT</b>\n\n"
          ...
          . "🎒 هدية العودة للمدرسة: <b>" . ($backToSchoolCfg['enabled'] ? '✅ مفعّلة' : '❌ معطلة') . "</b>\n\n"
...
function processEvent(string $psid, array $event): void
{
    ...
    if ($state === 'menu' || $state === 'offers') {
        // Ooredoo: رقم 1 = هدية يومية
        if ($text === '1' && ($session['operator'] ?? '') === 'ooredoo') {
            $user = getUser($psid);
        ...
        } elseif ($text === '0' && $backToSchoolCfg['enabled']) {
            // تفعيل هدية العودة للمدرسة
            handlePostback($psid, 'ACTIVATE_BACK_TO_SCHOOL');
        } elseif ($text === '1') {
            ...
            handlePostback($psid, 'MENU_MORE_OFFERS');
        } elseif ($text === '30') {
            $cfg  = getMatchGiftConfig();
            ...
            $backToSchoolText = $backToSchoolCfg['enabled'] ? "0️⃣ لتفعيل هدية العودة للمدرسة 2جيقا (صالحة لمدة 7 أيام)\n📩 أرسل: 0\n\n" : "";
            sendMessage($psid,
...
function handlePostback(string $psid, string $payload): void
{
    ...
    switch ($payload) {
        ...
        case 'ACTIVATE_ALGERIA_MATCH':
            ...
            activateAlgeriaMatchGift($psid, $user);
            break;
        case 'ACTIVATE_BACK_TO_SCHOOL':
            $backToSchoolCfg = getBackToSchoolConfig();
            if (!$backToSchoolCfg['enabled']) {
                sendMessage($psid, "⏰ هدية العودة للمدرسة غير متاحة حالياً.");
                return;
            }
            $sess = getSession($psid);
            $user = getUser($psid);
            ...
            if (!empty($sess['msisdn'])) $user['msisdn'] = $sess['msisdn'];
            activateBackToSchoolGift($psid, $user);
            break;
        case 'MENU_2G':
            $sess = getSession($psid); $user = getUser($psid);
            if (!$user || empty($user['access_token'])) { sendMessage($psid, "⚠️ يجب تسجيل الدخول أولاً، أرسل رقم هاتفك."); return; }
...
// ════════════════════════════════════════════════════════════════════════════
// Back to School Gift — تفعيل هدية العودة للمدرسة
// ════════════════════════════════════════════════════════════════════════════
function activateBackToSchoolGift(string $psid, array $user): void
{
    ...
    sendMessage($psid, "🔄 جاري تفعيل هدية العودة للمدرسة 🎒...");

    $url     = "https://apim.djezzy.dz/mobile-api/api/v1/subscribers/activate-product/{$msisdn}";
    $payload = json_encode(['packageCode' => 'BONUSBACK2SCHOOL7JR']);
    $headers = [
        'User-Agent: MobileApp/3.0.9',
    ...
    $raw = curlWithAllProxies($url, 'POST', $payload, $headers, 'BACK_TO_SCHOOL', 12, '/tmp/back_to_school.log');
    ...
    if ($httpCode === 200 || $httpCode === 201) {
        ...
        sendMessage($psid,
            "🎉 تم تفعيل هدية العودة للمدرسة بنجاح!\n\n" .
            "🎁 2 جيقا صالحة لمدة 7 أيام\n" .
            "✅ الرقم: {$displayMasked}\n\n" .
            "⚡ قناة التلغرام: https://t.me/tasjilbott"
        );
        clearSession($psid);
    ...
    if ($httpCode === 401) {
        sendMessage($psid,
            "⚠️ هدية العودة للمدرسة غير متاحة لهذا الرقم.\n\n" .
            "⚡ قناة التلغرام: https://t.me/tasjilbott"
        );
        return;
    }
...
// ════════════════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════════════════
// OOREDOO — Session Device Context Cache
// نُولّد iid+platsig مرة واحدة per-psid ونُعيد استخدامها في كل خطوات الجلسة
// مُطابق لـ Python: OoredooSession.__init__ يُولّد iid+psig مرة واحدة
// ════════════════════════════════════════════════════════════════════════════
function ooredooGetDeviceContext(string $psid): array
{
    $f = OOREDOO_SESSIONS_DIR . "/{$psid}_device.json";
    if (file_exists($f)) {
        $d = json_decode(file_get_contents($f), true);
        if (!empty($d['iid']) && !empty($d['plat_sig'])) return $d;
    }
    $ctx = ooredooNewDeviceContext();
    file_put_contents($f, json_encode($ctx));
    return $ctx;
}
function ooredooClearDeviceContext(string $psid): void
{
    $f = OOREDOO_SESSIONS_DIR . "/{$psid}_device.json";
    if (file_exists($f)) @unlink($f);
}

function ooredooBaseHeaders(string $msisdn, string $fingerprint, string $correlationId, string $timestamp, ?array $devCtx = null): array
{
    // إذا لم يُمرَّر devCtx نستخدم القيم الثابتة القديمة كـ fallback
    $iid     = $devCtx['iid']      ?? OOREDOO_INSTANCE_ID;
    $platSig = $devCtx['plat_sig'] ?? OOREDOO_PLAT_SIG;
...
{
    // ── توليد device context ديناميكي (مُطابق لـ Python: OoredooSession.__init__) ──
    $devCtx = $psid ? ooredooGetDeviceContext($psid) : ooredooNewDeviceContext();
    // ──────────────────────────────────────────────────────────────────────
    ...
    foreach ($candidates34 as $chkProxy) {
        ...
        $hdrs3 = ooredooBaseHeaders($msisdn, bin2hex(random_bytes(32)), bin2hex(random_bytes(8)) . $ts, $ts, $devCtx);
        ...
        // مُطابق لـ Python: _oid() — يستخدم x-platform-info بدل x-platform-data-signature
        $hdrs4 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
            'x-device-fingerprint: ' . bin2hex(random_bytes(32)),
            "x-nonce-id: {$nonce1}",
            'x-platform-origin: ' . OOREDOO_PLATFORM_ORIGIN,
            'platform: ' . OOREDOO_PLATFORM,
            'x-version: ' . OOREDOO_X_VERSION,
            ...
            "x-timestamp: {$ts}",
            'x-platform-info: ' . ($devCtx['plat_sig'] ?? OOREDOO_PLAT_SIG),
            'x-force-update: true',
            'x-instance-id: ' . ($devCtx['iid'] ?? OOREDOO_INSTANCE_ID),
        ];
 ...
 */
function ooredooVerifyOTP(string $msisdn, string $otp, ?array $stickyProxy, ?array $devCtx = null): array|string|false
{
    ...
    foreach ($candidates as $proxy) {
        ...
        // fingerprint عشوائي مُطابق لـ Python: _chk() يستخدم _hex()
        $hdrs6 = ooredooBaseHeaders($msisdn, bin2hex(random_bytes(32)), $corrId, $ts, $devCtx);
        ...
        // مُطابق لـ Python: _oid() — x-platform-info + fingerprint عشوائي
        $hdrs7 = [
            'User-Agent: Dart/3.11 (dart:io)',
            'Accept-Encoding: gzip',
            "x-msisdn: {$msisdn}",
...
{
    // timestamp حي — مُطابق لـ Python: x-timestamp يُولَّد في كل طلب
    $ts     = (string)(time() * 1000);
    $corrId = bin2hex(random_bytes(8)) . $ts;
    $devCtx = $tokHdrs['dev_ctx'] ?? null;
    ...
    $hdrs = [
        ...
        'x-version: ' . OOREDOO_X_VERSION,
        'x-signature: ' . OOREDOO_X_SIG,
        'x-platform-data-signature: ' . ($devCtx['plat_sig'] ?? OOREDOO_PLAT_SIG),
        "x-timestamp: {$ts}",
        'x-instance-id: ' . ($devCtx['iid'] ?? OOREDOO_INSTANCE_ID),
    ];
...
function ooredooGamificationPlay(string $msisdn, string $accessToken, ?array $tokenProxy = null, array $tokHdrs = []): ?array
{
    ...
    // fingerprint عشوائي مُطابق لـ Python — devCtx من tokHdrs إذا توفر
    $devCtx = $tokHdrs['dev_ctx'] ?? null;
    $hdrs3  = ooredooBaseHeaders($msisdn, bin2hex(random_bytes(32)), $corrId, $ts, $devCtx);
    $hdrs3[] = 'content-type: application/json; charset=utf-8';
    $hdrs3[] = 'x-native-integrity-token: ' . OOREDOO_INTEGRITY_1;
    ...
    // play يُرسل من نفس الـ proxy كـ checkpoint — timestamp حي مُطابق لـ Python
    $playTs   = (string)(time() * 1000);
    $playCorrId = bin2hex(random_bytes(8)) . $playTs;
    $hdrsPlay = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
        "x-msisdn: {$msisdn}",
...
function handleNewOoredooPhone(string $psid, string $phone): void
{
    ...
    ooredooClearDeviceContext($psid); // نُعيد توليد device context جديد لكل جلسة
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    if (preg_match('/^05\d{8}$/', preg_replace('/\D/', '', $text))) {
        $newPhone  = preg_replace('/\D/', '', $text);
        $newMsisdn = '213' . substr($newPhone, 1);
        sendMessage($psid, "📲 جاري إعادة إرسال رمز التحقق إلى {$newPhone}...");
        ooredooClearDeviceContext($psid);
    ...
    $devCtx = ooredooGetDeviceContext($psid);
    $result = ooredooVerifyOTP($msisdn, $m[1], $stickyProxy, $devCtx);
    if ($result === 'wrong_otp') {
        sendMessage($psid, "❌ الرمز المُدخل خاطئ!\n\n🔄 أعد إرسال الرمز الصحيح\n📱 أو أرسل رقمك مجدداً\n\n❌ للإلغاء أرسل: 0");
        return;
    }
...
function ooredooSendPostLoginInfo(string $psid, array $user): void
{
    ...
    if ($giftData !== null) {
        ...
        if ($played && $lastPlayed) {
            ...
            $nextAvail = $lastTs + (24 * 3600);
            $remaining = $nextAvail - time();
            if ($remaining > 0) {
    ...
    $text .= "\n━━━━━━━━━━━━━━\n\n";
    $text .= "📌 اختر ما تريد:\n\n";
    $text .= "1️⃣ تفعيل الهدية اليومية\n📩 أرسل: 1\n\n";
    $text .= "━━━━━━━━━━━━━━";

    $giftReady = (!($giftData['played'] ?? true));
    if (!$giftReady && isset($giftData['lastPlayedTime'])) {
        $lastTs = strtotime($giftData['lastPlayedTime']);
        $giftReady = (time() - $lastTs) >= (24 * 3600);
    }

    $btnTitle = $giftReady ? '🎁 تفعيل الهدية اليومية' : '⏳ الهدية غير متاحة بعد';
    ...
    fbApiCall(json_encode([
        ...
        'message'        => [
            'text'          => $text,
            'quick_replies' => [
                ['content_type' => 'text', 'title' => $btnTitle, 'payload' => 'OOREDOO_DAILY_GIFT'],
            ],
        ],
    ], JSON_UNESCAPED_UNICODE));
}
 ...
 */
function ooredooRefreshToken(string $refreshToken, string $msisdn, ?array $tokenProxy = null, ?array $devCtx = null): array|false
{
    // Refresh يحتاج نفس بروكسي التوكن الأصلي — timestamp حي
    $ts   = (string)(time() * 1000);
    $hdrs = [
        'User-Agent: Dart/3.11 (dart:io)',
        'Accept-Encoding: gzip',
    ...
    foreach ($candidates as $proxy) {
        ...
        if ($code === 200 && isset($json['access_token'])) {
            ...
            return [
                ...
                'tok_fingerprint'   => bin2hex(random_bytes(32)),
                'tok_timestamp'     => $ts,
                'tok_instance_id'   => $devCtx['iid'] ?? OOREDOO_INSTANCE_ID,
                'tok_plat_sig'      => $devCtx['plat_sig'] ?? OOREDOO_PLAT_SIG,
                'tok_correlation'   => bin2hex(random_bytes(8)) . $ts,
            ];
        }
    }
    return false;
...
function sendMenu(string $psid): void
{
    ...
    // إضافة هدية المباراة إذا كانت مفعلة
    if ($cfg['enabled']) {
        $extraRows[] = ['content_type' => 'text', 'title' => '🇩🇿 هدية المباراة', 'payload' => 'ACTIVATE_ALGERIA_MATCH'];
    }

    // إضافة هدية العودة للمدرسة إذا كانت مفعلة
    if ($backToSchoolCfg['enabled']) {
        $extraRows[] = ['content_type' => 'text', 'title' => '🎒 هدية العودة للمدرسة', 'payload' => 'ACTIVATE_BACK_TO_SCHOOL'];
    }
    ...
    fbApiCall(json_encode([
        ...
        'message'        => [
            'text' =>
                "📱 اختر العرض المناسب\n\n📌 قم باستخدام الأزرار الموجودة بالأسفل\nإذا لم تظهر لك الأزرار أرسل الرقم المناسب 👇\n\n━━━━━━━━━━━━━━\n\n" .
                ($backToSchoolCfg['enabled'] ? "0️⃣ لتفعيل هدية العودة للمدرسة 2جيقا (صالحة لمدة 7 أيام)\n📩 أرسل: 0\n\n" : "") .
                "1️⃣ لتفعيل 2G الأسبوعية\n📩 أرسل: 1\n\n" .
                "2️⃣ لتفعيل عرض 4GB بـ 70دج 🏷️\n📩 أرسل: 2\n\n" .
                "3️⃣ لإرسال دعوة 🎁\n📩 أرسل: 3\n\n" .
                "4️⃣ للمزيد من العروض 📦\n📩 أرسل: 4\n\n━━━━━━━━━━━━━━\n\n" .
            ...
            'quick_replies' => array_merge(
                $backToSchoolCfg['enabled'] ? [['content_type' => 'text', 'title' => '🎒 هدية العودة للمدرسة', 'payload' => 'ACTIVATE_BACK_TO_SCHOOL']] : [],
                [
