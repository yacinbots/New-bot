...
// ════════ Ooredoo — قيم ديناميكية (تُولَّد في runtime) ════════
// OOREDOO_DEVICE_ID, OOREDOO_INSTANCE_ID, OOREDOO_PLAT_SIG → see ooredooGetSessionIdentity()
...
// ════════ Ooredoo — fingerprints و timestamps تُولَّد ديناميكياً ════════
// جميع القيم السابقة الثابتة (FP_AUTHED, FP_TOKEN1, FP_TOKEN2, FP_GPLAY_CHK,
// CORR_AUTHED, timestamps الثابتة) حُذفت — تُولَّد الآن بـ ooredooRandHex()
// وooredooCorrId() وooredooNowMs() في كل طلب.
...
/**
 * ooredooNowMs — epoch milliseconds كـ string
 */
...
/**
 * ooredooRandHex — 64-char lowercase hex (x-device-fingerprint)
 * يتغير بكل طلب — لا يُعاد استخدامه أبداً
 */
function ooredooRandHex(): string
{
    return bin2hex(random_bytes(32));
}

/**
 * ooredooCorrId — UUID v1-style + timestamp_ms
 * يُستخدم لـ x-correlation-id
 */
...
/**
 * ooredooInstanceId — UUID v1 (version nibble = '1') + timestamp_ms
 * نفس بنية HAR: 7e12faf0-aabd-11f1-bcd7-b7386e3edfc9 + 1788786626591
 * يُولَّد مرة واحدة لكل جلسة مستخدم ويُحفظ في الـ session file.
 */
...
/**
 * ooredooPlatformSig — base64({platform,is-physical-device,device-id})
 * يُولَّد من instance_id — ثابت طول الجلسة، ديناميكي بين الجلسات.
 */
...
/**
 * ooredooGetSessionIdentity — يُعيد أو ينشئ هوية الجلسة (instance_id + platform_sig)
 * يُخزَّن في ملف الجلسة الخاص بالمستخدم — ثابت طول جلسة الـ OTP.
 * بعد تسجيل الدخول ناجح، يُحفظ مع access_token في getUser().
 */
function ooredooGetSessionIdentity(string $psid): array
{
    ...
    // مستخدم مسجّل مسبقاً → نقرأ من ملف المستخدم
    $user = getUser($psid);
    ...
    return [
        'instance_id'  => $iid,
        'platform_sig' => ooredooPlatformSig($iid),
    ];
}
...
// ════════════════════════════════════════════════════════════════════════════
/**
 * ooredooBaseHeaders — headers أساسية لـ checkpoint و طلبات BFF العادية (android platform)
 * $psid مطلوب لاستخراج instance_id + platform_sig الخاصَّين بالمستخدم.
 * $fingerprint/$correlationId/$timestamp: عشوائية جديدة لكل طلب إذا لم تُمرَّر.
 */
function ooredooBaseHeaders(
    string  $msisdn,
    string  $psid,
    string  $fingerprint    = '',
    ...
    $fp   = $fingerprint   !== '' ? $fingerprint   : ooredooRandHex();
    $corr = $correlationId !== '' ? $correlationId : ooredooCorrId();
    $ts   = $timestamp     !== '' ? $timestamp     : ooredooNowMs();
...
function ooredooSendOTP(string $msisdn, string $psid = ''): array|false
{
    ...
    // حساب هوية الجلسة مرة واحدة — نستخدمها في كل الطلبات
    ...
    foreach ($candidates34 as $chkProxy) {
        ...
        if (!$r3 || ($r3['ip_blocked'] ?? false)) {
            ooredooLog('[OTP] Step3 fail proxy=' . ($chkProxy['host'] ?? 'direct'));
            continue;
        }
...
{
    // استخراج هوية الجلسة من الملف المحفوظ (نفس instance_id الذي أرسلنا به checkpoint1)
    $identity = ooredooGetSessionIdentity($psid);
    $iid      = $identity['instance_id'];
    $platSig  = $identity['platform_sig'];
        ...
        if ($code7 === 200) {
            ...
            if (isset($json['access_token'])) {
                ...
                return [
                    ...
                    // قيم headers الديناميكية التي ولّدت التوكن — تُحفظ مع المستخدم
                    'tok_fingerprint'   => ooredooRandHex(), // fingerprint جديد لكل طلب لاحق
                    'tok_timestamp'     => $ts,
                    'tok_instance_id'   => $iid,
...
// ════════════════════════════════════════════════════════════════════════════
/**
 * ooredooGamificationStatus
 *
 * الاكتشاف الحاسم (من الاختبار المباشر):
 *   - platform=android → 401 (BFF يتطلب Play Integrity session)
 *   - platform=web + x-platform-origin=web + origin=https://my.ooredoo.dz → 200 ✅
 *
 * Web clients لا يمرون بـ Play Integrity → البف يتجاوز الفحص.
 * نحافظ على كل headers android الأخرى (msisdn, fingerprint, instance-id, etc.)
 * لأنها مطلوبة للبف داخلياً لتحديد المستخدم.
 */
function ooredooGamificationStatus(string $msisdn, string $accessToken, ?array $proxy = null, array $tokHdrs = [], string $psid = ''): ?array
{
    ...
    $hdrs = [
        ...
        "x-msisdn: {$msisdn}",
        'x-device-fingerprint: '         . ooredooRandHex(),
        // platform=web + x-platform-origin=web → يتجاوز Play Integrity check
    ...
    $code = $r['http_code'] ?? 0;
    if ($code !== 200) {
        ooredooLog("[GSTATUS] http={$code} proxy=" . ($proxy['host'] ?? 'direct'));
        return null;
    }
    return $r['json'] ?? null;
}
...
function ooredooValidateUser(string $msisdn, string $accessToken, ?array $proxy = null, array $tokHdrs = [], string $psid = ''): ?array
{
    ...
    $code = $r['http_code'] ?? 0;
    if ($code === 401) {
        ooredooLog("[VALIDATE] 401 proxy=" . ($proxy['host'] ?? 'direct'));
        return null;
    }
    return $r['json'] ?? null;
}
...
function handleNewOoredooPhone(string $psid, string $phone): void
{
    ...
    // توليد هوية الجلسة وحفظها قبل إرسال OTP
    $identity = ooredooGetSessionIdentity($psid);
    ...
    // حفظ instance_id + proxy للخطوة التالية
    ooredooSaveSession($psid, [
        'step'         => 'awaiting_otp',
        'msisdn'       => $msisdn,
        'ts'           => time(),
        'instance_id'  => $identity['instance_id'],
        'platform_sig' => $identity['platform_sig'],
    ]);
...
function handleAwaitingOoredooOtp(string $psid, string $text, array $session): void
{
    ...
    if (preg_match('/^05\d{8}$/', preg_replace('/\D/', '', $text))) {
        ...
        $newIdentity = ooredooGetSessionIdentity($psid);
        $otpRes = ooredooSendOTP($newMsisdn, $psid);
        if ($otpRes === false) {
            sendMessage($psid, "❌ تعذر إرسال الرمز، حاول مجدداً."); return;
        }
        ooredooSaveSession($psid, ['step' => 'awaiting_otp', 'msisdn' => $newMsisdn, 'ts' => time(), 'instance_id' => $newIdentity['instance_id'], 'platform_sig' => $newIdentity['platform_sig']]);
        ooredooSetStickyProxy($psid, $otpRes['proxy'] ?? []);
    ...
    // هوية الجلسة — محفوظة من مرحلة إرسال الـ OTP
    $identity = ooredooGetSessionIdentity($psid);
    ...
    $user = [
        ...
        // قيم headers ديناميكية مرتبطة بالتوكن
        'tok_fingerprint'  => $result['tok_fingerprint']  ?? ooredooRandHex(),
        'tok_timestamp'    => $result['tok_timestamp']    ?? ooredooNowMs(),
        'tok_instance_id'  => $result['tok_instance_id']  ?? $identity['instance_id'],
        'tok_plat_sig'     => $result['tok_plat_sig']     ?? $identity['platform_sig'],
...
{
    // instance_id و plat_sig: محفوظان من جلسة التسجيل الأصلية — ثابتان لهذا المستخدم
    return [
        ...
        'correlation'    => $user['tok_correlation'] ?? ooredooCorrId(),
    ];
}
